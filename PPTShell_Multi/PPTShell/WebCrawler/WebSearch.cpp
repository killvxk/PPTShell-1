...
void WebSearch::BuildJsonEntity4PPT()
{
	...
	for(int i = 0; i < vecSize; i++)
	{
		...
		jsonEntity->Name = Un2Str(m_vecMatchName.at(i));
		jsonEntity->objAddress = Un2Str(m_vecMatchObjAddress.at(i));
...
void WebSearch::SearchPic(CStringW keyWord, int begin)
{
	...
	for(int i = 0; i < vecSize; i++)
	{
		...
		jsonEntity->Name = Un2Str(m_vecMatchName.at(i));
		jsonEntity->objAddress = Un2Str(m_vecMatchObjAddress.at(i));
		jsonEntity->previewsAddress = Un2Str(m_vecMatchPreviewsAddress.at(i));
...
tstring WebSearch::JsonResInfoByType(tstring strKeyWord, ENResType enResType, UINT uIBeginCount, UINT uIEndCount)
{
	...
	StrCpyW((LPWSTR)chResTypeNa, (LPWSTR)g_chResTypeNa[enResType]);
	StrCpyW((LPWSTR)chResTypeForm, (LPWSTR)g_chResTypeNa[enResType]);
	StrCpyW((LPWSTR)chResTypeUrl, (LPWSTR)g_chResTypeNa[enResType]);

	strcat(chResTypeNa, "Na");
	strcat(chResTypeForm, "PreAdd");
	strcat(chResTypeUrl, "ObjAdd");
	...
	{  
		...
		json_temp[chResTypeNa] = Json::Value((*iter).Name.c_str());
		json_temp[chResTypeForm] = Json::Value((*iter).previewsAddress.c_str());
		json_temp[chResTypeUrl] = Json::Value((*iter).objAddress.c_str());
		itemArray.append(json_temp);
	}
	json_temp0["ItemsInfo"] = itemArray;
	string strRet = json_temp0.toStyledString();
