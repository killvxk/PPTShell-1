...
void CWANClientManager::OnMainWanClientPacket(SOCKET sock, int type, char *packet)
{
	...
	if( pHeader->dwMajorType == PT_REGISTER )
	{
		...
		if (pWanClient == NULL)
		{
			WRITE_LOG_LOCAL("WANClientManager::OnMainWanClientPacket, new CWANClient fail!")
			return ;
		}
		...
		{
			delete pWanClient;
			WRITE_LOG_LOCAL("WANClientManager::OnMainWanClientPacket, CWANClient Initialize fail!")
			return;
		}
		...
		{
			delete pWanClient;
			WRITE_LOG_LOCAL("WANClientManager::OnMainWanClientPacket, Connect PPTServer fail!ip:%s, port:%d", m_szWanServerIP, m_nWanServerPort)
			return;
		}
...
void CWANClientManager::OnWanClientClosed(SOCKET sock)
{
	...
	else
	{
		...
		if (pClient == NULL)
		{
			WRITE_LOG_LOCAL("CWANClientManager::OnWanClientClosed, Impossible to get here, sock:%u", sock);
		}
		else
		{
...
DWORD WINAPI CWANClientManager::MainClientConnectThread(LPARAM lParam)
{
	...
	WRITE_LOG_LOCAL("MainClientConnectThread exit");
	return 0;
}
...
void CWANClientManager::DealHeartbeat()
{
	while(TRUE)
	{
		...
		if (ret != 0)
		{
			WRITE_LOG_LOCAL("CWANClientManager::DealHeartbeat, WaitForSingleObject fail!");
			continue;
		}
		...
		WRITE_LOG_LOCAL("----------HeartBeat thread start to work!!!-------------");
			...
			while (true)
			{
				...
				for (; itrClient!=m_mapHeartbeatClients.end(); )
				{
					...
					if (!pClient->IfRcvHeartBeatAck())
					{
						...
						{
							WRITE_LOG_LOCAL("Have not receive heartbeat ack for 3 times, will  break up the socket! client:%d", pClient->GetSocket());
