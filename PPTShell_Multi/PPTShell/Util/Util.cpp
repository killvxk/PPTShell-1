...
tstring UrlEncode(tstring str, bool bFullUrl /*= true*/)
{
	tstring strTemp = "";
...
tstring		UrlEncodeEx(tstring str)
{
	...
	tstring strTemp = "";
	size_t length = str.length();
	size_t length1 = strUnsafe.length();
	bool bFind ;
	for (size_t i = 0; i < length; i++)
	{
		bFind = false;
		...
		if(bFind)
		{
			...
			{
				nTemp = nChar % 16;
				nChar = (int)floor((double)(nChar / 16));
				strTemp1 = hexVals[nTemp];
			}

			strTemp1 += hexVals[nChar];
			if(strTemp1.length() < 2)
			{
				strTemp1 += '0';
			}

			char * pszBuff = strrev((char *)strTemp1.c_str());
			strTemp += "%";
			strTemp += pszBuff;
		}
		else
...
{  
	...
	HWND _hwnd = FindWindow("NdCefOSRWindowsClass",NULL);
	if (IsWindow(_hwnd)){
...
bool createDirWithFullPath(char* strDirName)
{
	string strRelativePath(strDirName);
	int nPos = 1;
	string strTmpPath;
...
bool SavePassword( tstring strUserName, tstring strPassword )
{
	...
	TCHAR szKey[] = {'a','e','i','o','u','\0'};
	TCHAR szEncryptUserName[MAX_PATH * 2] = {0};
	EncryptA(strUserName.c_str(), strUserName.length(), szEncryptUserName, szKey);
	tstring strOut;
	CBase64::Encode((unsigned char *)szEncryptUserName, _tcslen(szEncryptUserName), strOut);

	_tcscpy_s(PasswordUnit.szUserName, MAX_PATH * 2, strOut.c_str());
	PasswordUnit.szUserName[MAX_PATH * 2] = 0;

	TCHAR szEncryptPassword[MAX_PATH * 2] = {0};
	EncryptA(strPassword.c_str(), strPassword.length(), szEncryptPassword, strUserName.c_str());
	CBase64::Encode((unsigned char *)szEncryptPassword, _tcslen(szEncryptPassword), strOut);

	_tcscpy_s(PasswordUnit.szPassword, MAX_PATH * 2, strOut.c_str());
...
bool GetPassword(LPTSTR szUserName, LPTSTR szPassword)
{
	...
	if(fp)
	{
		...
		TCHAR szDecryptUserName[MAX_PATH] = {0};
		TCHAR szDecryptUserName1[MAX_PATH] = {0};
		unsigned long nLen = MAX_PATH;
		const tstring strDecryptUserName = PasswordUnit.szUserName;
		...
		TCHAR szKey[] = {'a','e','i','o','u','\0'};
		DecryptA(szDecryptUserName, _tcslen(szDecryptUserName), szDecryptUserName1, szKey);
		_tcscpy_s(szUserName, MAX_PATH - 1, szDecryptUserName1);


		TCHAR szDecryptPassword[MAX_PATH] = {0};
		TCHAR szDecryptPassword1[MAX_PATH] = {0};
		const tstring strDecryptPassword = PasswordUnit.szPassword;
		nLen = MAX_PATH;
		...
		DecryptA(szDecryptPassword, _tcslen(szDecryptPassword), szDecryptPassword1, szUserName);
		
		_tcscpy_s(szPassword, MAX_PATH - 1, szDecryptPassword1);
		fclose(fp);
...
void EncryptA(const char* pInput , int nInPutLen, TCHAR * pOutput, const TCHAR * szKey)
{
	TCHAR szTemp[3] = {0};
	...
	for(int i = 0 ; i < nInPutLen ; i++)
	{
		...
		_stprintf(szTemp,"%0.2x",pInput[i]^szKey[i % _tcslen(szKey)]);
		pOutput[i*2] = szTemp[0];
		pOutput[i*2+1] = szTemp[1];
	}
}

void DecryptA(const char * pInput,int nInPutLen ,char * pOutput, const TCHAR * szKey)
{
	TCHAR szTemp[3] = {0};
	UINT nValue;
	for(int i = 0 ; i < nInPutLen/2 ; i++)
	{
		...
		nValue = nValue ^ szKey[i % _tcslen(szKey)];
		pOutput[i] = nValue;
	}
}
...
// zip
bool ZipPackDir( tstring strSrcDicPath, tstring strDestFile )
{
	return CZipper::ZipFolder(strSrcDicPath.c_str(), true);
}

bool UnZipPackDir( tstring strSrcFile, tstring strDestPath )
{
	CUnZipper UnZipper;
...
bool CopyDir( CString strSrcPath, CString strDstPath, LPTSTR szFailPath )
{
	...
	while(bWorking)
	{
		...
		if(!finder.IsDots())
		{
			...
			{
				BOOL bRet = CopyFile(strSrc, strDst, FALSE);
				if(!bRet)
				{
...
void _SetProcessDPIAware()
{
	...
	if( hUser32 )
	{
		...
		LPSetProcessDPIAware pSetProcessDPIAware = ( LPSetProcessDPIAware )GetProcAddress( hUser32, _T("SetProcessDPIAware"));
...
tstring GenerateQuestionThumbnail(tstring strPath)
{
	...
	_stprintf_s(szParam, _T("thumbnail %s?main=/%s&sys=pptshell&hidePage=footer \"%s\""), 
		UrlEncode(AnsiToUtf8(szPlayerPath)).c_str(), 
	...
	ShellExecute(NULL, "open", szExePath, szParam, strWorkDirectory.c_str(), SW_HIDE);
