...
{ 
	...
	m_fnIcrInit = (LPFN_IcrInit)GetProcAddress(m_hIcrModule, _T("IcrInit"));
	if( m_fnIcrInit == NULL )
		return FALSE;

	m_fnIcrSetCallBack = (LPFN_IcrSetCallback)GetProcAddress(m_hIcrModule, _T("IcrSetCallback"));
	if( m_fnIcrSetCallBack == NULL )
		return FALSE;

	m_fnIcrOnStart = (LPFN_IcrOnStart)GetProcAddress(m_hIcrModule, _T("IcrOnStart"));
	if( m_fnIcrOnStart == NULL )
		return FALSE;

	m_fnIcrOnClick = (LPFN_IcrOnClick)GetProcAddress(m_hIcrModule, _T("IcrOnClick"));
	if( m_fnIcrOnClick == NULL )
		return FALSE;

	m_fnIcrOnPageIndexChange = (LPFN_IcrOnPageIndexChange)GetProcAddress(m_hIcrModule, _T("IcrOnPageIndexChange"));
	if( m_fnIcrOnPageIndexChange == NULL )
		return FALSE;

	m_fnIcrOnRecvExamEvent = (LPFN_IcrOnRecvExamEvent)GetProcAddress(m_hIcrModule, _T("IcrOnRecvExamEvent"));
	if( m_fnIcrOnRecvExamEvent == NULL )
		return FALSE;

	m_fnIcrGetOnlineStudents = (LPFN_IcrGetOnlineStudents)GetProcAddress(m_hIcrModule, _T("IcrGetOnlineStudents"));
	if( m_fnIcrGetOnlineStudents == NULL )
		return FALSE;
	
	m_fnIcrInvokeNativeMethod = (LPFN_IcrInvokeNativeMethod)GetProcAddress(m_hIcrModule, _T("IcrInvokeNativeMethod"));
	if( m_fnIcrInvokeNativeMethod == NULL )
		return FALSE;

	m_fnIcrSendCmd = (LPFN_IcrSendCmd)GetProcAddress(m_hIcrModule, _T("IcrSendCmd"));
	/*if( m_fnIcrSendCmd == NULL )
	return FALSE;*/

	m_fnIcrRelease = (LPFN_IcrRelease)GetProcAddress(m_hIcrModule, _T("IcrRelease"));
	if( m_fnIcrRelease == NULL )
		return FALSE;

	m_fnIcrStop = (LPFN_IcrStop)GetProcAddress(m_hIcrModule, _T("IcrStop"));
	if( m_fnIcrStop == NULL )
		return FALSE;

	m_fnIcrUnInit = (LPFN_IcrUnInit)GetProcAddress(m_hIcrModule, _T("IcrUnInit"));
...
BOOL CIcrPlayer::IcrOnStart(CStream* pStream)
{
	...
	WRITE_LOG_LOCAL("IcrOnStart TeacherName:%s", strTeacherName.c_str());
	...
	WRITE_LOG_LOCAL("IcrOnStart ClassId:%s", strClassId.c_str());
	...
	WRITE_LOG_LOCAL("IcrOnStart StudentInfo:%s", strStudentInfo.c_str());
	...
	WRITE_LOG_LOCAL("IcrOnStart GrouopInfo:%s", strGroupInfo.c_str());
	if (nFreeMode == 1)//加一重保险，本身在设置模式为1的时候已经将字符串置空
	{
		strGroupInfo = _T("");
	}
	root["groups"] = strGroupInfo;

	// devices
	tstring strDeviceInfo = NDCloudUser::GetInstance()->GetDeviceInfo();
	WRITE_LOG_LOCAL("IcrOnStart DeviceInfo:%s", strDeviceInfo.c_str());
	if (nFreeMode == 1)//加一重保险，本身在设置模式为1的时候已经将字符串置空
	{
		strDeviceInfo = _T("");
	}
	root["devices"] = strDeviceInfo;
	...
	m_bUseStart = true;
	WRITE_LOG_LOCAL("IcrOnStart FullInfo:%s", str.c_str());
...
{
	SendEventToCefByThread(szEventName, szEventData);
	return true;
}
...
int CALLBACK CIcrPlayer::IcrMessageBox(const char* szText, const char* szCaption, unsigned int uType)
{
	...
	if( uType == MB_OK )
	{
		UIMessageBox(hWnd, szText, szCaption, _T("确定"), CMessageBoxUI::enMessageBoxTypeSucceed);
		return IDOK;
	}
	else if( uType == MB_OKCANCEL )
	{
		ret = UIMessageBox(hWnd, szText, szCaption, _T("确定,取消"), CMessageBoxUI::enMessageBoxTypeQuestion);
	...
	{
		ret = UIMessageBox(hWnd, szText, szCaption, _T("是,否"), CMessageBoxUI::enMessageBoxTypeQuestion);
		if( ret == ID_MSGBOX_BTN )
			return IDYES;
...
void CALLBACK CIcrPlayer::IcrShowToast(const char *szText, int nDuration)
{
	...
	CScreenToast::Toast(hWnd, szText, false, nDuration);
}
...
char* CALLBACK CIcrPlayer::IcrGetExamType()
{
	...
	strcpy(pQuestionType, Str2Ansi(strQuestionType).c_str());
...
void CALLBACK CIcrPlayer::IcrSetButtonState(int nUIComponentId, int nButtonId, int nState)
{
	...
	WRITE_LOG_LOCAL("IcrSetButtonState ComponentID:%d,ButtonID:%d,State:%d", nUIComponentId, nButtonId, nState);//方便排查工具栏隐藏问题2016.03.01 cws
	CScreenInstrumentUI* pScreenInstrumentUI = CScreenInstrumentUI::GetMainInstrument();
	if( pScreenInstrumentUI == NULL )
		return;
	...
	else if( nUIComponentId == ICR_UICOMPONET_TOOLBAR )
	{
		...
		case ICR_BUTTON_ID_HOMEWORK:
			pButton = pView->GetItem(_T("布置作业"));
			break;

		case ICR_BUTTON_ID_VOTE:
