...
// 	else
	{
		...
		{
			CCloudItemUI::m_dwResourceDownloadId = pHttpDownloadManager->AddTask(_T("mooc-lms.web.sdp.101.com"),szPost, _T(""), _T("GET"),_T(""), 80,MakeHttpDelegate(this, &CNdpCourseItemUI::OnGetNdpDownloadUrl) ,MakeHttpDelegate(NULL), MakeHttpDelegate(NULL) );
		}
...
bool CNdpCourseItemUI::OnGetNdpDownloadUrl( void * pParam )
{
	...
	if (reader.parse(str.c_str(), result))
	{
		...
		{
			m_strMd5 = result["md5"].asCString();
			m_strDownloadUrl = result["url"].asCString();
...
bool CNdpCourseItemUI::OnNdpCourseDownloaded( void * pParam )
{
	...
	if( pNotify->strFilePath != _T("") )
	{
		...
		if(GetFileAttributes(strXml.c_str()) != INVALID_FILE_ATTRIBUTES)
		{
			...
			strXml = UrlEncode(AnsiToUtf8(strXml));
			strPlayerPath = UrlEncode(AnsiToUtf8(strPlayerPath));
			...
			pCoursePlayUI->Init((WCHAR *)AnsiToUnicode(szParam).c_str(),COURSEPLAY_PLAYER);
