...
void CAssetsItem::DoInit()
{
	...
	CDuiRect rcCalc;
	rcCalc.right = GetFixedWidth();
	CalcText(GetManager()->GetPaintDC(), rcCalc, m_strName.c_str(), m_pTextLabel->GetFont(), DT_CALCRECT | DT_WORDBREAK | DT_EDITCONTROL | DT_LEFT|DT_NOPREFIX, UIFONT_GDI);
	int nLen = rcCalc.GetWidth();
	pLabel->SetPadding(CDuiRect((GetFixedWidth() - nLen)/2, 3, 0 ,0));
	pLabel->SetFixedWidth(nLen);
	...
	m_pGif = pGif;
	pGif->SetPadding(CDuiRect(0,3,0,0));
	pGif->SetBkImage("RightBar\\loading.gif");
...
void CAssetsItem::SetCountText( int nCount )
{
	...
	CDuiRect rcCalc;
	rcCalc.right = GetFixedWidth();
	CalcText(GetManager()->GetPaintDC(), rcCalc, szText, m_pCountLabel->GetFont(), DT_CALCRECT | DT_WORDBREAK | DT_EDITCONTROL | DT_LEFT|DT_NOPREFIX, UIFONT_GDI);
	int nLen = rcCalc.GetWidth();

	m_pCountLabel->SetPadding(CDuiRect((GetFixedWidth() - nLen)/2, 3, 0 ,0));
