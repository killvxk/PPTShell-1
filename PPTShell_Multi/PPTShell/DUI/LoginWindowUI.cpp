...
//#endif
	...
	if(std::tr1::regex_search(strUserName, sResult, szUserNameRule))
	{
		//手机
		std::tr1::regex szUserNameRuleEx("^1[34578]\\d{9}$");

		if(!std::tr1::regex_search(strUserName, sResult, szUserNameRuleEx))
		{
			ShowLoginTip(0, _T("您输入的手机号码格式不正确 ,请输入11位有效的手机号码"));
			//ShowLoginTip(0, _T("用户名格式不正确"));
			return;
		}
	}
	else
	{
		...
		if(!std::tr1::regex_search(strUserName, sResult, szUserNameRuleEx))
		{
			ShowLoginTip(0, _T("您输入的邮箱格式有误 ,请输入正确的邮箱"));
			//ShowLoginTip(0, _T("用户名格式不正确"));
			return;
		}
	}*/
	...
	case UCSTEP_LOGIN:
		{
			if( pUser->IsSuccess() )
			{
				...
				TCHAR szPost[1024];
				_stprintf(szPost, _T("/101ppt/getChapter.php?account=%d"),NDCloudUser::GetInstance()->GetUserId());
				if(pHttpDownloadManager)
				{
					pHttpDownloadManager->AddTask(_T("p.101.com"),szPost, _T(""), _T("Get"),_T(""), 80,MakeHttpDelegate(this, &CLoginWindowUI::OnUCLoginSaveStatus) ,MakeHttpDelegate(NULL), MakeHttpDelegate(NULL) );
				}
			}
			else
			{
...
bool CLoginWindowUI::OnUCLoginSaveStatus( void * pParam )
{
	...
	if(pNotify->dwErrorCode == 0)
	{
		...
		if( res )
		{
			...
			{
				strChapter = root["Chapter"].asCString();
				...
				if(sscanf_s(strChapter.c_str(),_T("%[^/]/%[^/]/%[^/]/%[^/]/%[^/]/%[^/]/%[^/]"), 
					szSectionCode, sizeof(szSectionCode) - 1,
					szGradeCode, sizeof(szGradeCode) - 1,
					szCourseCode, sizeof(szCourseCode) - 1,
					szEditionCode, sizeof(szEditionCode) - 1,
					szSubEditionCode,sizeof(szSubEditionCode) - 1,
					szChapterGuid, sizeof(szChapterGuid) - 1,
					szGrade, sizeof(szGrade) - 1) == 7)
				{
					...
					m_strGrade = szGrade;
					m_strGrade = Utf8ToAnsi(m_strGrade);
