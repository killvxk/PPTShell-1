...
void CEditClearUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("tipcolor")) == 0 )
	{
		...
		LPTSTR pstr = NULL;
		m_dwTipColor = _tcstoul(pstrValue, &pstr, 16);

		m_dwNormalTextColor =  GetTextColor();
		SetTextColor(m_dwTipColor);
...
void CEditClearUI::DoEvent(TEventUI& event)
{
	if( event.Type == UIEVENT_SETFOCUS )
	{
		...
		if(strText == m_tiptext || strText == _T(""))
		{
			SetTextColor(m_dwNormalTextColor);
			SetText(_T(""));
		}
	}
...
void CEditClearUI::PaintText( HDC hDC )
{
	...
	if( m_bPasswordMode ) {
		if(_tcsicmp(m_sText.GetData(), m_tiptext.c_str()) == 0)
		{
			RECT rc = m_rcItem;
