...
void CWndShadow::Create(HWND hParentWnd, HWND hShadowWnd)
{
	GetWindowRect(hShadowWnd, &m_MaskRect);
	CRect rt;
	GetWindowRect(hParentWnd, &rt);

	::MoveWindow(hParentWnd, m_MaskRect.left + (m_MaskRect.Width() - rt.Width())/2, m_MaskRect.top + (m_MaskRect.Height() - rt.Height())/2, rt.Width(), rt.Height(), TRUE);

	Create(hParentWnd);
	
	SetWindowPos(hParentWnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_SHOWWINDOW|SWP_NOMOVE|SWP_NOSIZE);  
	SetWindowPos(hParentWnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_SHOWWINDOW|SWP_NOMOVE|SWP_NOSIZE);  
}

void CWndShadow::Create(HWND hParentWnd, bool bFullShadow )
{
...
// 	int nShadWndHei = WndRect.bottom - WndRect.top + m_nSize * 2;
	...
	SIZE sizeWindow = { m_MaskRect.Width(), m_MaskRect.Height()};

//	HDC hDC = ::GetDC(m_hWnd);
//	HDC hdcMemory = CreateCompatibleDC(hDC);
	//HBITMAP hBitMap = CreateCompatibleBitmap(hDC, sizeWindow.cx, sizeWindow.cy);
	//HBITMAP hOriBmp = (HBITMAP)::SelectObject(hdcMemory, hBitMap);
	...
	PVOID pvBits = NULL;   
	HBITMAP hbitmap = CreateDIBSection(NULL, (PBITMAPINFO)&stBmpInfoHeader, DIB_RGB_COLORS, &pvBits, NULL, 0);
	ASSERT(hbitmap != NULL);
	memset( pvBits, 0x00, sizeWindow.cx * 4 * sizeWindow.cy);
	...
	if(m_bFullShadow)
	{
		SolidBrush brush(Color(90, 0, 0, 0));
		graph.FillRectangle(&brush, 0, 0, m_MaskRect.Width(),  m_MaskRect.Height());
	//	SolidBrush brush(Color(1, 0, 0, 0));

		//graph.FillRectangle(&brush, WndRect.left - m_MaskRect.left, WndRect.left - m_MaskRect.left + WndRect.right - WndRect.left, WndRect.right - WndRect.left, WndRect.bottom - WndRect.top);
	}
	else
	{
		SolidBrush brush(Color(100, 0, 0, 0));
		graph.FillRectangle(&brush, 0, 0, WndRect.left - m_MaskRect.left, sizeWindow.cy);
		graph.FillRectangle(&brush, WndRect.left - m_MaskRect.left, 0, WndRect.right - WndRect.left , WndRect.top - m_MaskRect.top);
		graph.FillRectangle(&brush, WndRect.left - m_MaskRect.left + WndRect.right - WndRect.left, 0, m_MaskRect.right - WndRect.right , sizeWindow.cy);
		graph.FillRectangle(&brush, WndRect.left - m_MaskRect.left, WndRect.top - m_MaskRect.top + WndRect.bottom - WndRect.top,
			WndRect.right - WndRect.left , m_MaskRect.bottom - WndRect.bottom);
		// 
	}


	POINT ptDst = {m_MaskRect.left, m_MaskRect.top};
	POINT ptSrc = {0, 0};
	SIZE WndSize = {sizeWindow.cx, sizeWindow.cy};
	BLENDFUNCTION blendPixelFunction= { AC_SRC_OVER, 0, 255, AC_SRC_ALPHA };

	MoveWindow(m_hWnd, ptDst.x, ptDst.y, m_MaskRect.Width(), m_MaskRect.Height(), FALSE);
...
void CWndShadow::MakeShadow(UINT32 *pShadBits, HWND hParent, RECT *rcParent)
{
	SIZE sizeWindow = { m_MaskRect.Width(), m_MaskRect.Height()};
	int nBytesPerLine = ((sizeWindow.cx * 32 + 31) & (~31)) >> 3;
	for(int i = rcParent->left - m_MaskRect.left  ; i < rcParent->right - rcParent->left ; i++ )
	{
		for(int j = rcParent->top - m_MaskRect.top  ; j < rcParent->bottom - rcParent->top ; j++ )
		{
			UINT32 *pPixel = pShadBits + i*j;
			*pPixel = *pPixel & 0x00FFFFFF;
		}
	}
	return;
...
void CWndShadow::SetMaskSize(CRect rect)
{
	m_MaskRect = rect;
}
