...
void CResourceDownloaderManager::NotifyDownloaderDestroy( CResourceDownloader* pDownloader , bool bCancel)
{
#ifdef DEVELOP_VERSION
	m_mapDownloader.erase(pDownloader);
	for (map<IDownloaderListener*, IDownloaderListener*>::iterator itor = m_mapListener.begin(); itor != m_mapListener.end(); ++itor)
	{
		itor->second->OnDownloaderDestroy(pDownloader, bCancel);
	}
#endif
}
