	...
	if(pNotify->dwErrorCode == 0)
	{
		...
		if( res )
		{
			...
			do 
			{
				...
				if( !root["uuid"].isNull() )
				{
					m_strUploadGuid = root["uuid"].asCString();
				}
				else
					break;
				...
				{
					m_strServerPath = root["dist_path"].asCString();
				}
				else
					break;
...
void CUploadItemUI::StartUpload()
{
	...
	SetState(eUploading);
	m_dwDownloadServerPathId = pHttpManager->AddTask(_T("esp-lifecycle.web.sdp.101.com"), szPost, _T(""), _T("GET"), _T(""), 80, 
