...
//	pVerScrollBar = m_pCloudSearch->GetVerticalScrollBar();
	...
	strVRVersionPath += "\\vrversion.dat";
		...
		if(pCategoryTree)
		{
			...
			if(dwUserId != 0)
			{
					...
					)
				{
					...
					pHttpManager->AddTask(_T("p.101.com"), _T("/101ppt/setChapter.php"), _T(""), _T("POST"), szPost, 80, 
	...
	if ( iter != m_mapLocalFile.end() )
	{
		...
		for ( iterFile; iterFile != iter->second.end(); iterFile++ )
		{
			...
			if ( _tcsicmp(sResInfo.strResPath.c_str(), strFilePath.c_str()) == 0 )
			{
				...
				{
					//文件重名
					string strFileDicPath = sResInfo.strResPath.substr(0,sResInfo.strResPath.find_last_of("\\")+1);
					string strNewPath = strFileDicPath + strNewName + sResInfo.strResPath.substr(sResInfo.strResPath.find_last_of("."));
					::rename(sResInfo.strResPath.c_str(),strNewPath.c_str());
