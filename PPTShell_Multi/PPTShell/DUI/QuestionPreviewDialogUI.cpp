...
void CQuestionPreviewDialogUI::InitWindow()
{
	...
	m_hWebWindow = CreateWindow("CoursePlayWebUIEx", _T(""), WS_POPUP | WS_VISIBLE | WS_CHILD,  0,0,0,0 , GetHWND() , NULL, AfxGetApp()->m_hInstance, NULL);
}
...
void CQuestionPreviewDialogUI::InitData( CStream* pStream)
{
	if(pStream)
	{
		...
		pStream->ResetCursor();
		string strUrl = pStream->ReadString();
		m_sCurrentUrl = AnsiToUnicode(strUrl);
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (pNewItem)
			{
				...
				string strName = pHandler->GetHandlerId();
				strName += "Preview";
...
void CQuestionPreviewDialogUI::OnBtnFullScreenClick( TNotifyUI& msg )
{
	if(m_pLayoutThumbnailList)
	{
		if(m_nCurrentIndex>=0&&m_nCurrentIndex<m_pLayoutThumbnailList->GetCount())
		{
			...
			{
				CStream stream(1024);
				stream.WriteString(UnicodeToAnsi(m_sCurrentUrl));
			...
			if(pItem)
			{
				...
				if(pControl)
				{
					...
					if(pHandler)
					{
						...
						string strTitle = pHandler->GetTitle();
						string strGuid = pHandler->GetGuid();
		...
		if (pParamer->GetHttpNotify()&&pParamer->GetHttpNotify()->dwErrorCode == 0)
		{
			if(pParamer->GetUserData()!=NULL)
			{
				...
				if(pStream)
				{
					...
					pStream->ResetCursor();
					string strTitle = pStream->ReadString();
					string strGuid = pStream->ReadString();
					string strQuestionName = pStream->ReadString();
							...
							if( res )
							{
								...
								if( pPagesElement != NULL )
								{
									...
									while( pPageElement != NULL )
									{
										...
										pPageElement->SetAttribute("href", szHref);
									...
									if( res )
									{
										...
										if( pAddonModuleElement != NULL )
										{
											...
											canInsert = NDCloudQuestionManager::GetInstance()->ConvertQuestionToCourseFile(strGuid, strQuestionType, strQuestionName, pParamer->GetHttpNotify()->strFilePath);
										}
									}
								}
							}
					...
					{
						doInsert = true;
						InsertQuestionByThread(strMainXmlPath.c_str(),strTitle.c_str(),strGuid.c_str(),0,0,&MakeEventDelegate(this, &CQuestionPreviewDialogUI::OnInsertCompleted));
					}
					delete pStream;
...
std::wstring CQuestionPreviewDialogUI::GetUrl()
{
...
void CQuestionPreviewDialogUI::ShowQuestion( wstring url )
{
	m_sCurrentUrl = url;
	...
	if(pObejct&&m_hWebWindow)
	{
		...
		pObejct->CoursePreview(m_hWebWindow,(WCHAR*)url.c_str());
		if ( m_hThread != NULL )
		{
			TerminateThread(m_hThread, 0);
			m_hThread = NULL;
...
{
	m_nInsertCount = 0;
	this->ShowQuestion(L"about:blank");
	this->HideQuestion();
		...
		if(pItem)
		{
			...
			if(index==m_nCurrentIndex)
			{
				if(pParamer->GetHttpNotify()&&pParamer->GetHttpNotify()->dwErrorCode == 0)
				{
					...
					if(res)
					{
						wstring strUrl = AnsiToUnicode(szParam);
						ShowQuestion(strUrl);
						//
						if(CQuestionPreviewFullScreenUI::GetInstance()->GetHWND()&&IsWindowVisible(CQuestionPreviewFullScreenUI::GetInstance()->GetHWND()))
						{
...
int CQuestionPreviewDialogUI::GetDirecotySize( CString strDirectoryPath )
{
	int fileSize = 0;
	CFileFind finder;
	BOOL bWorking = finder.FindFile(strDirectoryPath + "\\" + "*.*");
	while(bWorking)
	{
		...
		if(!finder.IsDots())
		{
			...
			CString strTempPath = strDirectoryPath + "\\" + strFileName;
...
bool CQuestionPreviewDialogUI::Initialize( HINSTANCE hInstance )
{
	...
	wcex.lpszClassName	= "CoursePlayWebUIEx";
	wcex.hIconSm		= NULL;
