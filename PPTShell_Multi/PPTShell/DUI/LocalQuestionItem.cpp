...
BOOL CLocalQuestionItemUI::GetQuestionInfo( tstring strJosnPath, tstring& strGuid, tstring& strQuestionType )
{
	...
	if ( jsonReader.parse(pBuff, rootItem) )
	{
		...
		strGuid			= questionGuid.asString();
		strQuestionType	= questionType.asString();
	}
...
void CLocalQuestionItemUI::UploadNetdisc(int currentModeType)
{
	...
	if (dwUserID != 0)
	{
		...
		if ((dwAttri != -1) && !(dwAttri&FILE_ATTRIBUTE_DIRECTORY))
		{
			...
			string strToast = _T("“") + strTitle + _T("”开始上传...");
...
bool CLocalQuestionItemUI::OnQuestionFolderUploaded( void* param )
{
	...
	if( pNotify->dwErrorCode == 0 && pStream != NULL )
	{
		...
		string strAuthorization = NDCloudUser::GetInstance()->GetAuthorizationHeader(Str2Ansi(EduPlatformHost), Str2Ansi(szUrl), _T("GET"));

		//
		DWORD dwTaskId = HttpDownloadManager::GetInstance()->AddTask(EduPlatformHost, szUrl, _T(""), _T("GET"), _T(""), 80,
	...
	else
	{
		...
		{
			string strToast=_T("“") + strTitle + _T("”上传失败");
			CToast::Toast(strToast);
		}
	}
...
bool CLocalQuestionItemUI::OnResIdQueried( void* param )
{
	...
	if( pNotify->dwErrorCode == 0 && pStream != NULL )
	{
		...
		if(res)
		{
			...
			if( nResModelType == UPLOAD_QUESTIONS )
			{
				string strFolder = strLocalPath.substr(strLocalPath.find_last_of("\\")+1);
				if(strFolder.compare(strGuid+"_default")==0)
				{
			...
			// pack this question
			NDCloudQuestionManager::GetInstance()->PackQuestion(strGuid, dwUserId, MakeHttpDelegate(NULL));
		}
	}
	if(pStream)
		delete pStream;
		...
		if(pNotify->nDataSize>0)
		{
			...
			if(res)
			{
				if( root["code"].isNull() )
				{
					...
					{
						string strToast=_T("“") + strTitle + _T("”上传成功");
						CToast::Toast(strToast);
					}
				}
			}
