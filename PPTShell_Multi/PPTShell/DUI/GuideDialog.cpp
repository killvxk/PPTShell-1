...
//				m_strSubEditionNodeCode = strSubEditionNodeCode;
		...
		else if( _tcsicmp(msg.pSender->GetClass(), _T("AutoHideOptionUI") ) == 0 )
		{
			...
			{
				insertTabLayout(pChapterNode,NDCloudGetChapterGUID());
...
void CGuideDialogUI::OnInitLocalTree()
{
	...
	GetPrivateProfileString(_T("Config"),_T("DefaultSetting"),_T(""),szDefaultSetting,sizeof(szDefaultSetting)-1, strPath.c_str());
		...
		while(pSubNode)
		{
			...
			if( strNodeCode == strNDCode && strNDCode.length() > 0)
			{
				if( strNDCode == _T("$ON040000") )
				{
					...
					GetPrivateProfileString(_T("Config"),_T("GradeText"),_T(""),szGradeText,sizeof(szGradeText)-1, strPath.c_str());
...
bool CGuideDialogUI::OnGetEdition(void * pParam)
{
	...
	BOOL bRet = NDCloudDecodeCategory(pNotify->pData, pNotify->nDataSize,m_CategoryTree->GetSelectedSectionCode(),m_pGradeOpt->GetNodeCode(),m_pCourseOpt->GetNodeCode(),"");
