			...
			case MSG_URLFAILED:
				{
					...
					if (pDlg->m_nErrorCount<3){
						...
						pObejct->CoursePlayDui(pDlg->m_hWebWindow,(WCHAR *) pDlg->m_nUrl.c_str());
					}else{
					}
...
void CCoursePlayUI::Init(WCHAR* nUrl,CoursePlay_Type nType,BOOL nShow )
{
	HWND hwnd = AfxGetApp()->m_pMainWnd->GetSafeHwnd();
	tstring nTitle = "";
	int nQuestionType = 0;
	...
	m_pSubTitleLabel->SetText("");
	m_pTitleLabel->SetText(nTitle.c_str());
	std::wstring ntemp(nUrl);
	m_nUrl = ntemp;
	m_nErrorCount = 0;
	pObejct->SetQuestionType(m_hWebWindow,nQuestionType);
	pObejct->CoursePlayDui(m_hWebWindow, nUrl);
