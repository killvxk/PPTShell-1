...
bool CCloudQuestionHandler::OnHandleInsert( void* pObj )
{
	...
	do 
	{
		...
		while( pPageElement != NULL )
		{
			...
			pPageElement->SetAttribute("id", GetGuid());
			pPageElement->SetAttribute("name", GetGuid());
	...
	if (pParamer->GetHttpNotify()&&pParamer->GetHttpNotify()->dwErrorCode == 0)
	{
		if (GetTrigger()&&GetTrigger() == pParamer->GetTrigger()&&GetTrigger()->IsVisible())
		{
			...
			if( res )
			{
				...
				//获取其他item信息
				CStream stream(1024);
				string strUrl = UnicodeToAnsi(AnsiToUnicode(szParam));
				...
				for (int i = 0; i < nCount; ++i)
				{
					...
					if (pHandlerVisitor)
					{
						...
						if (pHandler)
						{
							...
							if(!bFindIndex)
							{
								if(pHandler->GetGuid()!=this->GetGuid())
								{
									nTempIndex++;
								}
								else
				...
				stream.WriteDWORD(nTempIndex);
				stream.WriteDWORD(nTempCount);
				for (int i = 0; i < nCount; ++i)
				{
					IHandlerVisitor* pHandlerVisitor = dynamic_cast<IHandlerVisitor*>(pParaent->GetItemAt(i));
					if (pHandlerVisitor)
