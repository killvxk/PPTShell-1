...
bool CQuestionPreviewFullScreenUI::Initialize( HINSTANCE hInstance )
{
	...
	wcex.lpszClassName	= "CoursePlayWebFullScreen";
	wcex.hIconSm		= NULL;
	...
	if(m_hWebWindow==NULL)
	{
		if(m_pLayoutQuestion)
		{
			...
			if(m_hWebWindow==NULL)
			{
				...
				m_hWebWindow = CreateWindow("CoursePlayWebFullScreen", _T(""), WS_POPUP | WS_VISIBLE | WS_CHILD,  x,y,width,height , GetHWND() , NULL, AfxGetApp()->m_hInstance, NULL);
			}
...
void CQuestionPreviewFullScreenUI::ShowQuestion( wstring url )
{
	m_sCurrentUrl = url;
	CCefObject* pObejct = CefObject::GetInstance();
	if(pObejct&&m_hWebWindow)
	{
		...
		pObejct->CourseFullScreenPreview(m_hWebWindow,(WCHAR*)url.c_str());
	}
...
std::wstring CQuestionPreviewFullScreenUI::GetUrl()
{
...
void CQuestionPreviewFullScreenUI::InitData(CStream* pStream)
{
	...
	{
		pStream->ResetCursor();
		m_sCurrentUrl = AnsiToUnicode(pStream->ReadString());
		ShowQuestion(m_sCurrentUrl);
	}
}
