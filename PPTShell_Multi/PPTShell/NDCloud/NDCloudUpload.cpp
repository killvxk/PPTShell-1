...
void CNDCloudUploadFileTask::StartUpload()
{
	...
	pHttpManager->AddTask(EduPlatformHostLC, strUrl.c_str(), _T(""), _T("GET"), _T(""), 80, 
...
bool CNDCloudUploadFileTask::OnServerPathObtained( void * pParam )
{
	...
	if(sscanf_s(strUrl.c_str(), _T("http://%[^/]%s"), szHost, sizeof(szHost) - 1, szUrlPath , sizeof(szUrlPath) - 1) != 2)
	{
		m_bSuccess = FALSE;
		m_OnComplete(pParam);
		return false;
...
{ 
	...
	tstring strUrl = ComposeObtainServerPathUrl(m_dwUserId, m_strGuid);
	DWORD dwTaskId = HttpDownloadManager::GetInstance()->AddTask(EduPlatformHostLC, strUrl.c_str(), _T(""), _T("GET"), _T(""), 80, 
...
bool CNDCloudUploadFolderTask::OnServerPathObtained(void *pParam)
{
	...
	if( res )
	{
		...
		{
			m_strAccessUrl = root["access_url"].asCString();
			m_strSessionId = root["session_id"].asCString();
			m_strServerFilePath = root["dist_path"].asCString();
...
BOOL CNDCloudUploadFolderTask::UploadAllFiles()
{
	...
	TCHAR szUrlPath[1024];
	bool result = false;
	if(sscanf_s(m_strAccessUrl.c_str(), _T("http://%[^/]%s"), szHost, sizeof(szHost) - 1, szUrlPath , sizeof(szUrlPath) - 1) == 2)
	{
		...
		if( m_mapFilePaths.size() >0 )
		{
			...
			for(itr = m_mapFilePaths.begin(); itr != m_mapFilePaths.end(); itr++)
			{
				...
				TCHAR szName[1024];
				char * p = _tcsrchr((char *)strFilePath.c_str(), _T('\\'));
				if(p)
				{
					_tcscpy(szName, p + 1);
				}
