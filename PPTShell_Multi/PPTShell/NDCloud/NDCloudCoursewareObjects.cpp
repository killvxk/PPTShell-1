...
bool CNDCloudCoursewareObjects::PollPackState()
{
	...
	NDCloudDownloadEx(strUrl, _T(""), _T("GET"), _T(""),  MakeHttpDelegate(this, &CNDCloudCoursewareObjects::OnPackStateObtained));
...
bool CNDCloudCoursewareObjects::OnPackStateObtained(void* param)
{
	...
	tstring strState = root["archive_state"].asCString();
	if( strState == _T("ready") )
	{
		m_bPolled = TRUE;

		// can download now
		tstring strUrl = root["access_url"].asCString();
	...
	else if( strState == _T("unpack") || strState == _T("pending") )
	{
		...
		NDCloudDownloadEx(strUrl, _T(""), _T("POST"), _T(""),  MakeHttpDelegate(this, &CNDCloudCoursewareObjects::OnPackStarted));
		m_bPacking = TRUE;
	}
	else
	{
...
bool CNDCloudCoursewareObjects::OnCoursewareObjectsDownloaded(void* param)
{
	...
	if( pNotify->strFilePath != _T("") )
	{
		...
		CUnZipper UnZipper;
		UnZipper.UnZip(pNotify->strFilePath.c_str(), Str2Ansi(strFolder).c_str());
