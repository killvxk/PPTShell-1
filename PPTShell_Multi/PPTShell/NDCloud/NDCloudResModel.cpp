...
void CNDCloudResourceModel::SetResourceType(int nResModelType, tstring strResType)
{
	...
	itemMinor["taxoncode"]		= strResCode;
...
void CNDCloudResourceModel::SetPreview(tstring strFormat, tstring strUrl)
{
	...
	if( strFormat != _T("") && strUrl != _T("") )
		PreviewItem[strFormat] = Str2Utf8(strUrl);
...
//	item["creator"]		= _T("PPTShell");
	...
	item["creator"]		=  szUserId;
	item["enable"]		= _T("true");
	item["status"]		= _T("TRANSCODE_WAITING");
	item["provider"]	= _T("NetDragon Inc");
...
{
	m_Root["life_cycle"]["status"]		= strStatus;
}
...
DWORD CNDCloudResourceModel::CommitResourceModel(CHttpDelegateBase& delegate,void* pUserData)
{
	...
	//
	DWORD dwTaskId = HttpDownloadManager::GetInstance()->AddTask(EduPlatformHost, szUrl, strHeader.c_str(), strMethod.c_str(), strPost.c_str(), 80,
								delegate, 
