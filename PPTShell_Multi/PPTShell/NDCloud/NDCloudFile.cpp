...
#define DOWNLOAD_DENTRY_NOT_FOUND		_T("CS/DOWNLOAD_DENTRY_NOT_FOUND")
...
void CNDCloudFile::ReportResourceMissingError()
{
	...
	string strErrorMessage = "missing_resource=[File Missing]: ";
	strErrorMessage+=m_strUrl;
	
	pHttpManager->AddTask(_T("p.101.com"), _T("/101ppt/resourceMissing.php"), _T(""), _T("POST"), strErrorMessage.c_str(), 80, 
											 ...
											 CHttpDelegateBase &OnCompleteDelegate, CHttpDelegateBase &OnProgressCB,void* pUserData, tstring strMD5)
{
	...
	string strSize = szText;
	string strPath = NDCloudFileDirectory;
	strPath+="\\Image\\"+strFileName+"_"+strGuid+"_"+strSize+"."+strFileExt;
	...
	DWORD dwSize = 0;
	if( nThumbSize == 0 )
		dwSize = GetLastDownloadFileSize(strPath.c_str());

	return DownloadCloudFile(strFileUrl, strPath.c_str(), dwSize, OnCompleteDelegate, OnProgressCB, pUserData, strMD5);
}
...
DWORD CNDCloudFileManager::DownloadFileSearchPlatform( tstring strUrl, tstring strGuid, tstring strName, int nFileType, CHttpDelegateBase& OnCompleteDelegate, CHttpDelegateBase& OnProgressCB,void* pUserData/*=NULL*/, tstring strMD5 /*= _T("")*/ )
{
	...
	CHAR szUrlPath[1024];
	CHAR szExt[1024];
	if(sscanf_s(strUrl.c_str(), "http://%[^/]%s", szHost, sizeof(szHost) - 1, szUrlPath , sizeof(szUrlPath) - 1) != 2)
	{
		return 0;
	}
	...
	if(!pos)
		return 0;
	_tcscpy_s(szExt, sizeof(szExt) - 1 ,pos);
											 ...
											 tstring strSessionId /*= _T("")*/)
{
	...
	strFileUrl = UrlEncode(AnsiToUtf8(strFileUrl));
	...
	DWORD dwCrc = CalcCRC((char*)strFileUrl.c_str(), strFileUrl.length());
	...
	if( strSessionId != _T("") )
	{
		...
		strFileUrl += strSessionId;
		 ...
		 dwCrc = CalcCRC((char*)strFileUrl.c_str(), strFileUrl.length());
	}
...
tstring CNDCloudFileManager::ComposeUrl(tstring strUrl, int nThumbSize)
{
	...
	strUrl =  strUrl.substr(strlen(_T("${ref-path}")));
...
void CNDCloudFileManager::SplitDownloadParameters(tstring strUrl, tstring& strPath, tstring& strSize, tstring& strSession)
{
	...
	strSize = mapPairs["size"];
	strSession = mapPairs["session"];
...
tstring GetNDCloudPath()
{
	...
	{
		if ( GetDriveType(g_HardDiskLetters[i]) == DRIVE_FIXED )
		{
			if (GetDiskFreeSpaceEx(g_HardDiskLetters[i], &AvailableToCaller, &Disk, &Free))
			{
				if( Free.QuadPart > MaxFreeSize.QuadPart )
				{
					...
					{
						MaxFreeSize = Free;
						g_strNDCloudDrive = g_HardDiskLetters[i];
