...
BOOL CNDCloudUser::MobileLogin(CStream *loginStream, CHttpDelegateBase& delegate)
{
	if (IsLogin())
	{
		WRITE_LOG_LOCAL("CNDCloudUser::MobileLogin, user login already!");
		return TRUE;
	}

	if (loginStream == NULL)
	{
		WRITE_LOG_LOCAL("CNDCloudUser::MobileLogin, in param valid!");
		return FALSE;
	}
	...
	string strUserName = loginStream->ReadString();
//	string strPassword = loginStream->ReadString();
	DWORD dwUserID = loginStream->ReadDWORD();
	if (dwUserID <= 0)
	{
		WRITE_LOG_LOCAL("CNDCloudUser::MobileLogin, userid is error:%d", dwUserID);
		return FALSE;
	}
	string strMacKey = loginStream->ReadString();
	string strAccessToken = loginStream->ReadString();
	string strRealName = loginStream->ReadString();
	string strSex = loginStream->ReadString();
	string strSubject = loginStream->ReadString();
	string strOrgNodeName = loginStream->ReadString();
	string strOrgID = loginStream->ReadString();
	UINT64 ulOrgID = 0;
	try
	{
		sscanf_s(strOrgID.c_str(), "%I64u", &ulOrgID);
	}
	catch (CException* e)
	{
		ulOrgID = 0;
		WRITE_LOG_LOCAL("org id have invalid value:%s", strOrgID);
		return FALSE;
	}
	
	WRITE_LOG_LOCAL("strUserName:%s, userId:%d, mackey:%s, accesstoken:%s", strUserName.c_str(), dwUserID, strMacKey.c_str(), strAccessToken.c_str());
	WRITE_LOG_LOCAL("realname:%s, sex:%s, orgname;%s, orgid:%s", strRealName.c_str(), strSex.c_str(), strOrgNodeName.c_str(), strOrgID.c_str());
	...
	m_strUserName=Utf82Str(strUserName);
//	m_strPassword = "wff1234567";
	m_dwUserId = dwUserID;
	m_strRealName = Utf82Str(strRealName);
	m_strSex = Utf82Str(strSex);
	m_strSubjects = Utf82Str(strSubject);
	m_strOrgNodeName = Utf82Str(strOrgNodeName);
	m_ulOrgId = ulOrgID;
	m_strMacKey = Utf82Str(strMacKey);
	m_strAccessToken = Utf82Str(strAccessToken);
...
bool CNDCloudUser::OnUserLoginResponse(void *param)
{
	...
	if( root["user_id"].isNull() )
	{
		// error
		m_bSuccess			= FALSE;
		m_strErrorCode		= Ansi2Str(root["code"].asCString());
		m_strErrorMessage	= Ansi2Str(root["message"].asCString());
	...
	m_strMacKey				= Ansi2Str(root["mac_key"].asCString());
	m_strAccessToken		= Ansi2Str(root["access_token"].asCString());
	m_bSuccess				= TRUE;
	tstring	strServerTime	= root["server_time"].asCString();

	int nYear, nMonth, nDate, nHour, nMin, nSec, nMilli; 
	TCHAR szUTC[128];
	TCHAR szUTCNum[128];
	TCHAR szUTCSymbol[128];
	if(sscanf_s(strServerTime.c_str(), "%d-%d-%d%[^0-9]%d:%d:%d.%d%[+-]%s", &nYear, &nMonth, &nDate, szUTC, sizeof(szUTC) - 1, &nHour, &nMin, &nSec, &nMilli, szUTCSymbol, sizeof(szUTCSymbol) - 1, szUTCNum, sizeof(szUTCNum) - 1) == 10)
	{
		CTime time(nYear, nMonth, nDate, nHour, nMin, nSec);
		m_dwServerTime = time.GetTime();
		if( _tcsicmp(szUTCSymbol , _T("+")) == 0)
		{
			m_dwServerTime += (8 - atoi(szUTCNum)/100) * 60* 60;
		}
...
bool CNDCloudUser::OnUserInfoObtained(void* param)
{
	...
	m_strNickName = Ansi2Str(root["nick_name"].asCString());

	if( !root["org_exinfo"].isNull() )
	{
		if( !root["org_exinfo"]["real_name"].isNull() )
			m_strRealName = Ansi2Str(root["org_exinfo"]["real_name"].asCString());

		if( !root["org_exinfo"]["sex"].isNull() )
			m_strSex = Ansi2Str(root["org_exinfo"]["sex"].asCString());

		if( !root["org_exinfo"]["subjects"].isNull() )
			m_strSubjects = Ansi2Str(root["org_exinfo"]["subjects"].asCString());

		if( !root["org_exinfo"]["node_name"].isNull() )
			m_strOrgNodeName = Ansi2Str(root["org_exinfo"]["node_name"].asCString());
			...
			if (NodeItmes.size() > 0)
			{
				...
				for (int i=0; i< NodeItmes.size(); i++)
				{
					...
					{ 
						...
						CString strFullName = _T(SubItem["node_full_name"].asCString());  
						AfxExtractSubString(strGradeClassNO, strFullName, 4, '|'); 
						if (!strGradeClassNO.IsEmpty())
						{
							if (! bDefaultGrade)
							{
								m_strFullName = SubItem["node_full_name"].asCString();
...
bool CNDCloudUser::OnClassRoomObtained(void* param)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		tstring strClassName = Ansi2Str(item["class_name"].asCString());

		tstring strGradeName = Ansi2Str(item["grade_name"].asCString());
	...
	// authorization
	string strAuthorization = GetAuthorizationHeader(ClassroomHost, Str2Ansi(szUrl), "GET");

	string strHeader = _T("Content-Type: application/json");
	strHeader += _T("\r\n");
	strHeader += Ansi2Str(strAuthorization);
	...
	strAuthorization = GetAuthorizationHeader(ClassroomHost, Str2Ansi(szUrl), "GET");
...
bool CNDCloudUser::OnStudentInfoObtained(void *param)
{
	...
	m_pStudentInfoStream->WriteString(str);
...
bool CNDCloudUser::OnUserBearerLoginResponse( void* param )
{
	...
	if( !root["token"].isNull() )
	{
		if( root["token"]["user_id"].isNull() )
		{
			// error
			m_bSuccess			= FALSE;
			m_strErrorCode		= Ansi2Str(root["token"]["code"].asCString());
			m_strErrorMessage	= Ansi2Str(root["token"]["message"].asCString());
		...
		m_strMacKey				= Ansi2Str(root["token"]["mac_key"].asCString());
		m_strAccessToken		= Ansi2Str(root["token"]["access_token"].asCString());
		if( !root["info"].isNull() )
		{
			if( !root["info"]["nick_name"].isNull() )
				m_strUserName			= Ansi2Str(root["info"]["nick_name"].asCString());
		}

		tstring	strServerTime	= root["token"]["server_time"].asCString();
		int nYear, nMonth, nDate, nHour, nMin, nSec, nMilli; 
		TCHAR szUTC[128];
		TCHAR szUTCSymbol[128];
		TCHAR szUTCNum[128];
...
bool CNDCloudUser::OnGetBearerTokensResponse( void* param )
{
	...
	if( str != "" )
	{
		...
		{
			// register success
			tstring strBearerTokens = Ansi2Str(root["access_token"].asCString());
			...
			if(nType == BEARER_TOKEN)
			{
				...
				TCHAR szPost[1024];
				_stprintf(szPost, BearerLogin ,m_strBearerSessionId.c_str());

				tstring strHeader = "Authorization: Bearer \"";
...
BOOL CNDCloudUser::CheckBearerTokens( int nType )
{
	...
	GetPrivateProfileString(_T("config"),  _T("BearerTokens"), _T(""),  szBearerTokens, sizeof(szBearerTokens),  strPath.c_str());
	...
	else
	{
		...
		TCHAR szUrl[1024];
		_stprintf(szUrl, CheckBearerTokenUrl, szBearerTokens);
		CHttpDownloadManager* pHttpManager = HttpDownloadManager::GetInstance();
		if( pHttpManager == NULL )
			return false;
...
BOOL CNDCloudUser::Register(tstring strUserName, tstring strPassword, tstring strCheckCode, CHttpDelegateBase& delegate)
{
	...
	root["password"]	= Str2Utf8(strEncryptedPass);
	...
	if( str != "" )
	{
		...
		if( !root["cause"].isNull() )
		{
			if( !root["cause"]["code"].isNull() )
				m_strErrorCode = Ansi2Str(root["cause"]["code"].asCString());

			if( !root["cause"]["message"].isNull() )
				m_strErrorMessage = Ansi2Str(root["cause"]["message"].asCString());
		}
		else
		{
			m_strErrorCode = Ansi2Str(root["code"].asCString());
...
BOOL CNDCloudUser::ChangePassword( tstring strOldPassword, tstring strNewPassword, CHttpDelegateBase& delegate )
{
	...
	// json
	Json::Value root;
	root["old_password"]	= Str2Utf8(strEncryptedOldPass);
	root["new_password"]	= Str2Utf8(strEncryptedNewPass);

	Json::FastWriter writter;
	string str = writter.write(root);

	TCHAR szPutBuff[1024];
	_stprintf(szPutBuff, UserCenterChangePassUrl, GetUserId());
...
bool CNDCloudUser::OnGetBackPwdConfirmAccountResponse(void* param)
{
	...
	if( str != "" )
	{
		...
		if( !root["code"].isNull() && !root["message"].isNull() )
		{
			p.strCode = Ansi2Str(root["code"].asCString()).c_str();
			p.strMessage = Ansi2Str(root["message"].asCString());
		}
		else
		{
			if( !root["login_name"].isNull() )
			{
				p.strAccount = Ansi2Str(root["login_name"].asCString());
				if( !root["email"].isNull() )
				{
					p.strEmail = Ansi2Str(root["email"].asCString());
				}
				if( !root["mobile"].isNull() )
				{
					p.strPhone = Ansi2Str(root["mobile"].asCString());
				}
			}
		}
...
bool CNDCloudUser::OnResetPasswordResponse( void* param )
{
	...
	if( str != "" )
	{
		...
		{
			if( !root["cause"]["code"].isNull() )
				p.strCode = Ansi2Str(root["cause"]["code"].asCString());

			if( !root["cause"]["message"].isNull() )
				p.strMessage = Ansi2Str(root["cause"]["message"].asCString());
		}
		...
		else
		{
			if( !root["temppwd"].isNull() )
				p.strTempPwd = Ansi2Str(root["temppwd"].asCString());
		}
...
//	string strEncryptedTempPass = MD5EncryptString(Str2Ansi(strTempPassword));
	...
	root["new_password"]	= Str2Utf8(strEncryptedPass);
...
BOOL CNDCloudUser::GetUserAvatar( DWORD dwSize, CHttpDelegateBase& delegate )
{
	...
	TCHAR szPost[1024];
	_stprintf(szPost, UserGetAvatar ,NDCloudUser::GetInstance()->GetUserId(), NDCloudUser::GetInstance()->GetUserId(), dwSize);
...
BOOL CNDCloudUser::SetUserAvatarWithSession()
{
	...
	TCHAR szUrl[1024];
	_stprintf(szUrl, UserSetAvatar, m_strAvatarSession.c_str());

	TCHAR szPost[1024];
	_stprintf(szPost, _T("%s/%d.jpg"), m_strAvatarServerPath.c_str(), GetUserId() );
...
bool CNDCloudUser::OnGetUserAvatarSessionResponse( void* param )
{
	...
	if( str != "" )
	{
		...
		{
			m_strAvatarSession = root["session"].asCString();
			m_strAvatarServerPath = root["path"].asCString();
...
BOOL CNDCloudUser::ObtainCheckCodeImage(CHttpDelegateBase& delegate)
{
	...
	// generate guid
	m_CheckCodeSession = NewGuid(false);
	...
	DWORD dwTaskId = HttpDownloadManager::GetInstance()->AddTask(UserEspHost, UserEspCheckCodeUrl, _T(""), _T("GET"),  _T(""), 80,
...
BOOL CNDCloudUser::QueryLoginNameExist(tstring strUserName, CHttpDelegateBase &delegate)
{
	...
	InternetSetCookie(strFullUrl.c_str(), _T("JSESSIONID"), strGuid.c_str());

	tstring strUrl = UserEspQueryUserExist;
	strUrl += strUserName;

	DWORD dwTaskId = HttpDownloadManager::GetInstance()->AddTask(UserEspHost, strUrl.c_str(), _T(""), _T("GET"),  _T(""), 80,
...
BOOL CNDCloudUser::SendActiveEmail(tstring strEmail, CHttpDelegateBase& delegate)
{
	...
	DWORD dwTaskId = HttpDownloadManager::GetInstance()->AddTask(UserEspHost, UserEspSendActiveEmail, 
																_T("Content-Type: application/json"), _T("POST"),  Ansi2Str(str).c_str(), 80,
...
string CNDCloudUser::GetAuthorizationHeader(string strHost, string strUrl, string strMethod)
{
	...
	hmac_sha256_get(digest, (const uint8_t *)strMac.c_str(), strMac.length(), (const uint8_t *)m_strMacKey.c_str(), m_strMacKey.length());
...
tstring CNDCloudUser::GetTeacherGrade(tstring sFullName)
{
	...
	CString strFullName = _T(sFullName.c_str()); 
...
BOOL CNDCloudUser::LoadHistoryData()
{
	...
	FILE* fp = NULL;
	errno_t err = _tfopen_s(&fp, strPath.c_str(),"rb");
	if( err != 0 )
		return FALSE;
...
{ 
	...
	{ 
		...
		strOrgID = root["gradeinfo"]["orgid"].asCString();
		strClassID = root["gradeinfo"]["classid"].asCString();
...
{  
	...
	string str = strValue.c_str();

	bool res = reader.parse(str, root); 

	int nSize = root[_T(strItemName.c_str())].size();  
	if (nSize > 0)
	{
		return true;
	}
