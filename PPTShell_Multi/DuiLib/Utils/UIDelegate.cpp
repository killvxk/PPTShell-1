...
CEventSource& CEventSource::operator=( const CEventSource& d )
{
	for( int i = 0; i < d.m_aDelegates.GetSize(); i++ ) {
		CDelegateBase* pObject = static_cast<CDelegateBase*>(d.m_aDelegates[i]);
