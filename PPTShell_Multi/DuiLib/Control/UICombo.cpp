...
CComboUI::CComboUI() : m_pWindow(NULL), m_iCurSel(-1), m_uButtonState(0),m_iFont(-1)
{
...
void CComboUI::PaintText(HDC hDC)
{
    ...
    if( m_iCurSel >= 0 ) {
		if(!GetText().IsEmpty())
		{
			CRenderEngine::DrawText(hDC, m_pManager, rcText, GetText(), GetTextColor(),
				GetFont(), DT_SINGLELINE | uTextStyle);
		}
        /*CControlUI* pControl = static_cast<CControlUI*>(m_items[m_iCurSel]);
	...
	else if(!GetTipText().IsEmpty())
	{
		CRenderEngine::DrawText(hDC, m_pManager, rcText, GetTipText(), GetTipTextColor(),
			GetFont(), DT_SINGLELINE | uTextStyle);
	}
}

void CComboUI::SetFont( int iFont )
{
	m_iFont = iFont;
}

int CComboUI::GetFont()
{
	return m_iFont;
}

DWORD CComboUI::GetTextColor()
{
	if(m_dwTextColor==0)
	{
...
DWORD CComboUI::GetTipTextColor()
{
	if(m_dwTipTextColor==0)
	{
		m_dwTipTextColor = m_pManager->GetDefaultFontColor();
	}
	return m_dwTipTextColor;
}

void CComboUI::SetTipTextColor( DWORD dwTipTextColor )
{
	m_dwTipTextColor = dwTipTextColor;
}

DuiLib::CDuiString CComboUI::GetTipText() const
{
	return m_sTipText;
}

void CComboUI::SetTipText( LPCTSTR pstrText )
{
	if( m_sTipText == pstrText ) return;
