...
//APP名称
#define DS_APP_NAME "101教育PPT开发版"
//安装目录名
#define DS_DIR_NAME "101教育PPT开发版"
//组名
#define DS_GROUP_NAME "101教育PPT开发版"
//开发版名字
#define DS_APP_DEV_NAME "101教育PPT开发版"
//版本信息
#define DS_APP_VER_NAME "1.0.6.3"
//主软件名
...
OutputBaseFilename={#DS_APP_DEV_NAME}{#DS_APP_VER_NAME}
Outputdir=.
AllowNoIcons=yes
...
AppId={{4FA717EC-FE0C-4EAA-B76C-C7383CB97DBA}
...
Name: "{group}\{#DS_UNINSTALL_DEV_NAME}"; Filename: "{app}\unins000.exe"; Parameters: /SILENT;
//Name: "{group}\{#DS_UNINSTALL_DEV_NAME}"; Filename: "{app}\unins000.exe";
...
Source: "{#DS_PACKAGE_DIR_NAME}\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs uninsneveruninstall
//以上是包含文件夹的目录
...
Const
  GUID = '{4FA717EC-FE0C-4EAA-B76C-C7383CB97DBA}';
  //鼠标指向
  MyCursor1 = 101;
  MyCursor2 = 102;
  Radius  = 5;
  ...
  function InternetCheckConnectionW(var szUrl: String;  dwReserved: Integer;  dwtmp: Integer): Boolean;
  external 'InternetCheckConnectionW@winInet.dll stdcall';
  
  function IsNetworkAlive(dwReserved: Integer): Boolean;
  external 'IsNetworkAlive@Sensapi.dll stdcall';
  
  
  function InternetQueryOptionA( szUrl: TComponent;  dwReserved: Integer; szTemp: Integer; dwtmp: Integer): Boolean;
  external 'InternetQueryOptionA@winInet.dll stdcall';
...
begin
    if RegQueryStringValue(HKLM, '{#DS_UNINSTALL_REGEDIT_PATH}'+'101教育PPT开发版', 'DisplayIcon', szInstallPath) then
      begin
...
// while ( count<5 ) do
//  begin
//   Result:= InternetCheckConnectionW(szUrl, 1, 0);
//   sleep(1000);
//   count := count+1;
//  end
  
//Result:= IsNetworkAlive(dwConnectionTypes);
end;
  ...
  begin
    ...
    begin
       ...
       WinHttpReq.SetRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        WinHttpReq.Send('<"eventId":1005,"eventTime":0,"params":[],"userId":0,"versionCode":"{#DS_APP_VER_NAME}","ver":1>');
...
//      DelTree(ExpandConstant(UninstallStringDirPath), True, True, True);
      ...
      if RegQueryStringValue(HKLM, '{#DS_UNINSTALL_REGEDIT_PATH}'+'101教育PPT开发版', 'UninstallString', ResultStr) then
      begin
...
begin
  ...
  begin
    ...
    //判断是否已经打开安装程序
//    IsAppRunning:= IsMyAppRunning('{#DS_APP_DEV_NAME}{#DS_APP_VER_NAME}.exe');
//    while IsAppRunning do
//    begin
...
//    else
//    begin
//      Exec(ExpandConstant('{uninstallexe}'), ' /Silent', '', SW_SHOW, ewNoWait, tempUninstallResult)
//      Result := False;
//      //exit;
//    end
...
//      MyAppName := '{code:GetDefaultInstallRoot}';
//      // 以下重命名卸载文件
