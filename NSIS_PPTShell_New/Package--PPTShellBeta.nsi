...
!define PRODUCT_NAME "101教育PPT内测版"
!define PRODUCT_DIR "101教育PPT内测版"
!define PROGRAM_NAME "101PPT"
!define MUST_PACKAGE_FOLDER_NAME "ImportantFile_Beta"
...
Section -AdditionalIcons
  ...
  ;桌面快捷方式
  ${If} $Bool_Desktop == 1
    CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\${PROGRAM_NAME}.exe" "${PRODUCT_NAME}" "$INSTDIR\App.ico"
...
Function ReportInstall
  StrCpy $R8 "data={$\"eventId$\":1005,$\"eventTime$\":0,$\"params$\":[],$\"userId$\":0,$\"versionCode$\":$\"${PRODUCT_VERSION}$\",$\"ver$\":1}"
...
Function un.InstFiles.Show
    ...
    ;${NSD_CreateLabel} 270 70 300 60 "${PRODUCT_NAME}"
    GetDlgItem $R0 $R2 1997  ;设置应用名位置
    System::Call "user32::MoveWindow(i R0, i 80, i 95, i 460, i 60) i R2"
    StrCpy $Lbl_AppName $R0
    SetCtlColors $Lbl_AppName "FFFFFF" transparent ;背景设成透明
