...
bool CZipper::AddFolderToZipFile(const char*foldername, const char* rootfolder)
{
	...
	char aboslutepath[LONG_PATH + 1] = {0};
	char savefoldernameinzipfile[LONG_PATH + 1] = {0};
	...
	if (0 != _strnicmp(aboslutepath, rootfolder, lstrlen(rootfolder)))
	{
		return false;
	}
	else{
		// else
		lstrcpy(savefoldernameinzipfile, aboslutepath + lstrlen(rootfolder));
	}
	...
	char DirSpec[LONG_PATH] = {0};  // directory specification
	DWORD dwError = 0;
	...
	} while (::FindNextFile(hFind, &FindFileData) != 0 && flag);
...
//	const char* password=NULL;
    ...
    zi.dosDate = 0;
    zi.internal_fa = 0;
    zi.external_fa = GetFileAttributes(filename);
...
//		err = getFileCrc(filenameinzip,buf,size_buf,&crcFile);
	...
	char savenamebuffer[LONG_PATH] = {0};
	...
	TCHAR szDestExt[8]				= {0};
	if (_tsplitpath_s(filename, szDestDrive, szDestDir, szDestFileName, szDestExt))
	{
		return false;
	}
	...
	savefilenameinzip += szDestFileName;
	savefilenameinzip += szDestExt;
...
bool CZipper::ZipFolder(const char* szFilePath, bool ignoreself)
{
	...
	// make zip path
	char szDrive[_MAX_DRIVE] = {0}, szFolder[LONG_PATH] = {0}, szName[_MAX_FNAME * 2] = {0}, szExt[_MAX_FNAME] = {0};
	_tsplitpath_s(szFilePath, szDrive, szFolder, szName, szExt);
	_tcscat(szName, szExt);

	char szZipPath[LONG_PATH] = {0};
	_makepath(szZipPath, szDrive, szFolder, szName, "zip");

	char szRootFolder[LONG_PATH] = {0};
	if ( ignoreself )
	{
...
bool CUnZipper::UnCurrentZipFile(const char* dstfolder, bool ingorepath , const char* password )
{
	...
	if ( UNZ_OK == err )
	{
		...
		char parentpath[LONG_PATH * 2+1] = {0};
