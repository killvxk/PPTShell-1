...
void CUpdateLogUI::InitWindow()
{
	...
	m_pLayoutLogList = static_cast<CVerticalLayoutUI*>(m_PaintManager.FindControl(_T("layoutLogList")));

	//读取点赞时间缓存
	ReadPraiseDateFile();
	if (PathFileExists(m_strLogFilePath.c_str())&&PathFileExists(m_strPraiseFilePath.c_str()))
	{
		ReadVersionLogFile();
		ReadPraiseFile();
		ShowContent();
...
{      
	tstring strVersion1 = Ansi2Str(item1["versionCode"].asString());
	tstring strVersion2 = Ansi2Str(item2["versionCode"].asString());
			...
			for (int i=0;i<m_vPraises.size();i++)
			{
				...
				if(pItem)
				{
					...
					if(btnPraise)
					{
						...
						{
							btnPraise->SetBkImage(_T("UpdateLog\\praise.png"));
						}
						else
						{
							btnPraise->SetBkImage(_T("UpdateLog\\unpraise.png"));
						}
					}
					...
					if(lblNum)
					{
						...
						lblNum->SetText(Ansi2Str(cNum).c_str());
						if(info.GetIsParised())
						{
...
void CUpdateLogUI::ShowUpdateLog()
{
	if(m_pLayoutLogList)
	{
		...
		for(int i = 0; i < vecItems.size(); i++)
		{
			...
			string strDate = item["date"].asString();
			tstring strVersion = Ansi2Str(item["versionCode"].asString());
			string strContent = item["content"].asString();

			string strYear = strDate.substr(0, 4);	 

			int st=strDate.find_first_of(_T('-'));
			string strMonthDay="";
			if (st>0)
			...
			CreateItemSytle(m_pLayoutLogList, strMonthDay.c_str(), Str2Ansi(strVersion).c_str(), strContent.c_str(),isCurrent);
		}
...
void CUpdateLogUI::CreatYearDot(CContainerUI* pParent, const char* lpszYear,bool isCurrent)
{
	if(pParent)
	{
		...
		if(layoutYear)
		{
			CLabelUI* lblYear = dynamic_cast<CLabelUI*>(layoutYear->FindSubControl(_T("lblYear")));
			if(lblYear)
			{
				lblYear->SetText(Ansi2Str(lpszYear).c_str());
			}
			if(isCurrent)
			{
				CControlUI* ctrLineTop = dynamic_cast<CControlUI*>(layoutYear->FindSubControl(_T("ctrLineTop")));
				if(ctrLineTop)
				{
...
void CUpdateLogUI::CreateItemSytle(CContainerUI* pParent, const char* lpszMothDate, const char* lpszTitle, const char* lpszLogContent,bool isCurrent)
{
	if(pParent)
	{
		...
		if(pItem)
		{
			...
			CLabelUI* lblDate = dynamic_cast<CLabelUI*>(pItem->FindSubControl(_T("lblDate")));//日期
			if(lblDate)
			{
				lblDate->SetText(Ansi2Str(lpszMothDate).c_str());
				if(isCurrent)
				{
			...
			CLabelUI* lblVersonTitle = dynamic_cast<CLabelUI*>(pItem->FindSubControl(_T("lblVersonTitle")));//版本号
			if(lblVersonTitle)
			{
				tstring strTmp = Ansi2Str(lpszTitle);
				tstring strVerTitle = _T("版本：");
				strVerTitle+=strTmp;
			...
			CVerticalLayoutUI* layoutContent = dynamic_cast<CVerticalLayoutUI*>(pItem->FindSubControl(_T("layoutContent")));//更新内容
			if(layoutContent)
			{
				RECT rcCalc = {0};
				...
				pLabel->SetText(Ansi2Str(lpszLogContent).c_str());
				...
				//计算文本高度
				this->CalcText(m_PaintManager.GetPaintDC(), rcCalc, Ansi2Str(lpszLogContent).c_str(), pLabel->GetFont(), DT_CALCRECT | DT_WORDBREAK | DT_EDITCONTROL | DT_LEFT|DT_NOPREFIX, UIFONT_GDI);
			...
			CContainerUI* containerContent = dynamic_cast<CContainerUI*>(pItem->FindSubControl(_T("containerContent")));
			if(containerContent)
			{
				...
				if(isCurrent)
				{
					containerContent->SetBkImage(_T("file='UpdateLog\\log_item_now_bk.png' corner='11,32,10,3'"));
				}
				else
				{
					containerContent->SetBkImage(_T("file='UpdateLog\\log_item_pass_bk.png' corner='11,32,10,3'"));
				}
			}
			heigth+=8;//占位符高度
...
#endif
		m_dwLogTaskId = pHttpDownloadManager->AddTask(_T("p.101.com"),strLogUrl.c_str(),_T(""),_T("GET"),"",80, 
			MakeHttpDelegate(this, &CUpdateLogUI::OnUpdateLogCompeleted),
			MakeHttpDelegate(NULL),
			MakeHttpDelegate(NULL));
		//开启获取点赞列表任务
		tstring strPraiseUrl=_T("/101ppt/getLikes.php");
		m_dwPraiseTaskId = pHttpDownloadManager->AddTask(_T("p.101.com"),strPraiseUrl.c_str(),_T(""),_T("GET"),"",80, 
...
bool CUpdateLogUI::OnPraiseCompeleted( void* param )
{
	...
	if(pNotify->dwErrorCode==0)
	{
		...
		pNotify->pData[pNotify->nDataSize] = _T('\0');
...
bool CUpdateLogUI::ReadVersionLogFile()
{
	// read json from file
	FILE* fp = NULL;
	int err = _tfopen_s(&fp, m_strLogFilePath.c_str(), _T("rb"));
	if( err != 0 )
		return false;
	...
	if( !m_LogRootItem["versionCode"].isNull() )
		m_strVersion = Ansi2Str(m_LogRootItem["versionCode"].asString());
...
bool CUpdateLogUI::WriteVersionLogFile(char* strJson)
{
	...
	string str = m_LogRootItem.toStyledString();

	FILE* fp = NULL;
	int err = _tfopen_s(&fp, m_strLogFilePath.c_str(), _T("wb"));
	if( err != 0 )
		return false;
...
void CUpdateLogUI::ReadPraiseFile()
{
	...
	try
	{
		int err = _tfopen_s(&fp, m_strPraiseFilePath.c_str(), _T("rb"));
		if( err == 0 )
		{
			fseek(fp, 0, SEEK_END);
			int nSize = ftell(fp);
						...
						if(jRootItem.isArray()&&jRootItem.size()>0)
						{
							...
							for (int i=0;i<jRootItem.size();i++)
							{
								...
								if(jText.isString())
								{
									...
									char cNum[20];
									itoa(i+1,cNum,10);
									tstring text = Ansi2Str(cNum);
									text+=_T(".")+Ansi2Str(jText.asString());
									info.SetText(text);
								}
								//根据点赞列表设置是否已经点赞过
...
void CUpdateLogUI::WritePraiseFile(char* strJson)
{
	try
	{
		...
		if(res&&items.size()>0)
		{
			...
			FILE* fp = NULL;
			int err = _tfopen_s(&fp, m_strPraiseFilePath.c_str(), _T("wb"));
			if( err == 0 )
			{
		...
		if(msg->pSender!=NULL && msg->pSender->GetTag()!=NULL)
		{
			...
			if(info&&!info->GetIsParised())
			{
				try
				{
					...
					info->SetIsParised(true);
					//设置控件显示
					msg->pSender->SetBkImage(_T("UpdateLog\\praise.png"));
					CHorizontalLayoutUI* ctrParent = static_cast<CHorizontalLayoutUI*>(msg->pSender->GetParent());
					if(ctrParent)
					{
						CLabelUI* lblNum=static_cast<CLabelUI*>(ctrParent->FindSubControl(_T("lblNum")));
						if(lblNum)
						{
							char cNum[20];
							itoa(num,cNum,10);
					...
					if( pHttpDownloadManager != NULL )
					{
						...
						tstring strPostUrl=_T("/101ppt/addLikeNum.php");
						TCHAR  szPost[1024] = {0};
						_stprintf(szPost, _T("id=%d&mac=%s"),info->GetId(),Ansi2Str(strMac).c_str());
						m_dwLogTaskId = pHttpDownloadManager->AddTask(_T("p.101.com"),strPostUrl.c_str(),_T(""),_T("POST"),Str2Ansi(szPost).c_str(),80, 
							MakeHttpDelegate(NULL),
							MakeHttpDelegate(NULL),
							MakeHttpDelegate(NULL));
					}
...
{
	if (PathFileExists(m_strPraiseDateFilePath.c_str()))
	{
		FILE* fp = NULL;
		try
		{
			int err = _tfopen_s(&fp, m_strPraiseDateFilePath.c_str(), _T("rb"));
			if( err == 0 )
			{
				fseek(fp, 0, SEEK_END);
				int nSize = ftell(fp);
...
void CUpdateLogUI::WritePraiseDateFile( int n_id,int n_date )
{
	if (PathFileExists(m_strPraiseDateFilePath.c_str()))
	{
		...
		if(jRootItem!=NULL)
		{
			...
			FILE* fp = NULL;
			int err = _tfopen_s(&fp, m_strPraiseDateFilePath.c_str(), _T("wb"));
			if( err == 0 )
			{
				fwrite(strResult.c_str(), strResult.length(), 1, fp);
			}
...
int CompareVersion(tstring strVersion1, tstring strVersion2)
{
	...
	{
		string str1 = Str2Ansi(vecStrings1[i]);
		string str2 = Str2Ansi(vecStrings2[i]);
