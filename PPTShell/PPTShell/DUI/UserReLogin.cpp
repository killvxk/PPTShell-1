...
{	
	...
	for (int i = 0; i < lGradeList.size(); i++)
	{
		...
		{
			sGradeName = Ansi2Str(root["gradeinfo"]["gradename"].asCString());
			sGradeName+= Ansi2Str(root["gradeinfo"]["classname"].asCString()); 
...
{ 
	...
	{ 
		...
		bool res = reader.parse(str, root); 
		sClassID = Ansi2Str(root["gradeinfo"]["classid"].asCString()); 
		sGradeName = Ansi2Str(root["gradeinfo"]["gradename"].asCString());
		sGradeName+= Ansi2Str(root["gradeinfo"]["classname"].asCString()); 
