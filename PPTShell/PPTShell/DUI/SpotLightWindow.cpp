...
bool CSpotLightWindowUI::ZoomBtnEvent( void *pParam )
{
	...
	{
		POINT pointCur= GetManager()->GetMousePos();
...
bool CSpotLightWindowUI::BackgroundEvent( void *pParam )
{
	...
	pointCur = GetManager()->GetMousePos();
...
// 			GetCursorPos(&pointCur);
	...
	if (pNotify->Type == UIEVENT_BUTTONDOWN )
	{
		m_pointStart = GetManager()->GetMousePos();
		m_bDialogDown = true;
	}
...
// 修改聚光灯的开灯/关灯
void DuiLib::CSpotLightWindowUI::SetLightStatus(DWORD dwLightStatus)
{
	CButtonUI* pLightStatusBtn = dynamic_cast<CButtonUI *>(m_pSpotLightToolsV->FindSubControl(_T("LightStatusBtn")));
	...
	case SPOTLIGHT_STATUS_CLOSE:
		{
			...
			m_dwLightStatus = SPOTLIGHT_STATUS_ON;
			m_strBtnImagePath = _T("file='SpotLightWindow\\icon_closelight.png' dest='19,10,45,36'");
			pLightStatusBtn->SetNormalImage(_T("file='SpotLightWindow\\icon_closelight.png' dest='19,10,45,36'"));
			pLightStatusBtn->SetHotImage(_T("file='SpotLightWindow\\icon_closelight_on.png' dest='19,10,45,36'"));
	...
	case SPOTLIGHT_STATUS_ON:
		{
			...
			m_dwLightStatus = SPOTLIGHT_STATUS_CLOSE;
			m_strBtnImagePath = _T("file='SpotLightWindow\\icon_openlight.png' dest='19,10,45,36'");
			pLightStatusBtn->SetNormalImage(_T("file='SpotLightWindow\\icon_openlight.png' dest='19,10,45,36'"));
			pLightStatusBtn->SetHotImage(_T("file='SpotLightWindow\\icon_openlight_on.png' dest='19,10,45,36'"));
...
// 设置聚光灯的中心点坐标
void DuiLib::CSpotLightWindowUI::SetSpotLightPos(DWORD dwXPos, DWORD dwYPos)
{
	int iXOffset,iYOffset;
	iXOffset = dwXPos - m_pointCirclePointPos.x;
	iYOffset = dwYPos - m_pointCirclePointPos.y;

	MoveSpotLight(iXOffset, iYOffset);
}
// 设置聚光灯的形状。
// nType是SPOTLIGHT_SHAPE_CIRCLE或SPOTLIGHT_SHAPE_RECTANGLE
void DuiLib::CSpotLightWindowUI::SetShapeType(int nType)
{
	m_dwShape = nType;
	m_pSpotLightUI->SetShape(nType);

	CButtonUI* pLightShapeBtn = dynamic_cast<CButtonUI *>(m_pSpotLightToolsV->FindSubControl(_T("LightShapeBtn")));
	// 设置其他状态
	switch(nType)
	{
	case SPOTLIGHT_SHAPE_CIRCLE:
		{
			// 切换为圆形
			m_strBtnImagePath = _T("file='SpotLightWindow\\icon_rectangle.png' dest='19,10,45,36'");
			pLightShapeBtn->SetNormalImage(_T("file='SpotLightWindow\\icon_rectangle.png' dest='19,10,45,36'"));
			pLightShapeBtn->SetHotImage(_T("file='SpotLightWindow\\icon_rectangle_on.png' dest='19,10,45,36'"));
	...
	case SPOTLIGHT_SHAPE_RECTANGLE:
		{
			// 切换为矩形
			m_strBtnImagePath = _T("file='SpotLightWindow\\icon_circle.png' dest='19,10,45,36'");
			pLightShapeBtn->SetNormalImage(_T("file='SpotLightWindow\\icon_circle.png' dest='19,10,45,36'"));
			pLightShapeBtn->SetHotImage(_T("file='SpotLightWindow\\icon_circle_on.png' dest='19,10,45,36'"));
