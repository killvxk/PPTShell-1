...
bool CProgressLoadingUI::OnUserLogined(void *param)
{
	...
	if( nStep == UCSTEP_LOGIN )
	{
		...
		string strAuthorization = NDCloudUser::GetInstance()->GetAuthorizationHeader(Str2Utf8(EduPlatformHostLC), Str2Utf8(szUrl), "GET");
	...
	else if( nStep == UCSTEP_USERINFO )
	{
		NDCloudUser::GetInstance()->SetLoginComplete(true);
...
{
	int pos = strCommandLine.find( _T("pptshell:") );
	if( pos == -1 )
		return FALSE;

	strCommandLine = strCommandLine.substr(pos+_tcslen(_T("pptshell:")));

	pos = strCommandLine.rfind(_T("\""));
	if( pos != -1 )
		strCommandLine = strCommandLine.substr(0, pos);
	...
	{
		WRITE_LOG_LOCAL(_T("SplitString error"));
		return FALSE;
	}
	...
	if( vecStrings.size() < 2 )
	{
		WRITE_LOG_LOCAL(_T("mode parse error"));
		return FALSE;
	}
	...
	if( pos == -1 )
	{
		WRITE_LOG_LOCAL(_T("Auth parse error"));
		return FALSE;
	}
	...
	if( len == 0 )
	{
		WRITE_LOG_LOCAL(_T("base64_decode error"));
		return FALSE;
	}
	...
	if( pos1 == -1 )
	{
		WRITE_LOG_LOCAL(_T("Access token parse error!"));
		return FALSE;
	}
	...
	if( pos1 == -1 )
	{
		WRITE_LOG_LOCAL(_T("nonce parse error!"));
		return FALSE;
	}
	...
	if( pos1 == -1 )
	{
		WRITE_LOG_LOCAL(_T("mac parse error!"));
		return FALSE;
	}
