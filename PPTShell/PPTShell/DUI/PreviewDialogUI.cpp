...
void CPreviewDlgUI::InitFlashWnd(LPCTSTR lpszFileName)
{
	if (!CheckPtr()) return;
...
void CPreviewDlgUI::InitCourseWndLocal(CStream* pStream)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (pItem)
		{
			...
			pItem->SetResourceUrl(strTitle.c_str(), strUrl.c_str(), m_strPPTGuid.c_str());
			pItem->SetFixedHeight(72);
			pItem->SetFixedWidth(128);
			pItem->SetGroup(_T("local"));
			pItem->SetLocal(true);
...
tstring CPreviewDlgUI::GeneratePngPic(LPCTSTR lpszSourceUrl)
{
	...
	{
		CompressImagePixel(Str2Unicode(strImagePath).c_str(), wszTempFileName, m_nHeight-50, m_nWidth); 
	}
...
bool CPreviewDlgUI::OnPictureDownloaded(void* param)
{
	...
	FILE* fp = NULL;
	int err = _tfopen_s(&fp, pNotify->strFilePath.c_str(), _T("rb"));
	if( err == 0 )
	{
		fseek(fp, 0, SEEK_END);
		int nSize = ftell(fp);
