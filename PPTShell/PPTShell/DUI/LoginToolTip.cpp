...
{ 
	...
	TCHAR szText[260]	= {0};
	_stprintf(szText, szForamt, sTeacherName.c_str(), _T("老师,欢迎您！"));
	m_pTeacherName->SetShowHtml();
	m_pTeacherName->SetText(szText);

	tstring strGradeName = sGradeName;
	if (sGradeName == _T(""))
	{
		strGradeName = _T("无");
	}  
	...
	_stprintf(szText, szGradeForamt, _T("当前班级："), strGradeName.c_str());
	m_pTeacherGradeName->SetShowHtml();
	m_pTeacherGradeName->SetText(szText);
}
