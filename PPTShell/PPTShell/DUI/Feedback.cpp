...
void CFeedbackDialogUI::InitWindow()
{
	...
	{
		tstring strUserContact = NDCloudUser::GetInstance()->GetUserName();
...
void CFeedbackDialogUI::OnEditDescriptionTextChanged( TNotifyUI& msg )
{
	if(m_pEditDescription)
	{
		if(m_pLblInputLeftCount)
		{
			...
			TCHAR cNum[20];
			 _itot(textLeftCount, cNum, 10);
			m_pLblInputLeftCount->SetText(cNum);
		}
		tstring strDescription = m_pEditDescription->GetText();
...
void CFeedbackDialogUI::OnEditOperationTextChanged( TNotifyUI& msg )
{
	if(m_pEditOperation)
	{
		tstring strOperation = m_pEditOperation->GetText();
...
void CFeedbackDialogUI::OnEditStyleTextChanged( TNotifyUI& msg )
{
	if(m_pEditStyle)
	{
		tstring strStyle = m_pEditStyle->GetText();
		if(strStyle.empty())
		{
			isStyleModify = false;
		}
...
void CFeedbackDialogUI::OnEditFunctionTextChanged( TNotifyUI& msg )
{
	if(m_pEditFunction)
	{
		tstring strFunction = m_pEditFunction->GetText();
...
void CFeedbackDialogUI::OnBtnAddPicClick( TNotifyUI& msg )
{
	...
	if (bRet)
	{
		if(vFiles.size()>0)
		{
			if(vFiles.size()>5)
			{
				CToast::GetInstance(this->GetHWND())->Show(_T("最多只能选择5张图片"));
			}
			else
			{
				int count=0;
				...
				{
					TCHAR c[MAX_PATH];
					_itot(leftCount, c, 10);
					tstring strLeftCount = c;
					tstring strLeftTip = _T("只能再选择");
...
void CFeedbackDialogUI::AddPictures( vector<tstring> files )
{
	if(files.size()>0)
	{
		...
		{
			//判断文件大小
			if (PathFileExists(files[i].c_str()))
			{
		...
		if(sizeNotify)
		{
			CToast::GetInstance(this->GetHWND())->Show(_T("图片大小不能大于5M"));
		}
		else if(needNotify)
		{
			CToast::GetInstance(this->GetHWND())->Show(_T("相同图片不能再次添加"));
		}
		RefreshPictureList();
	}
}
...
void CFeedbackDialogUI::OnBtnSubmitClick( TNotifyUI& msg )
{
	...
	{
		CToast::GetInstance(this->GetHWND())->Show(_T("请先选择您想要反馈的问题"));
	}
}
...
void CFeedbackDialogUI::StartUploadPic1Task()
{
	m_dwUploadPic1TaskId = HttpUploadManager::GetInstance()->AddUploadTask(Feedback_host, Feedback_UploadPicUrl, 
		m_strPic1Path.c_str(), _T(""), _T("POST"), 80, MakeHttpDelegate(this, &CFeedbackDialogUI::OnUploadPic1Compeleted), MakeHttpDelegate(NULL));
	...
	if(pNotify)
	{
		if(pNotify->dwErrorCode==0 && pNotify->nDataSize>0)
		{
			try
			{
				...
				if(res)
				{
					tstring strPic = Utf82Str(root["pic"].asString());
...
void CFeedbackDialogUI::StartUploadPic2Task()
{
	m_dwUploadPic2TaskId = HttpUploadManager::GetInstance()->AddUploadTask(Feedback_host, Feedback_UploadPicUrl, 
		m_strPic2Path.c_str(), _T(""), _T("POST"), 80, MakeHttpDelegate(this, &CFeedbackDialogUI::OnUploadPic2Compeleted), MakeHttpDelegate(NULL));
...
void CFeedbackDialogUI::StartUploadPic3Task()
{
	m_dwUploadPic3TaskId = HttpUploadManager::GetInstance()->AddUploadTask(Feedback_host, Feedback_UploadPicUrl, 
		m_strPic3Path.c_str(), _T(""), _T("POST"), 80, MakeHttpDelegate(this, &CFeedbackDialogUI::OnUploadPic3Compeleted), MakeHttpDelegate(NULL));
...
void CFeedbackDialogUI::StartUploadPic4Task()
{
	m_dwUploadPic4TaskId = HttpUploadManager::GetInstance()->AddUploadTask(Feedback_host, Feedback_UploadPicUrl, 
		m_strPic4Path.c_str(), _T(""), _T("POST"), 80, MakeHttpDelegate(this, &CFeedbackDialogUI::OnUploadPic4Compeleted), MakeHttpDelegate(NULL));
...
bool CFeedbackDialogUI::OnUploadPic4Compeleted( void* param )
{
	...
	if(pNotify)
	{
		if(pNotify->dwErrorCode==0&&pNotify->nDataSize>0)
		{
			...
			if(res)
			{
				tstring strPic = Ansi2Str(root["pic"].asString());
				m_vRemotePictures.push_back(strPic);
				if(!m_strPic5Path.empty())
				{
					StartUploadPic5Task();
...
void CFeedbackDialogUI::StartUploadPic5Task()
{
	m_dwUploadPic5TaskId = HttpUploadManager::GetInstance()->AddUploadTask(Feedback_host, Feedback_UploadPicUrl, 
		m_strPic5Path.c_str(), _T(""), _T("POST"), 80, MakeHttpDelegate(this, &CFeedbackDialogUI::OnUploadPic5Compeleted), MakeHttpDelegate(NULL));
...
void CFeedbackDialogUI::StartPostFeedbackTask()
{
	...
	if(pHttpDownloadManager)
	{
		...
		for (int i=0;i<m_vRemotePictures.size();i++)
		{
			if(i==0)
			{
				strPic = Str2Ansi(m_vRemotePictures[i]);
			}
			else
			{
				strPic +="|"+ Str2Ansi(m_vRemotePictures[i]);
		...
		jData["contacContent"] = Str2Ansi(strContent);
		//联系类型
		if(!strContent.empty())
		{
		...
		{
			tstring strContent= m_pEditOperation->GetText();
			jop["B"] = Str2Ansi(strContent);
		}
		if(m_pEditStyle&&m_pEditStyle->IsVisible()&&isStyleModify)
		{
			tstring strContent= m_pEditStyle->GetText();
			jop["C"] = Str2Ansi(strContent);
		}
		if(m_pEditFunction&&m_pEditFunction->IsVisible()&&isFunctionModify)
		{
			tstring strContent= m_pEditFunction->GetText();
			jop["D"] = Str2Ansi(strContent);
		}
		jData["op"]=jop;
		//问题描述
		if(m_pEditDescription&&isDescriptionModify)
		{
			tstring strDescript = m_pEditDescription->GetText();
			jData["prbDescript"]=Str2Ansi(strDescript);
		}
		//用户信息
		DWORD dwUserId = NDCloudUser::GetInstance()->GetUserId();
		...
		string strUserName = Str2Ansi(NDCloudUser::GetInstance()->GetUserName());
		jData["userName"]=strUserName;
		jData["versionCode"]=Str2Ansi(g_Config::GetInstance()->GetVersion());
		jData["fro"]=1;
...
void CFeedbackDialogUI::ShowUserContactMode(tstring strUserContact)
{
...
bool CFeedbackDialogUI::IsMailString( tstring strMail)
{
	bool result = false;
	tregex regexMail(_T("^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\\.[a-zA-Z0-9_-]+)+$"));
	tsmatch sResult;
	result = regex_search(strMail, sResult, regexMail);
	return result;
}

bool CFeedbackDialogUI::IsPhoneString( tstring strPhone )
{
	bool result = false;
	tregex regexPhone(_T("^1[34578]\\d{9}$"));
	tsmatch sResult;
	result = regex_search(strPhone, sResult, regexPhone);
	return result;
}
