...
void CGroupExplorerUI::InitWindow()
{
	...
	m_pUploadList	= dynamic_cast<CVerticalLayoutUI*>(m_PaintManager.FindControl(_T("uploadcontent")));
		...
		else
		{
			...
			if(m_nCurrentType==LocalFileBasicExercises||m_nCurrentType==LocalFileInteractiveExercises)
			{
				for (iter; iter != pFileList->end(); iter++)  //zcs 11-26
				{
					...
					{
						tstring fileName = strPath.substr(strPath.find_last_of(_T("\\"))+1);
...
void CGroupExplorerUI::InsertKeyWordJson(tstring & strKeyWord)
{
	...
	FILE* fp = _tfopen(m_strCloudKWPath.c_str(), _T("wb+"));
	if( fp == NULL )
	{
		return ;
	}
...
{
	if(m_nCurrentType==LocalFileCourse||m_nCurrentType==LocalFileBasicExercises||
		m_nCurrentType==LocalFileInteractiveExercises||m_nCurrentType==LocalFileVideo||
		m_nCurrentType==LocalFileImage||m_nCurrentType==LocalFileFlash||m_nCurrentType==LocalFileVolume)
	{
...
void CGroupExplorerUI::OnSeachFailHandle()
{
	...
	for (int i = 0; i < nLevel; i++)
	{
		...
		do 
		{
			...
			strKeyWord1 = valueKey["keyword"].asString();
	...
	case WM_DROPFILES:
		{
			...
			if (m_pUploadList->IsVisible())
			{
				CDBankUploadUI* pUpload = dynamic_cast<CDBankUploadUI*>(m_pUploadList->FindSubControl(_T("uploadLayout")));
				if (pUpload)
				{
...
void CGroupExplorerUI::ShowUploadUI()
{
	...
	//m_laySeeMore->SetVisible(false);
	m_pSliderTabLayout->SelectItem(7,false);
	...
	SetDlgAcceptFiles(true);
	if(m_PaintManager.FindControl(_T("uploadLayout")))
		return;
	...
	if(pUpload)
	{
		pUpload->SetName(_T("uploadLayout"));
		m_pUploadList->Add(pUpload);
	}
					...
					for(int i=0;i<m_pList->GetCount();i++)
					{
						...
						if(pItem)
						{
							if(strResource.compare(pItem->GetResource())==0)
							{
								...
								if(m_lbCount)
								{
									tstring strCount = m_lbCount->GetText();
									int count = _ttoi(strCount.c_str());
									if(--count>=0)
									{
										SetCountText(count);
									}
	...
	if(pObj)
	{
		...
		if(pNotify)
		{
			...
			if(pItem)
			{
				...
				{
					tstring strNewName = pRenameDlg->GetNewName();
					if(!strNewName.empty())
					{
						if(isClound)
						{
