	...
	if (pEvent->Type == UIEVENT_TIMER)
	{
		...
		else if (pEvent->wParam == TimerId_CountDown)
		{
			...
			else
			{
				...
				CButtonUI* pStopBtn		= (CButtonUI*)m_PaintManager.FindControl(_T("vr_stop"));
				CButtonUI* pPlayBtn	= (CButtonUI*)m_PaintManager.FindControl(_T("vr_play"));
				...
				if (strtype==_T("VRTYPE_EXE")){
					CMainFrame* pMainFrame = (CMainFrame*)AfxGetMainWnd();
					pMainFrame->Show3DAmination(strExePath);
				}if (strtype==_T("VRTYPE_VIDEO")){
					CMainFrame* pMainFrame = (CMainFrame*)AfxGetMainWnd();
		...
		else if (pEvent->wParam == TimerId_UpdatePos)
		{
			...
			UpdateInstrumentPos();
			m_PaintManager.SetTimer(m_layToolTab, TimerId_Adjust, 500);
		}
	}
	return true;
}
	...
	if (g_Config::GetInstance()->GetModuleVisible(MODULE_ND_ICRPLAY))
	{
		...
		else
		{
			...
			else
			{
				...
				if (!NDCloudUser::GetInstance()->CheckItemsSize(strStudentInfo, _T("items")))//有班级信息但是没有学生信息，也进入自由授课模式
				{
					NDCloudUser::GetInstance()->SetFreeMode(1);
				} 
				else
...
void CScreenInstrumentUI::CloseInstrument()
{
	...
	MagicPen_MainUI::GetInstance()->CloseLocalWindows();//关闭放大镜功能2016.03.10 cws
	//SetTaskBar(true);
...
void CScreenInstrumentUI::OnPageScanneded()
{
	...
	{
		CButtonUI* pPalyBtn		= (CButtonUI*)m_PaintManager.FindControl(_T("vr_play"));
		CButtonUI* pStopBtn		= (CButtonUI*)m_PaintManager.FindControl(_T("vr_stop"));
		CContainerUI* pBtnsLay	= (CContainerUI*)m_PaintManager.FindControl(_T("vr_tool_lay"));

		pBtnsLay->SetVisible(true);
...
LRESULT CScreenInstrumentUI::HandleMessage( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	{	
		...
		pFnGetGestureInfo pGetGestureInfo = (pFnGetGestureInfo)::GetProcAddress(hUserDll, "GetGestureInfo");
		if(pGetGestureInfo)
		{
	...
	else if(uMsg == WM_SHOWMAGICPEN)
	{
		// 打开放大镜
		OutputDebugString(_T("CScreenInstrumentUI::HandleMessage - ShowMagicPen\n"));
		MagicPen_MainUI::GetInstance()->SetControlMode();
		MagicPen_MainUI::GetInstance()->ShowLocalWindows();
		MagicPen_MainUI::GetInstance()->SetControlMode(2);
	}
	else if(uMsg == WM_HIDEMAGICPEN)
	{
		// 关闭放大镜
		OutputDebugString(_T("CScreenInstrumentUI::HandleMessage - CloseMagicPen\n"));
		//MagicPen_MainUI::GetInstance()->SetMode(1);
		MagicPen_MainUI::GetInstance()->SetControlMode();
		MagicPen_MainUI::GetInstance()->CloseLocalWindows();
		MagicPen_MainUI::GetInstance()->SetControlMode(2);
	}
	else if(uMsg == WM_MOVEMAGICPEN)
	{
		// 拖动放大区域
		//OutputDebugString(_T("CScreenInstrumentUI::HandleMessage - MoveMagicPen\n"));
		...
		if(wParam == 0)
		{
			MagicPen_MainUI::GetInstance()->SetControlMode();
			TCHAR szLog[256];
			_stprintf(szLog, _T("CScreenInstrumentUI::HandleMessage - Start Draw,Point x=%d, y=%d\n"), pPoint->x, pPoint->y);
			OutputDebugString(szLog);
		}

		MagicPen_MainUI::GetInstance()->DrawRoundRect(wParam, pPoint->x, pPoint->y);
	...
	else if(uMsg == WM_OPENBLACKBOARD)
	{
		// 打开黑板
		OutputDebugString(_T("CScreenInstrumentUI::HandleMessage - OpenBlackBoard\n"));
		CBlackBoarderUI::GetInstance()->ShowBlackBoard();
	}
	else if(uMsg == WM_CLOSEBLACKBOARD)
	{
		// 关闭黑板
		OutputDebugString(_T("CScreenInstrumentUI::HandleMessage - CloseBlackBoard\n"));
		CBlackBoarderUI::GetInstance()->CloseBlackBoard();
	}
...
void CScreenInstrumentUI::SetPreNextButtonEnable( bool enable )
{
	...
	{
		CControlUI* pre_action = m_pLeftActionButtons->FindSubControl(_T("pre_action"));
		if(pre_action)
		{
			pre_action->SetEnabled(enable);
		}
		CControlUI* next_action = m_pLeftActionButtons->FindSubControl(_T("next_action"));
		if(next_action)
		{
	...
	{
		CControlUI* pre_action = m_pRightActionButtons->FindSubControl(_T("pre_action"));
		if(pre_action)
		{
			pre_action->SetEnabled(enable);
		}
		CControlUI* next_action = m_pRightActionButtons->FindSubControl(_T("next_action"));
		if(next_action)
		{
			next_action->SetEnabled(enable);
		}
...
void CScreenInstrumentUI::HandleDrawMessage( tstring lpEventName, tstring lptcsEventData, tstring& strResult )
{
// 	strResult = "";
...
void CScreenInstrumentUI::StopVR(BOOL nPlay)
{
	if (IsWindow(this->GetHWND())){
		...
		else if ( strtype == _T("VRTYPE_VIDEO") )
		{
			CMainFrame* pMainFrame = (CMainFrame*)AfxGetMainWnd();
...
void CScreenInstrumentUI::StopTopMostTimer()
{
	if (m_layToolTab)
	{
		m_PaintManager.KillTimer(m_layToolTab, TimerId_TopMost);
...
void CScreenInstrumentUI::StartTopMostTimer()
{
	if (m_layToolTab)
	{ 
		MagicPen_MainUI::GetInstance()->TopMostView(); 
		m_PaintManager.SetTimer(m_layToolTab, TimerId_TopMost, 2000);
	}
}

void CScreenInstrumentUI::TopMostView()
{
