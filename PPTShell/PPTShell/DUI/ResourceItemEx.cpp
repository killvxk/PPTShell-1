...
bool CResourceItemExUI::OnItemEvent(void *param)
{
	...
	else if( pEvent->Type == UIEVENT_MOUSEMOVE )
	{
		...
		SHORT nLeftButtonPressed = GetAsyncKeyState(VK_LBUTTON);

		if( m_dwDragStartTime != 0 && nDiff > 10 && nLeftButtonPressed != 0 )
		{
			int width = GetWidth();
			...
			if( m_pDragDialog == NULL )
			{
				...
				Gdiplus::Image* pImage = new Image(Str2Unicode(strImage).c_str());
				...
				m_pDragDialog->BeginShowWindow(hWnd);
...
void CResourceItemExUI::OnDeleteComplete( LPCTSTR lptcsError )
{
	...
	{
		//toast or ?
		CToast::Toast( _T("删除失败:") + tstring(lptcsError));
		return;
	}
...
void CResourceItemExUI::OnRenameComplete( LPCTSTR lptcsError, LPCTSTR lptcsNewName )
{
	...
	{
		//toast or ?
		CToast::Toast( _T("重命名失败:") + tstring(lptcsError));
		return;
	}
...
CResourceItemExUI* CResourceItemExUI::AllocResourceItem( int nItemType, int nFromType /*= -1*/ )
{
		...
		case DBankCourse:
			{
				...
				pCompnent				= new CCloudComponent(pCompnent, lptcsType);
