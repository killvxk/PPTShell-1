...
void CRenameDlgUI::OnBtnOK( TNotifyUI& msg )
{
	...
	if(m_pEditRename)
	{
		tstring strText= m_pEditRename->GetText();
		if(strText.empty())
		{
			CToast::Toast(_T("请输入名称"));
		}
		else
		{
...
tstring CRenameDlgUI::GetNewName()
{
	return m_strNewName;
}
...
void CRenameDlgUI::SetOldName2Edit(tstring strOldName)
{
	if (m_pEditRename)
	{
