...
void CCloudResourceHandler::OnDownloadCompleted( THttpNotify* pHttpNotify )
{
	...
	{
		RemoveDownloader((CResourceDownloader*)pTranfer, eResult_Error);
	}
	else
	{
...
void CCloudResourceHandler::OnDownloadInterpose( THttpNotify* pHttpNotify )
{
	...
	{
		ITransfer*		pTranfer= (ITransfer*)pHttpNotify->pDetailData;
		RemoveDownloader((CResourceDownloader*)pTranfer, eResult_Cancel);
...
void CCloudResourceHandler::RemoveDownloader( CResourceDownloader* pDownloader, int nResult /*= 0*/ )
{
	if (m_pCurrentDownloader == pDownloader)
	{
		m_pCurrentDownloader = NULL;
		CResourceDownloaderManager::GetInstance()->NotifyDownloaderDestroy(pDownloader, nResult);
