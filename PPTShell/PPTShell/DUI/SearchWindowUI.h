...
private:
	...
	CVerticalLayoutUI*				m_pSearchLCOptionsLayout;
	CVerticalLayoutUI*				m_pSearchBaiduOptionsLayout;
	...
	vector<CCheckBoxUI*>m_vecCheckBoxs;			//101教育库可选项容器
	vector<CCheckBoxUI*>m_vecBaiduCheckBoxs;	//百度搜索可选项容器

	HANDLE				m_hThread;

	DWORD				m_dwCheckBoxFlag;
	DWORD				m_dwBaiduCheckBoxFlag;

	bool				m_bSelectAllCheckBox;	//判断
	bool				m_bBaiduSelectAllCheckBox;
...
protected:
	...
	bool				OnCheckBoxALLNotify(void * pObj);		//101教育库搜索可选项事件
	bool				OnCheckBoxNotify(void * pObj);			//101教育库搜索可选项事件
	bool				OnBaiduCheckBoxALLNotify(void * pObj);	//百度搜索可选项事件
	bool				OnBaiduCheckBoxNotify(void * pObj);		//百度搜索可选项事件
...
private:
	...
	void				BaiduCheckBoxSelectAll(bool bSelected);
	void				SetBaiduAllCheckBox(bool bSelected);
	...
	void				ClearSelectedOptions(int nType);//清除可选项
};