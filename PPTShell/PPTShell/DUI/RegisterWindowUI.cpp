	...
	case 0:
		{
			...
			//
			tregex szUserNameRule(_T("^\\d+$"));
			tsmatch sResult;
			if(regex_search(strRegisterUserName, sResult, szUserNameRule))
			{
				//手机
				tregex szUserNameRuleEx(_T("^1[34578]\\d{9}$"));

				if(!regex_search(strRegisterUserName, sResult, szUserNameRuleEx))
				{
			...
			{
				//邮箱
				tregex szUserNameRuleEx(_T("^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\\.[a-zA-Z0-9_-]+)+$"));
				if(!regex_search(strRegisterUserName, sResult, szUserNameRuleEx))
				{
