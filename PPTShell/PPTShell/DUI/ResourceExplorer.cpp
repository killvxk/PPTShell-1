...
void CResourceExplorerUI::SetTitleText( LPCTSTR lptcsName )
{
	if (lptcsName != NULL)
	{
		SIZE textSize = CRenderEngine::GetTextSize(m_PaintManager.GetPaintDC(), &m_PaintManager, lptcsName, m_lbName->GetFont(), DT_CALCRECT | DT_WORDBREAK | DT_EDITCONTROL | DT_LEFT| DT_NOPREFIX);
		m_lbName->SetText(lptcsName);
		m_lbName->SetFixedWidth(textSize.cx);
