...
bool CMessageBoxUI::OnTopmostWndEvent(void* pObj)
{
	TEventUI* pEvent = (TEventUI*)pObj;
	if (pEvent->Type == UIEVENT_TIMER)
	{
		if (pEvent->wParam == TOPMOST_TIMER_ID)
		{
			 SetWindowPos(this->GetHWND(), HWND_TOPMOST, 0,0,0,0, SWP_NOSIZE| SWP_NOMOVE| SWP_SHOWWINDOW);  
		} 
	}
	return true;
} 

void CMessageBoxUI::StopTimer(UINT nResult, bool bTopMost)
{
	if (bTopMost)
	{
		if (nResult != ID_MSGBOX_BTN )//点击了取消
		{			
	
		}
		m_PaintManager.KillTimer(m_pMessageTitle,TOPMOST_TIMER_ID);
	}
}


UINT CMessageBoxUI::MessageBox(HWND hParent, LPCTSTR lpszText, LPCTSTR lpszCaption,  LPCTSTR lpszBtnType, ENMessageBoxType enIconType, UINT nDefaultButtonIndex , LPCTSTR lpszStyleId, int nWidth, int nHeight, bool bDoModal, LPCTSTR lpszId ,bool bTopMost)
{
	try
	{
		...
		{
			//增加置顶2016.03.16 cws
			m_PaintManager.SetTimer(m_pMessageTitle, TOPMOST_TIMER_ID, 1000);
			m_pMessageTitle->OnEvent += MakeDelegate(this, &CMessageBoxUI::OnTopmostWndEvent);
		}
...
UINT CMessageBoxUI::MessageBox( HWND hParent, LPCTSTR lpszMainText, DWORD dwMainTextColor, int nMainTextFont, LPCTSTR lpszText, DWORD dwTextColor, int nTextFont, LPCTSTR lpszCaption, LPCTSTR lpszBtnType, ENMessageBoxType enIconType /*= enMessageBoxTypeInfo*/, UINT nDefaultButtonIndex /*= 0*/, LPCTSTR lpszStyleId /*= NULL*/, int nWidth /*= 340*/, int nHeight /*= 150*/,bool bDoModal /*= true */,LPCTSTR lpszId /*= NULL */,bool bTopMost )
{
	int scrWidth, scrHeight;  
	RECT rect;  
...
UINT  UIMessageBox(HWND hParent, LPCTSTR lpszText, LPCTSTR lpszCaption, LPCTSTR lpszBtnType, CMessageBoxUI::ENMessageBoxType enIconType/* = CMessageBoxUI::enMessageBoxTypeInfo*/, UINT nDefaultButtonIndex, LPCTSTR lpszStyleId /*= NULL*/, int nWidth/* = 340*/, int nHeight/* = 150*/,bool bDoModal/* = true */,LPCTSTR lpszId /* =NULL */,bool bTopmost/* = false*/)
{
	...
	if(pMsgBox->GetHWND() == NULL)
		pMsgBox->Create(hParent, _T("MessageBox"), WS_POPUP, WS_EX_TOPMOST, 0, 0, 0, 0);//弹出框置顶 2016.03.10 cws
	...
	UINT nResult = pMsgBox->MessageBox(hParent, lpszText, lpszCaption, lpszBtnType, enIconType, nDefaultButtonIndex,  lpszStyleId, nWidth, nHeight,bDoModal,lpszId, bTopmost);
	pMsgBox->StopTimer(nResult, bTopmost);
	//ShowWindow(pMsgBox->GetHWND(), SW_SHOW);

	return nResult;
}

UINT UIMessageBox( HWND hParent, LPCTSTR lpszMainText, DWORD dwMainTextColor, int nMainTextFont, LPCTSTR lpszText, DWORD dwTextColor, int nTextFont, LPCTSTR lpszCaption, LPCTSTR lpszBtnType, CMessageBoxUI::ENMessageBoxType enIconType /*= CMessageBoxUI::enMessageBoxTypeInfo*/, UINT nDefaultButtonIndex /*= 0*/, LPCTSTR lpszStyleId /*= NULL*/, int nWidth /*= 340*/, int nHeight /*= 150*/,bool bDoModal /*= true */,LPCTSTR lpszId /*= NULL */ ,bool bTopmost /*= false*/)
{
	...
	UINT nResult = pMsgBox->MessageBox(hParent, lpszMainText, dwMainTextColor, nMainTextFont, lpszText, dwTextColor, nTextFont, lpszCaption, lpszBtnType, enIconType, nDefaultButtonIndex,  lpszStyleId, nWidth, nHeight,bDoModal,lpszId, bTopmost);
	//ShowWindow(pMsgBox->GetHWND(), SW_SHOW); 
	pMsgBox->StopTimer(nResult, bTopmost);
	return nResult;
}
