...
CSearchWindowUI::CSearchWindowUI()
{
	...
	m_dwBaiduCheckBoxFlag	= CheckBox_Course | CheckBox_Image;
...
void CSearchWindowUI::Init()
{
	...
	m_pTipTriangle->SetVisible(true);
#else
	m_pTipTriangle->SetVisible(false);
#endif
	...
	m_pSearchHistoryLayout	= dynamic_cast<CVerticalLayoutUI*>(FindSubControl(_T("searchhistory")));
	m_pSearchLoadingLayout	= dynamic_cast<CVerticalLayoutUI*>(FindSubControl(_T("searchLoading")));
	m_pSearchContentLayout	= dynamic_cast<CVerticalLayoutUI*>(FindSubControl(_T("searchContent")));

	m_pSearchLCOptionsLayout	= dynamic_cast<CVerticalLayoutUI*>(FindSubControl(_T("searchLCSelectOptions")));
	m_pSearchBaiduOptionsLayout	= dynamic_cast<CVerticalLayoutUI*>(FindSubControl(_T("searchBaiduSelectOptions")));

	m_pNetlessLayout		= dynamic_cast<CVerticalLayoutUI*>(FindSubControl(_T("netlessLayout")));
	m_pSearchContentList	= dynamic_cast<CTileWithButtonLayoutUI*>(FindSubControl(_T("searchContentList")));
	...
	CButtonUI* pButton		= dynamic_cast<CButtonUI*>(FindSubControl(_T("clearHistory")));
	...
	pCheckBox = dynamic_cast<CCheckBoxUI*>(FindSubControl(_T("boxallBaidu")));
	pCheckBox->OnNotify += MakeDelegate(this, &CSearchWindowUI::OnBaiduCheckBoxALLNotify);
	pCheckBox->Selected(true);
	m_vecBaiduCheckBoxs.push_back(pCheckBox);
	pCheckBox = dynamic_cast<CCheckBoxUI*>(FindSubControl(_T("boxcourseBaidu")));
	pCheckBox->OnNotify += MakeDelegate(this, &CSearchWindowUI::OnBaiduCheckBoxNotify);
	pCheckBox->Selected(true);
	m_vecBaiduCheckBoxs.push_back(pCheckBox);
	pCheckBox = dynamic_cast<CCheckBoxUI*>(FindSubControl(_T("boximageBaidu")));
	pCheckBox->OnNotify += MakeDelegate(this, &CSearchWindowUI::OnBaiduCheckBoxNotify);
	pCheckBox->Selected(true);
	m_vecBaiduCheckBoxs.push_back(pCheckBox);
	//
...
bool CSearchWindowUI::OnBtnSearchNotify( void * pObj )
{
	...
	if (pNotify->sType == DUI_MSGTYPE_CLICK)
	{
		if(CheckNetStatus() || m_pSearchContentLayout->IsVisible())
		{
			...
			tstring strKeyWord = m_pSearchEdit->GetText();
			...
			if(m_dwSearchType == Search_Type_101PPT && m_dwCheckBoxFlag == CheckBox_NULL)
			{
			...
			else if(m_dwSearchType == Search_Type_Baidu && m_dwBaiduCheckBoxFlag == CheckBox_NULL)
			{
				CToast::Toast(_T("请选择搜索项目"));
				if(!m_pSearchHistoryLayout->IsVisible() && !m_pSearchContentLayout->IsVisible())
					ShowSearchHistory();
...
void CSearchWindowUI::ShowSearchHistory()
{
	...
	FILE* fp = _tfopen(m_strKeyWordPath.c_str(), _T("rb"));
	...
	if(nCount > 0)
	{
		for(int i = 0; i< nCount ; i++)
		{
			...
			tstring strKeyWord = Ansi2Str(value["keyword"].asString());
			tstring strdate = Ansi2Str(value["date"].asString());
...
void CSearchWindowUI::InsertKeyWordJson( tstring & strKeyWord )
{
	...
	valueKW["keyword"] = Str2Ansi(strKeyWord);
	valueKW["date"] = Str2Ansi(strTime.GetString());
	...
	for(int i = 0; i < (int)dwCount ; i++)
	{
		...
		if( strKeyWord == Ansi2Str(valueKW["keyword"].asString()))
		{
			nExsit = i;
	...
	string str = m_jsonKeyWordWriter.write(valueNew);

	m_jsonKeyWordRoot = valueNew;

	FILE* fp = _tfopen(m_strKeyWordPath.c_str(), _T("wb+"));
	if( fp == NULL )
	{
		return ;
	}
...
DWORD		WINAPI CSearchWindowUI::OnSearchWithBaiduThread( LPARAM lParam )
{
	...
	if( pSearchWindowUI->m_dwBaiduCheckBoxFlag & CheckBox_Course)
	{
		...
		if(!pSearchWindowUI->m_mapResCount[nType]->bEnd)
		{
			...
			string strResult = Str2Ansi(strJsonResult);
			Json::Reader		m_jsonReader;
			Json::Value			m_jsonRoot;
			bool bRet = m_jsonReader.parse(strResult.c_str(),m_jsonRoot);
			if(bRet)
			{
				//
				...
				if(nCount != Request_Count)
				{
					pSearchWindowUI->m_mapResCount[nType]->bEnd = true;
				}
				for(int i = 0 ; i < nCount ; i++)
				{
					// count
	...
	if( pSearchWindowUI->m_dwBaiduCheckBoxFlag & CheckBox_Image)
	{
		...
		if(!pSearchWindowUI->m_mapResCount[nType]->bEnd)
		{
			...
			if(bRet)
			{
				...
				Json::Value& items = m_jsonRoot["ItemsInfo"];
				int nTempCount = items.size();
				pSearchWindowUI->m_mapResCount[nType]->nStart += nTempCount;
				if(nTempCount != (Request_Count - nCount))
				{
					pSearchWindowUI->m_mapResCount[nType]->bEnd = true;
				}
				...
				{
					// count
					pImageStream->WriteDWORD(nTempCount);
...
void CSearchWindowUI::ShowSearchResult()
{
	...
	if(m_dwRequestedResCount < m_dwResourceTotal && m_dwSearchType == Search_Type_101PPT)
	{
		m_pSearchContentList->ShowMoreButton();//显示加载更多的按钮
		m_pSearchContentList->SetBtnMoreCallBack(MakeDelegate(this, &CSearchWindowUI::OnBtnMoreCallBack));
	}

	if(m_dwSearchType == Search_Type_Baidu && m_dwRequestedResCount != 0)
	{
		bool bEnd = false;
		map<int,Record_Requested*>::iterator iter;
		for(iter = m_mapResCount.begin() ; iter != m_mapResCount.end() ; iter++)
...
void CSearchWindowUI::BaiduCheckBoxSelectAll( bool bSelected )
{
	for(int i = 0 ;i < m_vecBaiduCheckBoxs.size(); i++)
	{
		if(m_vecBaiduCheckBoxs[i]->GetName() != _T("boxallBaidu"))
		{
			m_vecBaiduCheckBoxs[i]->Selected(bSelected);
		}
	}
}
...
void CSearchWindowUI::SetBaiduAllCheckBox( bool bSelected )
{
	for(int i = 0 ;i < m_vecBaiduCheckBoxs.size(); i++)
	{
		if(m_vecBaiduCheckBoxs[i]->GetName() == _T("boxallBaidu"))
...
bool CSearchWindowUI::OnBaiduCheckBoxALLNotify( void * pObj )
{
	TNotifyUI* pNotify = (TNotifyUI *)pObj;
	if(pNotify->sType == DUI_MSGTYPE_CLICK)
	{
		CCheckBoxUI* pCheckBox = dynamic_cast<CCheckBoxUI*>(pNotify->pSender);
		m_bBaiduSelectAllCheckBox = true;
		BaiduCheckBoxSelectAll(!pCheckBox->IsSelected());
	}
	return true;
}
...
bool CSearchWindowUI::OnBaiduCheckBoxNotify( void * pObj )
{
	TNotifyUI* pNotify = (TNotifyUI *)pObj;
	if(pNotify->sType == DUI_MSGTYPE_CLICK)
	{
		m_bBaiduSelectAllCheckBox = false;
	}
	if(pNotify->sType == DUI_MSGTYPE_SELECTCHANGED)
	{
		CCheckBoxUI* pCheckBox = dynamic_cast<CCheckBoxUI*>(pNotify->pSender);
		if(pCheckBox->GetName() == _T("boxcourseBaidu"))
		{
			if(pCheckBox->IsSelected())
				m_dwBaiduCheckBoxFlag |= CheckBox_Course;
			else
				m_dwBaiduCheckBoxFlag &= ~CheckBox_Course;
		}
		else if(pCheckBox->GetName() == _T("boximageBaidu"))
		{
			if(pCheckBox->IsSelected())
				m_dwBaiduCheckBoxFlag |= CheckBox_Image;
			else
				m_dwBaiduCheckBoxFlag &= ~CheckBox_Image;
		}
		if(!m_bBaiduSelectAllCheckBox)
		{
			if(m_dwBaiduCheckBoxFlag == (CheckBox_Course | CheckBox_Image))
				SetBaiduAllCheckBox(true);
			else
				SetBaiduAllCheckBox(false);
		}
	}
...
bool CSearchWindowUI::SelectSearchType( void* pObj )
{
	...
	if(pButton->GetName() == _T("tipListPPT"))
	{
		...
		m_pSearchBaiduOptionsLayout->SetVisible(false);
		m_pSearchLCOptionsLayout->SetVisible(true);
	...
	else if(pButton->GetName() == _T("tipListBaidu"))
	{
		...
		m_pSearchLCOptionsLayout->SetVisible(false);
		m_pSearchBaiduOptionsLayout->SetVisible(true);
...
void CSearchWindowUI::ClearSelectedOptions( int nType )
{
	if(nType == Search_Type_101PPT)
	{

	}
	else if(nType == Search_Type_Baidu)
	{
