...
#define EduPlatformUrlQuestionInfo					_T("/v0.6/questions/actions/query?words%s&limit=(%d,%d)&include=TI,CG,LC&category=$RE0200&category_exclude=$RE0211,$RE0206&relation%s&coverage=%s")
...
tstring NDCloudComposeUrlCourseInfo(tstring strChapterGUID /*= ""*/, tstring strSearchKeyWord /*=""*/, int nLimitStart /*=0*/, int nLimitEnd /*=0*/,DWORD dwUserId /*= 0*/)
{
	if( strSearchKeyWord != _T("") )
		strSearchKeyWord = _T("=") + UrlEncodeEx(Str2Utf8(strSearchKeyWord));
...
int NDCloudDecodeExercisesCount(char* pData, int nDataSize)
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		if( !item["preview"].isNull() && !item["preview"]["question_small"].isNull() )
			strPreviewUrl = Ansi2Str(item["preview"]["question_small"].asCString());

		// item.xml href 
		if( !item["tech_info"].isNull() && !item["tech_info"]["href"].isNull() && !item["tech_info"]["href"]["location"].isNull() )
			strXmlUrl = Ansi2Str(item["tech_info"]["href"]["location"].asCString());

		if( !item["life_cycle"].isNull() && !item["life_cycle"]["status"].isNull() )
		{
			strStatus = Ansi2Str(item["life_cycle"]["status"].asCString());

			_tcslwr((TCHAR*)strStatus.c_str());
...
BOOL NDCloudDecodeImageList(IN char* pData, IN int nDataSize, OUT CStream* pImageStream)
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		pImageStream->WriteString(Ansi2Str(strItemJson));
	...
	int nStart, nEnd;
	if(_stscanf_s(strLimit.c_str(), _T("(%d,%d)"), &nStart, &nEnd ) == 2)
	{
...
BOOL NDCloudDecodeVolumeList(IN char* pData, IN int nDataSize, OUT CStream* pVolumeStream)
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		pVolumeStream->WriteString(Ansi2Str(strItemJson));
	}
...
BOOL NDCloudDecodeVideoList(IN char* pData, IN int nDataSize, OUT CStream* pVideoStream)
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		pVideoStream->WriteString(Ansi2Str(strItemJson));
	}
...
BOOL NDCloudDecodeFlashList(IN char* pData, IN int nDataSize, OUT CStream* pFlashStream)
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		pFlashStream->WriteString(Ansi2Str(strItemJson));
	}
...
BOOL NDCloudDecodePPTTemplateList(IN char* pData, IN int nDataSize, OUT CStream* pPPTTemplateStream)
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		if( bPPTUrlMissing )
		{
			...
			if( pHttpManager )
			{
				pHttpManager->AddTask(_T("p.101.com"), _T("/101ppt/resourceMissing.php"), _T(""), _T("POST"), Str2Ansi(strPost).c_str(), 80, 
					MakeHttpDelegate(NULL), 
					MakeHttpDelegate(NULL), 
					MakeHttpDelegate(NULL), 
					FALSE);
		...
		for(int j = 0; j < nThumbCount; j++)
		{
			...
			if( !previewItem.isNull() )
			{
				tstring strThumbUrl = Ansi2Str(previewItem.asCString());
		...
		pPPTTemplateStream->WriteString(Ansi2Str(strItemJson));
	}
...
BOOL NDCloudDecodeCourseList(IN char* pData, IN int nDataSize, OUT CStream* pCourseStream)
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		Json::FastWriter writter;
		string strItemJson = writter.write(item);
		pCourseStream->WriteString(Ansi2Str(strItemJson));
	}
...
BOOL NDCloudDecodeQuestionList(IN char* pData, IN int nDataSize, OUT CStream* pQuestionStream)
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		pQuestionStream->WriteString(Ansi2Str(strItemJson));
	}
...
BOOL NDCloudDecodeCoursewareObjectsList( IN char* pData, IN int nDataSize, OUT CStream* pCoursewareObjectsStream )
{
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		if( !item["categories"].isNull() )
		{
			if(!item["categories"]["res_type"].isNull())
			{
				...
				for(int i = 0; i < (int)itemResType.size(); i++)
				{
					...
					{
						strCoursewareObjectsName = Ansi2Str(item["categories"]["res_type"][1]["taxonname"].asCString());
						break;
					}
				}
			}
		...
		pCoursewareObjectsStream->WriteString(Ansi2Str(strItemJson));
	}
...
BOOL NDCloudDecodeVRResourceList(IN char* pData, IN int nDataSize, OUT CStream* pVRResourceStream, BOOL bIncludeEmptyVR /*= TRUE*/)
{
	...
	// 
	Json::Value FileItemArray(Json::arrayValue);
	Json::Value EmptyFileItemArray(Json::arrayValue);
	...
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		// file url
		if( !item["tech_info"].isNull() && !item["tech_info"]["href"].isNull() && !item["tech_info"]["href"]["location"].isNull()  )
			FileItemArray.append(item);
		else
			EmptyFileItemArray.append(item);
	}
...
_CYCLE:
	for(int i = 0; i < (int)items.size(); i++)
	{
		...
		if( !item["preview"].isNull() )
		{
			...
			for(int j = 0; ; j++)
			{
				char szSummaryKey[MAX_PATH];
				sprintf_s(szSummaryKey, "summary%d", j+1);
	...
	if( nCycle == 0 && bIncludeEmptyVR )
	{
		nCycle ++;
		items = EmptyFileItemArray;
		goto _CYCLE;
	}
...
BOOL NDCloudSetChapterGUID(tstring strGUID, tstring strChapterName, bool bInit)
{
	...
	CStream stream(1024);
	stream.WriteString(strGUID.c_str());
	stream.WriteString(strChapterName.c_str());
