...
bool CNDCloudContentDeleteService::OnSessionObtained(void *param)
{
	...
	tstring strSessionId = Ansi2Str(root["session"].asCString());
	...
	string strAuthorization = "Authorization: Bearer \"";
	strAuthorization += Str2Ansi(strToken);
	strAuthorization += "\"";
...
BOOL CNDCloudContentRenameService::StartService()
{
	...
	//
	string strAuthorization = NDCloudUser::GetInstance()->GetAuthorizationHeader(Str2Ansi(EduPlatformHostLC), Str2Ansi(szUrl), "PUT");
	...
	bool res = reader.parse(Str2Ansi(m_strJson), root);
	if( !res )
		return FALSE;

	root["title"] = Str2Ansi(m_strTitle);

	Json::FastWriter writter;
	string strPost = writter.write(root);
 	
	strPost = AnsiToUtf8(strPost);
