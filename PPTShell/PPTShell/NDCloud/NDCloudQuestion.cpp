...
bool CNDCloudQuestion::PollPackState()
{
	...
	NDCloudDownloadEx(strUrl, _T(""), _T("GET"), "",  MakeHttpDelegate(this, &CNDCloudQuestion::OnPackStateObtained));
		...
		if(!root["archive_state"].isNull() )
		{
			...
			else if( strState == _T("unpack") )
			{
				...
				if( !m_bPacking )
				{
					...
					string strHeader = NDCloudUser::GetInstance()->GetAuthorizationHeader(Str2Utf8(EduPlatformHost), Str2Utf8(strUrl), "POST");

					NDCloudDownloadEx(strUrl, Ansi2Str(strHeader), _T("POST"), "",  MakeHttpDelegate(this, &CNDCloudQuestion::OnPackStarted));
					m_bPacking = TRUE;
				}
			}
			else if( strState == _T("pending") )
			{
				...
				if( m_nPendingCount >= 3 )
				{
					...
					{
						tstring strUrl = NDCloudComposeUrlPackQuestion(m_strGuid, m_dwUserId);
						string strHeader = NDCloudUser::GetInstance()->GetAuthorizationHeader(Str2Ansi(EduPlatformHost), Str2Utf8(strUrl), "POST");
				...
				if (item.isArray()){
					...
					for ( int nIndex = 0;nIndex < iSize;++ nIndex )   
					{
						...
						if (subitem.isObject()){
							...
							nType.szQuestionResCode = Utf82Str(subitem["code"].asString());
							nType.szQuestionResType = Utf82Str(subitem["type"].asString());
							nType.szQuestionResLabel =  Utf82Str(subitem["label"].asString());
...
tstring CNDCloudQuestionManager::FindQuestionResType(tagQuestionOptType ntype,tstring nUrl){
	...
	{
		for(int i=0;i<m_vQuestionsType.size();i++){
			tstring nUrlRestype = _T("/")+m_vQuestionsType[i].szQuestionResType+ _T("?");
			tstring nQustionRestype = _T("&question_type=")+m_vQuestionsType[i].szQuestionResType+ _T("&");
										  ...
										  bool bBasicQuestion, CHttpDelegateBase& delegate)
{
	...
	categoriesArray.append(Str2Ansi(strCategories));
	...
	string strAuthorization = NDCloudUser::GetInstance()->GetAuthorizationHeader(Str2Ansi(EduPlatformAddQuestionHost), Str2Ansi(EduPlatformAddQuestionUrl), "POST");
...
bool CNDCloudQuestionManager::OnQuestionAdded(void* param)
{
	...
	tstring strQuestionGuid = Ansi2Str(root["identifier"].asCString());
...
tstring CNDCloudQuestionManager::ComposeBasicQuestionEditUrl(tstring strTokenInfo, tstring strUserId, tstring strQuestionGuid, tstring strChapterGuid)
{
	...
	tstring strEncodedTokenInfo = UrlEncode(szEncodedTokenInfo, false);
...
BOOL CNDCloudQuestionManager::ConvertQuestionToCourseFile(tstring strGuid, tstring strQuestionType, tstring strQuestionName, tstring strPath )
{
	...
	//// load template file
	//bool res = doc.LoadFile(Str2Ansi(strXmlFilePath).c_str());
	//if( !res )
	//	return FALSE;
	...
	//char szHref[MAX_PATH];
	//sprintf_s(szHref, "pages/page.xml?v=%u", tm.GetTime());

	//TiXmlElement *pPage= new TiXmlElement("page");

	//pPage->SetAttribute("id", Str2Ansi(strGuid).c_str());
	//pPage->SetAttribute("name", Str2Ansi(strGuid).c_str());
	//pPage->SetAttribute("href", szHref);
	//pPage->SetAttribute("preview", "");
	//pPage->SetAttribute("reportable", "true");
	//pPagesElement->LinkEndChild(pPage);

	//res = doc.SaveFile(Str2Ansi(strMainXmlPath).c_str());
	//if(!res)
	//	return FALSE;
	...
	//// save
	//res = doc.LoadFile(Str2Ansi(strXmlFilePath).c_str());
	//if( !res )
	//	return FALSE;
