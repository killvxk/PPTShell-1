...
BOOL CNDCloudLocalQuestionManager::CreateQuestion()
{
	...
	char szUrl[2048];
	sprintf(szUrl, "http://localhost:3001/prepare/index.html?chapter_id=%s&chapter_name=%s&creator=%s", strUtf8ChapterGuid.c_str(), strUtf8ChapterName.c_str(), strUtf8UserId.c_str());
...
BOOL CNDCloudLocalQuestionManager::InsertQuestion(tstring strGuid, bool bBasicQuestion, bool bInsertQuestion)
{
	...
	if( !bBasicQuestion )
	{
		...
		TiXmlDocument doc;
		bool res = doc.LoadFile(Str2Utf8(szPath).c_str());
		if( !res )
			return FALSE;
		...
		{
			pPageElement->SetAttribute("id", Str2Ansi(strGuid).c_str());
			pPageElement->SetAttribute("name", Str2Ansi(strGuid).c_str());
			pPageElement->SetAttribute("reportable", "true");
			pPageElement = pPageElement->NextSiblingElement();
		}

		doc.SaveFile(Str2Utf8(szPath).c_str()); 
	}
...
BOOL CNDCloudLocalQuestionManager::UploadQuestion(tstring strTitle,tstring strGuid,bool bBasicQuestion,CHttpDelegateBase &OnCompleteDelegate,void* pUserData)
{
	...
	if( dwUserId != 0 )
	{
		...
		bool fileExist = true;
		//
		tstring strQuestionFolder = GetLocalPath() + _T("\\Package\\nodejs\\app\\userdatas\\edu\\esp\\")+(bBasicQuestion? _T("questions"): _T("interaction"))+ _T("\\") +strGuid;
		strQuestionFolder+= _T(".pkg");
		...
		{
			isCloud = true;
			strQuestionFolder = NDCloudFileManager::GetInstance()->GetNDCloudDirectory()+ _T("\\question\\") + strGuid + _T("_default");
			_stprintf_s(szMetaFilePath, _T("%s\\metadata.json"), strQuestionFolder.c_str());
			WIN32_FIND_DATA FindFileData;
			HANDLE hFile = ::FindFirstFile(szMetaFilePath, &FindFileData);
...
BOOL CNDCloudLocalQuestionManager::UpdateQuestion(tstring strGuid, bool bBasicQuestion)
{
	...
	stream.WriteString(szPath);
	stream.ResetCursor();
	BroadcastEvent(EVT_LOCALRES_UPDATE_FILE, &stream);
...
BOOL CNDCloudLocalQuestionManager::EditQuestion(tstring strGuid, tstring strQuestionType /* = _T */, tstring strQuestionPath /*= _T("")*/)
{
	...
	if ( pCoursePlayUI != NULL )
	{
		...
		if( strQuestionPath.find(strNDCloudPath) != -1 )
		{
			...
			_stprintf_s(szUrl, _T("http://localhost:3001/prepare/edit.html?id=%s&question_type=%s&question_base=%s&chapter_name=%s"), strGuid.c_str(), strQuestionType.c_str(), strQuestionBase.c_str(), strChapterName.c_str());
		}
		else
			_stprintf_s(szUrl, _T("http://localhost:3001/prepare/edit.html?id=%s&question_type=%s&chapter_name=%s"), strGuid.c_str(), strQuestionType.c_str(), strChapterName.c_str());
		
		pCoursePlayUI->Init(Str2Unicode(szUrl), COURSEPLAY_EDITOR);
...
BOOL CNDCloudLocalQuestionManager::GetQuestionCodeInfo( tstring strJosnPath, tstring& strGuid, tstring& strQuestionType,tstring& strQuestionCode )
{
	...
	if( !questionGuid.isNull() )
		strGuid	= Utf82Str(questionGuid.asCString());

	if( !questionType.isNull() )
		strQuestionType	= Utf82Str(questionType.asCString());
		if( !rootItem["categories"].isNull() && !rootItem["categories"]["res_type"].isNull() )
		{
			...
			{
				tstring strTaxoncode = Utf82Str(resTypeItems[i]["taxoncode"].asCString());
