...
// get http data
// 
void CHttpDownloadTask::GetHttpData(tstring host, tstring url, int nPort, tstring method, string post)
{
	...
	hRequest = ::HttpOpenRequestA(hConnection, Str2Utf8(method).c_str(), Str2Utf8(url).c_str(), 0, 0, 0, dwFlags, 0);
	...
	if( nPort == INTERNET_DEFAULT_HTTPS_PORT )
	{
		...
		DWORD dwFlagSize = sizeof(dwSecurityFlags);

		InternetQueryOption(hRequest, INTERNET_OPTION_SECURITY_FLAGS, (LPVOID)&dwSecurityFlags, &dwFlagSize);
	
		dwSecurityFlags |= SECURITY_FLAG_IGNORE_REVOCATION;
		InternetSetOption(hRequest, INTERNET_OPTION_SECURITY_FLAGS, &dwSecurityFlags, dwFlagSize);
	}
	...
	if( !res )
	{
		WRITE_LOG_LOCAL(_T("GetHttpData->HttpSendRequest errorcode= 3,GetLastError:%d"),GetLastError());
		dwErrorCode = 3;
		goto _OUT;
	}
...
// get http file
// 
void CHttpDownloadTask::GetHttpFile(tstring host, tstring url, int nPort, tstring method, string post)
{
	HINTERNET hInternet		= NULL;
	HINTERNET hConnection	= NULL;
	HINTERNET hRequest		= NULL;
	...
	hRequest = HttpOpenRequestA(hConnection, Str2Utf8(method).c_str(), Str2Utf8(url).c_str(), 0, 0, 0, INTERNET_FLAG_KEEP_CONNECTION|INTERNET_FLAG_DONT_CACHE, 0);
	if(!hRequest)
	{
		dwErrorCode = 2;
		goto _OUT;
	...
	if( !res )
	{
		WRITE_LOG_LOCAL(_T("GetHttpFile->HttpSendRequest errorcode= 3,GetLastError:%d"),GetLastError());
		dwErrorCode = 3;
		goto _OUT;
	}
