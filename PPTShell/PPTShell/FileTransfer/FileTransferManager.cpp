...
// send file
//
BOOL CFileTransferManager::SendFile(TCHAR *szFilePath, SOCKET receiver, int nConnectType, int nFileType /*= 0*/, int iFileID/*=0*/)
{
...
// recv file
//
BOOL CFileTransferManager::RecvFile(TCHAR* szFilePath, DWORD dwFileSize, DWORD dwFileCrc, int nRecvStep, char* pFileData /* = NULL */, int nDataSize /* = 0 */)
{
...
BOOL CFileTransferManager::CheckFileIsExists(tstring& strFilePath, DWORD dwRecvFileCRC)
{
	// 1.检查文件是否存在；2.检查文件的CRC是否一致
	...
	dwLocalFileCRC = CalcFileCRC(strFilePath.c_str());

	if(dwLocalFileCRC == dwRecvFileCRC)
		bFileExists = TRUE;
	//else if(dwLocalFileCRC == 0)
	//	bFileExists = FALSE;
	else
		bFileExists = FALSE;
