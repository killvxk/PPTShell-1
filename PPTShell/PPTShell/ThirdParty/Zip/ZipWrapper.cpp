...
//		err = getFileCrc(filenameinzip,buf,size_buf,&crcFile);
	...
	char szDestDrive[8]			= {0};
	char szDestDir[MAX_PATH * 4]		= {0};
	char szDestFileName[MAX_PATH]	= {0};
	char szDestExt[8]				= {0};
	if (_splitpath_s(filename, szDestDrive, szDestDir, szDestFileName, szDestExt))
	{
		return false;
	}
...
bool CZipper::ZipFolder(const char* szFilePath, bool ignoreself)
{
	...
	_splitpath_s(szFilePath, szDrive, szFolder, szName, szExt);
	strcat(szName, szExt);
