...
// true-包含空白部分，计算坐标点在屏幕的百分比；false-减去空白部分的宽/高，计算坐标点在PPT页面的百分比
POINTF CPPTController::ScreenPixelToSlidePos(DWORD dwXPos, DWORD dwYPos, bool bIncludeBlank)
{
	float fXPos, fYPos;
	...
	if(bIncludeBlank)
	{
		fXPos = 1.0*dwXPos / nScreenWidth;
		fYPos = 1.0*dwYPos / nScreenHight;
	}
	else
	{
		int nSlideWidth = 0;
		...
		//float fZoom = min(fPerW, fPerH);
		//int nActWidth = fZoom*nScreenWidth;
		//int nActHeight = fZoom* nScreenHight;

		if (fPerW<fPerH)
		{
			int nActHeight = fPerW*nSlideHeight;
			int nSlideTop = (nScreenHight-nActHeight)/2;
			if(dwYPos < nSlideTop)
				fYPos = 0.0f;
			else
				fYPos = (dwYPos - nSlideTop)*1.0f/nActHeight;

			fXPos = 1.0*dwXPos / nScreenWidth;
		}
		else
		{
			int nActWidth = fPerH*nSlideWidth;
			int nSlideLeft = (nScreenWidth-nActWidth)/2;
			if(dwXPos < nSlideLeft)
				fXPos = 0.0f;
			else
				fXPos = (dwXPos - nSlideLeft)*1.0f/nActWidth;

			fYPos = 1.0*dwYPos / nScreenHight;
		}
	}

	POINTF pointf = {fXPos, fYPos};
	return pointf;
}
...
BOOL CPPTController::Open(LPCTSTR lpszPathName, BOOL bReadOnly)
{
	...
	for(int i = 1 ; i <= presentations->GetCount(); i++)
	{
		pPre = presentations->Item(_variant_t((LONG)i));
		if ( pPre != NULL )
		{
			tstring strFullName = pPre->GetFullName();
			WRITE_LOG_LOCAL(_T("File exsit Name :%s"), strFullName.c_str());
			if(strFullName.compare(lpszPathName) == 0)
			{
				/*HWND hWnd =  HWND(pPre->GetApplication()->GetHWND());
				UpdatePPTHwnd(hWnd);
				SetForegroundWindow(hWnd);*/
				CToast::Toast(_T("文件已打开"));
				return TRUE;
			}
		}
	}
...
BOOL CPPTController::Close(BOOL bBefore/* = TRUE*/)
{
	...
	while (  pPresentation != NULL )
	{
		pPresentation->Close();
		bClose = TRUE;

		pPresentation = GetPresentation(bBefore);
	}
...
BOOL CPPTController::InsertPalceHolder(LPCTSTR szPath, int& nSlideId, int& nPalceHolderId, int nLeft, int nTop, int nWidth, int nHeight, int nSlideIdx /*=0*/ )
{
	//为0页时，新建一页
	SlidesPtr slidesPtr = GetSlides();
	_SlidePtr pSlide	= NULL;
	if (slidesPtr && slidesPtr->GetCount() == 0)
	{
		...
		InsertPicture(szPath, nSlideId, nPalceHolderId, nLeft, nTop, nWidth, nHeight, nSlideIdx);
...
BOOL CPPTController::ExportToImages(const TCHAR* szFolderPath, const TCHAR* szExt, int nStartSlideIdx /*= -1*/, int nEndSlideIdx /*= -1*/, int nImageWidth /*= 0*/, int nImageHeight /*= 0*/, bool* pShouldCancel /*= NULL*/ )
{
	//BOOL isChange = IsChanged(FALSE);
	CResGuard::CGuard gd(m_Lock);
...
BOOL CPPTController::ExportToImages( TCHAR* szPPTPath, TCHAR* szFolderPath, TCHAR* szExt, bool* pShouldCancel /*= NULL*/, int* pCount/*= NULL*/, int nStartSlideIdx /*= -1*/, int nEndSlideIdx /*= -1*/, int nExistCount /*= 0*/, int nWidth /*= 0*/, int nHeight /*= 0*/ )
{
	CResGuard::CGuard gd(m_Lock);
...
BOOL CPPTController::ExportBackgroundToImages(TCHAR* szFolderPath, TCHAR* szExt, int nIndex)
{
	CResGuard::CGuard gd(m_Lock);
...
BOOL CPPTController::ExportBackgroundToImages_check(TCHAR* szFilePath, TCHAR* szExt, _SlidePtr pSlide)
{
	BEGIN_CATCH_COM_ERROR
...
BOOL CPPTController::ExportBackgroundToImages(TCHAR* szFilePath, TCHAR* szExt, _SlidePtr pSlide)
{
...
BOOL CPPTController::ExportBackgroundToImages(TCHAR* szPath, TCHAR* szFolderPath, TCHAR* szExt, int nIndex)
{
	CResGuard::CGuard gd(m_Lock);
...
// Get outline
//
BOOL CPPTController::GetOutLine(vector<tstring> &vecOutLine,  tstring strPrefixIfNoExist /*= ""*/)
{
	CResGuard::CGuard gd(m_Lock);
	...
	for(int i = 1; i <= nCount; i++)
	{
		...
		if( strTitle == _T("") )
		{
			if( strPrefixIfNoExist != _T("") )
			{
				TCHAR szTitle[MAX_PATH];
				wsprintf(szTitle, _T("%s %d"), strPrefixIfNoExist.c_str(), i);
...
BOOL CPPTController::IsPPTActive()
{
	if(m_PPTHwnd != NULL)
	{
#ifdef MULTI_VERSION //解决版本2010弹出切换到编辑状态的问题。
		return TRUE;
#endif
...
BOOL CPPTController::PresentationToImage(_PresentationPtr Pres, TCHAR* szDir)
{
	CResGuard::CGuard gd(m_Lock);
	BEGIN_CATCH_COM_ERROR
...
void CPPTController::OnSize( UINT nType, int cx, int cy )
{
	...
	if (m_PPTHwnd == NULL )
		return;
	if (nType == SIZE_MAXIMIZED && GetPPTVersion() == PPTVER_2007)
	{
		::MoveWindow(m_PPTHwnd, 3, -GetOffsetWithForbidenDrag(), cx, cy + GetOffsetWithForbidenDrag(), TRUE);
	}else
...
// Pack
//
DWORD CPPTController::PackAllResource(HWND hwnd, void* pNotify, TCHAR* szPath)
{
	WRITE_LOG_LOCAL(_T("----------------------PackAllResource start"));
	...
	TCHAR szPPTZipDir[MAX_PATH * 4];
	//创建临时目录
	//////////////////////////////////////////////////////////////////////////
	_sntprintf_s(szPPTZipDir, _countof(szPPTZipDir) - 1,  _T("%s%s%s_打包文件"),szSrcDrive, szSrcDir, szSrcFileName);
	DWORD dwCRC = CalcFileCRC(szPath);
	tstring strNdpFilePath = NDCloudFileManager::GetInstance()->GetNDCloudDirectory();
	_sntprintf_s(szPackDir, _countof(szPackDir) - 1,  _T("%s\\PPTTemp\\%x"),strNdpFilePath.c_str(), dwCRC);
	...
	VARIANT_BOOL bFinal = pPre->GetFinal();

	if ( bFinal == VARIANT_TRUE )
		pPre->PutFinal(VARIANT_FALSE);
...
//	CDandcef* pDandCef = new CDandcef;
	...
	for(int i = 1; i <= nSlidesCount && !pPackNotify->bCancel; i++)
	{
		...
		for (int j = 1; j <= nShapesCount; j++)
		{
			...
			if (_tcsstr(szName, _T("WindowsMediaPlayer")) )
			{
				...
				tregex szUrlRule(_T("^(http[s]{0,1})://[a-zA-Z0-9_-]+$"));
				tsmatch sResult;
				if(regex_search(strUrl, sResult, szUrlRule))
				{
					Player.DetachDispatch();
					//url
					continue;
				...
				else if (!_tsplitpath_s(strUrl.c_str(), szResDrive, szResDir, szResFileName, szResExt))
				{
					...
					{
						//绝对路径
						_sntprintf_s(szPackRelativePath, _countof(szPackRelativePath) - 1,  _T("Video\\Slider%d\\%s%s"), i, szResFileName, szResExt);
						_sntprintf_s(szPackDestFile, _countof(szPackDestFile) - 1,  _T("%s\\%s"),szPackDir, szPackRelativePath);
						//拷贝视频和音频文件
					...
					else
					{
						...
						if (_tcsnicmp(szResDir, _T("\\\\"), 2) == 0 ) // 共享盘
							_sntprintf_s(szPackSrcFile, _countof(szPackSrcFile) - 1,  _T("%s"), strUrl.c_str());
						else
							_sntprintf_s(szPackSrcFile, _countof(szPackSrcFile) - 1,  _T("%s%s%s"), szSrcDrive, szSrcDir, strUrl.c_str());
						//拷贝视频和音频文件
						createDirWithFullPath(szPackDestFile);
			...
			else if (_tcsstr(szName, _T("andcef")) )//习题
			{
				...
				if(regex_search(strPath, sResult, szUrlRule))
				{
					//url
					DandCef.DetachDispatch();
					continue;
				...
				else if (!_tsplitpath_s(strPath.c_str(), szResDrive, szResDir, szResFileName, szResExt))
				{
					...
					{
						//绝对路径
						_sntprintf_s(szPackRelativePath, _countof(szPackRelativePath) - 1,  _T("Question\\%s\\"), cstrId.GetString());
						_sntprintf_s(szPackDestPath, _countof(szPackDestPath) - 1,  _T("%s\\%s"),szPackDir, szPackRelativePath);
						_sntprintf_s(szPackSrcPath, _countof(szPackSrcPath) - 1,  _T("%s%s%s\\"), szResDrive, szResDir, cstrId.GetString());
					...
					else
					{
						...
						_sntprintf_s(szPackRelativePath, _countof(szPackRelativePath) - 1,  _T("Question\\%s\\"), cstrId.GetString());
						_sntprintf_s(szPackDestPath, _countof(szPackDestPath) - 1,  _T("%s\\%s"), szPackDir, szPackRelativePath);
						_sntprintf_s(szPackSrcPath, _countof(szPackSrcPath) - 1,  _T("%s%s%s%s\\"), szSrcDrive, szSrcDir, szResDir, cstrId.GetString());
	...
	//pPre->PutSaved(Office::msoTrue);

	//if ( bFinal == VARIANT_TRUE )
	//	pPre->PutFinal(VARIANT_TRUE);
	...
	::SendMessage(hwnd, WM_USER_PACK_PPT, (WPARAM)pPackNotify, (LPARAM)NULL);
	if(ZipPackDir(Str2Ansi(szPackDir)) == FALSE)
	{
		WRITE_LOG_LOCAL(_T("ZipPackDir Fail, PackDir:%s, error:%d"), szPackDir, GetLastError());
		return 7;
	}
	//
	...
	tstring strNdPPTFile = szPPTZipDir;
	...
	{
		WRITE_LOG_LOCAL(_T("MoveFileEx Fail, PPTZipFile:%s, NdPPTFile,%s, error:%d"), strPPTZipFile.c_str(), strNdPPTFile.c_str(), GetLastError());
		return 8;
	}
	//
...
// UnPack
//
DWORD CPPTController::UnPackAllResource(HWND hwnd, void* pNotify, TCHAR* szPath)
{
	WRITE_LOG_LOCAL(_T("----------------------UnPackAllResource start"));
	...
	if(UnZipPackDir(Str2Ansi(szPath), Str2Ansi(szUnPackDir)) == FALSE)
	{
...
// 	tstring strConfigPath = szUnPackDir;
// 	strConfigPath += _T("\\config.ini");
// 	WritePrivateProfileString(_T("Config"), _T("Status"), _T("1"), strConfigPath.c_str());
	...
	if(hFind != INVALID_HANDLE_VALUE)
	{
		do 
		{
			...
			if(strFileName.find(_T("~$")) == tstring::npos && strFileName.find(_T(".ppt")) != tstring::npos )
			{
				...
				//特殊处理 .pptxx或者.pptt
				TCHAR szNewFileDrive[8]			= {0};
				TCHAR szNewFileDir[MAX_PATH * 4]		= {0};
				TCHAR szNewFileFileName[MAX_PATH]	= {0};
				TCHAR szNewFileExt[8]				= {0};

				_tsplitpath_s(strUnpackFile.c_str(), szNewFileDrive, szNewFileDir, szNewFileFileName, szNewFileExt);
				if(_tcsicmp(szNewFileExt , _T(".pptxx")) == 0 || _tcsicmp(szNewFileExt , _T(".pptt")) == 0 )
				{
					tstring strNewUnpackFile = strUnpackFile;
					TCHAR * pPath = (TCHAR*)strNewUnpackFile.c_str();
					pPath[strNewUnpackFile.length() - 1] = 0;

					WRITE_LOG_LOCAL(_T("MoveFileEx SrcFile:%s, DstFile:%s"), strUnpackFile.c_str(), strNewUnpackFile.c_str());
					if(MoveFileEx(strUnpackFile.c_str(), strNewUnpackFile.c_str(), MOVEFILE_COPY_ALLOWED|MOVEFILE_REPLACE_EXISTING))
					{
						WRITE_LOG_LOCAL(_T("MoveFileEx success"));
						strUnpackFile = strNewUnpackFile;
					}
				}
				break;
			}
...
// 			pPre->PutFinal(VARIANT_FALSE);
	...
	int nSlidesCount = pSlides->GetCount();
	WRITE_LOG_LOCAL(_T("GetSlides success Count:%d"), nSlidesCount);
	...
	for(int i = 1; i <= nSlidesCount && !pPackNotify->bCancel; i++)
	{
		...
		for (int j = 1; j <= nShapesCount; j++)
		{
			...
			{
				WRITE_LOG_LOCAL(_T("Find Mediaplayer name:%s,SliderNumber:%d"), strName.c_str(), j);

				OLEFormatPtr pOle = pShape->GetOLEFormat();
				IDispatchPtr pObj = pOle->Object;

				Player.AttachDispatch(pObj, FALSE);
				WRITE_LOG_LOCAL(_T("AttachDispatch success"));

				tstring strUrl = Player.get_URL();
				WRITE_LOG_LOCAL(_T("GetUrl %s"), strUrl.c_str());
				...
				else if (!_tsplitpath_s(strUrl.c_str(), szResDrive, szResDir, szResFileName, szResExt))
				{
					...
					{
						//相对路径
						_sntprintf_s(szPackAbsolutePath, _countof(szPackAbsolutePath) - 1,  _T("%s\\Video\\Slider%d\\%s%s"),szUnPackDir, i, szResFileName, szResExt);
			...
			{
				WRITE_LOG_LOCAL(_T("Find CEF name:%s,SliderNumber:%d"), strName.c_str(), j);
				...
				CString cstrUrl = DandCef.GetUrl();
				WRITE_LOG_LOCAL(_T("GetUrl %s"), cstrUrl.GetString());
				...
				{
					CString cstrPackAbsolutePath ;
					if(_tcslen(szResDrive) == 0)
					{
						//相对路径
						_sntprintf_s(szPackAbsolutePath, _countof(szPackAbsolutePath) - 1,  _T("%s\\Question\\%s\\"), szUnPackDir, cstrId.GetString());
						cstrPackAbsolutePath = szPackAbsolutePath;
					}
					else
					{
						cstrPackAbsolutePath = strPath.c_str();
						cstrPackAbsolutePath += _T("\\");
					}
					//修改配置
					...
					CString cstrImagePath = cstrPackAbsolutePath;
...
// 		pPre->PutFinal(VARIANT_TRUE);
	...
	//WritePrivateProfileString(_T("Config"), _T("Status"), _T("2"), strConfigPath.c_str());
...
tstring CPPTController::GetPPTPath()
{
	if (!m_pApplication)
		return _T("");

	tstring strPath = m_pApplication->GetPath();
	strPath += _T("\\");
...
void CPPTController::CheckPPTRunTime()
{
	...
	if( hr != S_OK )
	{
		WRITE_LOG_LOCAL(_T("_ApplicationPtr.CreateInstance fail. HRESULT:%x"), (unsigned long int)hr);
		// 提示用户修复
...
int CPPTController::GetCurrentShowPosition()
{
	...
	if( pSlideShowView == NULL )
		return FALSE;
	nShowPage = pSlideShowView->GetCurrentShowPosition();
	return nShowPage;
