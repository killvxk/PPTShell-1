...
void CPPTEventHandler::PresentationClose( LPDISPATCH Pres )
{
	...
	//BroadcastPostedEvent(EVT_PPT_FILE_OPERATION, CLOSE_FILE, 0, 0);
}
...
{	
			...
			if ( !strPath.empty() )
			{
				...
				if ( pTags != NULL )
				{
					...
					if( _tcsicmp(strInfo.c_str(), _T("ExportToImages"))!= 0 )
					{
						...
						tstring strCurPath	= _T("");
				
						//get current Pres Info
						_ApplicationPtr pApp		= ((_PresentationPtr)Pres)->GetApplication();
						_PresentationPtr pCurPres	= pApp->GetActivePresentation();

						if ( pCurPres != NULL )
						{
							strCurPath = pCurPres->GetFullName();
						}

						BOOL bSyncSlideImage = (strFullPath == strCurPath);

						CStream* pStream	= new CStream(256);
						pStream->WriteString(strFullPath);
						pStream->WriteBOOL(bSyncSlideImage);
	...
	{
		WRITE_LOG_DEBUG(_T("PresentationSave error"));
	}
}
...
void CPPTEventHandler::PresentationOpen( LPDISPATCH Pres )
{
	try
	{
		...
		if ( m_PPTHwnd == (HWND)AppPtr->GetHWND() && 
			strName.length() != 0 && strName.find(_T(" [自动保存]")) == 0 &&
	...
	{
		m_bNewPPT	= FALSE;
		WRITE_LOG_DEBUG(_T("PresentationOpen error"));
	}
}
...
void CPPTEventHandler::AfterPresentationOpen( LPDISPATCH Pres )
{
	...
	{
		WRITE_LOG_DEBUG(_T("AfterPresentationOpen error"));
	}
	m_bNewPPT	= FALSE;
}
...
void CPPTEventHandler::NewPresentation( LPDISPATCH Pres )
{
	...
	{
		m_bNewPPT	= FALSE;
		WRITE_LOG_DEBUG(_T("NewPresentation error"));
	}
}
...
void CPPTEventHandler::WindowActivate( LPDISPATCH Pres,LPDISPATCH Wn )
{
	...
	try
	{
		...
		if ( !(strName == _T("相册") && strPath.empty()) )
		{
			...
			BroadcastPostedEvent(EVT_PPT_FILE_OPERATION, WINDOW_ACTIVATE, NULL, 0);
			return;
		}
	...
	{
		WRITE_LOG_DEBUG(_T("WindowActivate error"));
	}
...
void CPPTEventHandler::SlideShowBegin( LPDISPATCH Wn )
{
	...
	{
		WRITE_LOG_DEBUG(_T("SlideShowBegin error"));
	}
}
...
void CPPTEventHandler::SlideShowNextSlide( LPDISPATCH Wn )
{
	try
	{
		...
		tstring str3DPath = _T("");
		try
		{
			str3DPath = Get3DPath( (LPDISPATCH)pSlide );
	...
	{
		WRITE_LOG_DEBUG(_T("SlideShowNextSlide error"));
	}
}
...
void CPPTEventHandler::SlideSelectionChanged( LPDISPATCH SldRange )
{
	try
	{
		...
		WRITE_LOG_LOCAL(_T("CPPTEventHandler::SlideSelectionChanged, slideid:%d"), iSlideIdx);
		BroadcastPostedEvent(EVT_PPT_FILE_OPERATION, SLIDE_CHANGED, iSlideIdx, 0);
		//SLIDE_CHANGED
	}
	catch (...)
	{
		WRITE_LOG_DEBUG(_T("SlideSelectionChanged error"));
	}
}
...
tstring CPPTEventHandler::GenerateGuid()
{
	...
	_stprintf_s(szGuid, _T("PPT%d_%d"), i++, GetTickCount());
	return szGuid;
}
...
// 
DWORD WINAPI CPPTEventHandler::DelaySendWindowActiveThread(LPVOID lParam)
{
	CPPTEventHandler* pThis = (CPPTEventHandler*)lParam;
	pThis->DelaySendWindowActiveMsg();
...
void CPPTEventHandler::DelaySendWindowActiveMsg()
{
