...
void CPPTInfoSender::ExportThumbnail()
{
	...
	while( TRUE )
	{
		...
		for (; itrPage!=setAllPagesRqst.end(); itrPage++)
		{
			...
			{
				pController->ExportToImages(szFolderPath, _T("png"), iBeginIdx, iLastIdx);	
				iBeginIdx = iCurrIdx;
				iLastIdx = iCurrIdx;
			}
		}
...
void CPPTInfoSender::UpdateSlideThumbToAllUser(const tstring &strPPTFileName, int iSlideCnt)
{
	CUserManager* pUserManager = UserManager::GetInstance();
	map<DWORD, CUser*> mapUsers = pUserManager->GetUsers();
	...
	for(itr = mapUsers.begin(); itr != mapUsers.end(); itr++)
	{
		...
		for (;itrPage!=setSlidesOfUser.end(); itrPage++)
		{
			...
			{
				WRITE_LOG_LOCAL(_T("CPPTInfoSender::UpdateSlideThumbToAllUser, have not gen new slide images' crccode! impossible to get here!!"));
				return ;
			}
			...
			{
				WRITE_LOG_LOCAL(_T("Increment update, send new image, newid:%d"), iIdx);
				SendSlideUpdateNew(dSocket, dConnType, iIdx, 0);
//				SendSlideThumbOfFile(dSocket, dConnType, strPPTFileName, iIdx);
			}
			else
			{
				WRITE_LOG_LOCAL(_T("Increment update, send update image, newid:%d, oldid:%d"), iIdx, itrOldCrcToFile->second);
...
bool CPPTInfoSender::OnSyncPPTInfo(void *param)
{
	...
	case PPT_EDIT_NEWFILE:
		{
			WRITE_LOG_LOCAL(_T("OnSyncPPTInfo -- Syn PPT Thumbnails"));
			if ( m_bImageSynchronizing ) 
				break;
			...
			if ( pController != NULL )
			{
				...
				if ( nSlideIndex >= 0 )
					...
					WRITE_LOG_LOCAL(_T("PPT_EDIT_NEWFILE SlideIndex Error, SlideIndex=%d"), nSlideIndex);
			}
			
		}
		break;
	case PPT_EDIT_CLOSEFILE:
		{
			CPacketStream packetCloseFile;
			packetCloseFile.PackThreeCommand(ACTION_PUSH_DATA, PUSH_P2M_PPTCTL, PPTSTATE_P2M_CLOSE);
			SendCommandToAllUsers(packetCloseFile);
		}
		break;
	...
	case PPT_PLAY_END:
		{
			...
			{
				// PPT2007，手机端点击下一页从黑幕退出，有时候不会响应SlideShowNextSlide。要发送当前页号给手机端
				int iSlideIdx = 0;
				iSlideIdx = pController->GetActiveSlideIndex();
...
BOOL CPPTInfoSender::SendPPTOutLine(SOCKET sock, int nConnectType)
{
	...
	vector<tstring> vecOutline;
	vecOutline.clear();
	pController->GetOutLine(vecOutline, _T("幻灯片"));
...
BOOL CPPTInfoSender::SendSlideThumbList(SOCKET sock, int nConnectType, const vector<int> &vecSlideIdxs)
{
	...
	for(unsigned int i = 0; i <vecSlideIdxs.size(); i++)
	{
		...
		{
			pController->ExportToImages(m_strPPTFilePath.c_str(), _T("png"), iIdx, iIdx);
			TCHAR szPath[MAX_PATH];
...
BOOL CPPTInfoSender::SendSlideThumbOfFile(SOCKET sock, int nConnectType, const tstring& strPPTFileName, int iSlideIdx)
{
	static tstring strPath = GetLocalPath();
	TCHAR szPath[MAX_PATH];
	...
	WRITE_LOG_LOCAL(_T("Socket:%d, send slide image, index:%d"), sock, iSlideIdx);
	return TRUE;
}
...
BOOL CPPTInfoSender::SendPlayBegin(SOCKET sock, int nConnectType)
{
	...
	WRITE_LOG_LOCAL(_T("----SendPlayBegin to user:%d"), sock);
	return pApp->SendStreamPacket(sock, &PPTInfoStream, nConnectType);
}
...
BOOL CPPTInfoSender::SendSlideUpdateStart(SOCKET sock, int nConnectType)
{
	...
	WRITE_LOG_LOCAL(_T("socket:%d, Increment update, send begin"), sock);
}
...
BOOL CPPTInfoSender::SendSlideUpdatePageCnt(SOCKET sock, int nConnectType, int iPageCnt)
{
	...
	return pApp->SendStreamPacket(sock, &pkgStream, nConnectType);
	WRITE_LOG_LOCAL(_T("socket:%d, Increment update, send page count:%d"), sock, iPageCnt);
}
...
BOOL CPPTInfoSender::SendSlideUpdateNew(SOCKET sock, int nConnectType, int iNewID, int iOldID)
{
	...
	return pApp->SendStreamPacket(sock, &pkgStream, nConnectType);
	WRITE_LOG_LOCAL(_T("socket:%d, Increment update, send replace image, newid:%d, oldid:%d"), sock, iNewID, iOldID);
}
...
BOOL CPPTInfoSender::SendSLideUpdateEnd(SOCKET sock, int nConnectType)
{
	...
	return pApp->SendStreamPacket(sock, &pkgStream, nConnectType);
	WRITE_LOG_LOCAL(_T("socket:%d, Increment update, send end"), sock);
}
...
BOOL CPPTInfoSender::SendPlayBeginToAllUsers()
{
	...
	for(itr = mapUsers.begin(); itr != mapUsers.end(); itr++)
	{
		...
		WRITE_LOG_LOCAL(_T("CPPTInfoSender::SendPlayBeginToAllUsers"));
...
// 发送CPacketStream给所有连接用户
BOOL CPPTInfoSender::SendCommandToAllUsers(CPacketStream& packetStream)
{
	CPPTShellApp* pApp = (CPPTShellApp*)AfxGetApp();
	...
	for(itr = mapUsers.begin(); itr != mapUsers.end(); itr++)
	{
		...
		pApp->SendStreamPacket(pUser->GetSocket(), &packetStream, pUser->GetConnectType());
	}
