...
public:
	...
	BOOL					InsertPalceHolder(LPCTSTR szPath, int& nSlideId, int& nPalceHolderId, int nLeft = -1, int nTop = -1, int nWidth = -1, int nHeight = -1, int nSlideIdx = 0);
	...
	BOOL					PresentationToImage(_PresentationPtr Pres, TCHAR* szDir);
	BOOL					ExportToImages(const TCHAR* szFolderPath, const TCHAR* szExt, int nStartSlideIdx = -1, int nEndSlideIdx = -1, int nImageWidth = 0, int nImageHeight = 0, bool* pShouldCancel = NULL);
	BOOL					ExportToImages(TCHAR* szPPTPath, TCHAR* szFolderPath, TCHAR* szExt, bool* pShouldCancel = NULL, int* pCount= NULL, int nStartSlideIdx = -1, int nEndSlideIdx = -1, int nExistCount = 0, int nWidth = 0, int nHeight = 0);
	BOOL					ExportBackgroundToImages(TCHAR* szFolderPath, TCHAR* szExt, int nIndex);
	BOOL					ExportBackgroundToImages(TCHAR* szPath, TCHAR* szFolderPath, TCHAR* szExt, int nIndex);

	BOOL					ExportBackgroundToImages(TCHAR* szFolderPath, TCHAR* szExt, _SlidePtr pSlide);
	BOOL					ExportBackgroundToImages_check(TCHAR* szFolderPath, TCHAR* szExt, _SlidePtr pSlide);//核对是否有背景
	// Pack
	DWORD					PackAllResource(HWND hwnd, void* pNotify, TCHAR* szPath);
	DWORD					UnPackAllResource(HWND hwnd, void* pNotify, TCHAR* szPath);
	//

	// Others
	int						GetSlideCount();
	string					GetPresentationName();
	BOOL					GetOutLine(vector<tstring>& vecOutLine, tstring strPrefixIfNoExist = _T(""));
	POINT					SlidePosToScreenPixel(POINTF pt);
	void					SlidePosToScreenPos(float &fWidth, float &fHeight);
	POINTF					ScreenPixelToSlidePos(DWORD dwXPos, DWORD dwYPos, bool bIncludeBlank=false);
