...
// 	FILE * fp = _tfopen(strPath.c_str(),_T("r"));
// 	if(fp)
// 	{
// 		//fscanf(fp, _T("%s\n"),szRes);
// 		fread(szRes, sizeof(szRes) - 1, 1, fp);
// 		fclose(fp);
// 	}
...
bool CUpdate::OnGetNewestVersionUrl(void * pParam)
{
	...
	m_strServerVersion = Ansi2Str(pNotify->pData);

	tstring strPath = GetLocalPath();
	strPath += _T("\\version.dat");

	char szLocalVersion[32] = "0.0.0.0";
	FILE * fp = _tfopen(strPath.c_str(),_T("r"));
	if(fp)
	{
		//fscanf(fp, _T("%s\n"),szLocalVersion);
		fread(szLocalVersion, sizeof(szLocalVersion) - 1, 1, fp);
		fclose(fp);
	}

	_VersionStruct versionStruct;
	_VersionStruct serverVersionStruct;
	if(!versionStruct.InitFromString(Ansi2Str(szLocalVersion).c_str()))
	{
...
bool CUpdate::OnGetUpdateLog(void * pParam)
{
	...
	pNotify->pData[pNotify->nDataSize] = 0;
	m_strUpdateLog = Ansi2Str(pNotify->pData);
