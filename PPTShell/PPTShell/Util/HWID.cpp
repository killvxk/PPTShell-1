...
bool cHWID::IsPhysicalLocalMac( char* szDesc )
{
	...
	strKey += szTempConn;
	LONG nRet = ::RegOpenKeyExA( HKEY_LOCAL_MACHINE, strKey.c_str(), 0, KEY_READ|KEY_WOW64_64KEY, &hKEY );
	...
	nRet = ::RegQueryValueExA( hKEY, szTempPn, NULL, &dwType, (LPBYTE)szData, &dwDataSize );
	::RegCloseKey(hKEY);
	if ( nRet != ERROR_SUCCESS )          
		return false;
...
BOOL cHWID::WinNTHDSerialNumAsPhysicalRead( BYTE* dwSerial, UINT* puSerialLen, UINT uMaxSerialLen )
{
	...
	for( UINT uDrive = 0; uDrive < 4; ++ uDrive )
	{
		...
		hPhysicalDriveIOCTL = CreateFileA( szDriveName, GENERIC_READ | GENERIC_WRITE, 
...
int cHWID::obtainBiosVersion(BYTE* pOutBuf)
{
	...
	LONG ret = RegOpenKeyExA(HKEY_LOCAL_MACHINE, szTemsy+2, 0, wd, &hKey);
	if( ret != ERROR_SUCCESS )
		return 0;

	DWORD dwSize = 0;
	char szSysB[]="SystemBiosVersion";
	ret = RegQueryValueExA(hKey, szSysB, 0, 0, 0, &dwSize);
	if( ret != ERROR_SUCCESS )
		return 0;
	...
	ret = RegQueryValueExA(hKey, szSysB, 0, 0, pData, &dwSize);
	if( ret != ERROR_SUCCESS )
	{
		delete pData;
		return 0;
...
int cHWID::obtainProductID(BYTE* pOutBuf)
{
	...
	if( bVistaWin7 )
		ret = RegOpenKeyExA(HKEY_LOCAL_MACHINE, szT1+2, 0, KEY_QUERY_VALUE | KEY_WOW64_64KEY/*0x20019*/, &hKey);
	else
		ret = RegOpenKeyExA(HKEY_LOCAL_MACHINE, szT2+2, 0, KEY_QUERY_VALUE | KEY_WOW64_64KEY/*0x20019*/, &hKey);
	...
	char szTp[]="ProductId";
	DWORD dwSize = 0;
	ret = RegQueryValueExA(hKey, szTp, 0, 0, NULL, &dwSize);
	if( ret != ERROR_SUCCESS )
		return 0;
	...
	ret = RegQueryValueExA(hKey, szTp, 0, 0, pData, &dwSize);
	if( ret != ERROR_SUCCESS )
	{
		delete pData;
		return 0;
