...
#define ICR_BUTTON_ID_ASSIGN_HOMEWORK        5               //布置作业按钮ID
#define ICR_BUTTON_ID_INTERACTIVE_TOOL           6               //互动工具按钮ID
#define ICR_BUTTON_ID_RACE_ANSWER                  7            //抢答按钮ID
...
@param szEventName 事件名称，\0结尾，utf-8编码
@param szEventData 事件数据，\0结尾，utf-8编码
@return 成功或失败
*/
typedef bool(__stdcall *LPFN_IcrSendExamEventCallbackPtr)(const char* szEventName, const char* szEventData);

/**
@name IcrGetTopLevelWindowHandle pptshell主窗口
@return pptshell主窗口
*/
...
@param szText 消息内容，\0结尾，utf-8编码
@param szCaption 标题，\0结尾，utf-8编码
...
@param szText 消息内容，实现必须拷贝一份消息内容，\0结尾，utf-8编码
...
@return 习题类型，不再使用时调IcrRelease回调释放，NULL表示不是习题，\0结尾，utf-8编码
*/
...
@param szParamJson start所需参数JSON串格式，\0结尾，utf-8编码：
    exam: 所有习题完整路径，习题包main.xml所在的文件夹
    is_recovery: 标识是否恢复，true或false，当程序异常退出后重新启动，教师选择恢复到之前状态时，设置为true
    slide_pos: 幻灯片位置，花名册显示时需要
    {
    "token":{"access_token":"登录时返回的token", "mac_key":"登录时返回key"},
    "teacher":{"id":"teacher-id", "name" : "nick-name", "isfreemode":1|0},
    "class":{"id":"class-id"},
...
@param ppStudentsJson 学生信息JSON串，\0结尾，utf-8编码，不再使用调IcrRelease释放
...
/**
@name IcrInvokeNativeMethod JS调用native方法，必须在非UI线程中调用避免卡住UI
@param szMethodJson 方法名称及参数JSON串，utf-8格式，JSON格式如下：
    {
    "method_name":"dohandwrite",
    "param": "json-param-string",
    }
@return 无返回值返回{}，utf-8格式，无需使用调用IcrRelease释放
*/
