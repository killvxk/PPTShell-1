...
void CWANThread::DoRecv(CWANClient* pClient, PIOCPOVERLAPPED pIOCPInfo, DWORD dwTransferred)
{
	...
	WRITE_LOG_DEBUG(_T("CWANThread DoRecv, Packet Processed, Start PostRecv"));
...
DWORD WINAPI CWANThread::RecvThread(LPARAM lParam)
{
	...
	while(!pThis->m_bCloseApp)
	{
		...
		WRITE_LOG_DEBUG(_T("CWANThread GetIOCPStatus Return trans=%u,client pointer=%x, &pointer=%x, key=%x, &key=%x\n"), dwTransferred, pClient, &pClient, pCompletionKey, &pCompletionKey);
		...
		if(!bStatusRet)
		{
			...
			WRITE_LOG_LOCAL(_T("CWANThread GetIOCPStatus Error=%u"), dwErr);
			...
			if(dwErr == WAIT_TIMEOUT || dwErr==ERROR_NETNAME_DELETED)
			{
				...
				{
					WRITE_LOG_LOCAL(_T("CWANThread IOCP MainClient Socket Closed"));
					pClient->CloseSocket();
					Sleep(1);
				}
				else
			...
			WRITE_LOG_DEBUG(_T("CWANThread IOCP Client Remove - 1"));	// 排查BUG，以后可取消
			continue;
		}

		if(dwTransferred == 0)
		{
			WRITE_LOG_LOCAL(_T("CWANThread IOCP Transferred Byte is Zero"));
			// 客户端关闭的情况
			pClient->CloseSocket();
			pThis->RemoveClient(pClient);
			WRITE_LOG_DEBUG(_T("CWANThread IOCP Client Remove - 2"));	// 排查BUG，以后可取消
			continue;
		}
		// 处理接收数据
		WRITE_LOG_DEBUG(_T("CWANThread IOCP Start Packet Process"));	// 排查BUG，以后可取消
		PIOCPOVERLAPPED pIOCPInfo = (PIOCPOVERLAPPED)pOverlapped;
		pThis->DoRecv(pClient, pIOCPInfo, dwTransferred);
		WRITE_LOG_DEBUG(_T("CWANThread IOCP Finish Packet Process"));	// 排查BUG，以后可取消
	}
