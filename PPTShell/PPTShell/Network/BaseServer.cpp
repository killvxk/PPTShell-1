...
DWORD WINAPI CBaseServer::IOCPWorkThread(LPARAM lParam)
{
	...
	while( pThis->IsWorking() )
	{
		...
		{
			WRITE_LOG_LOCAL(_T("CBaseServer::IOCPWorkThread, Recieve break signal!!"));
			break;
		}

		if (NumberOfBytesTransfered==0 && lpOverlapped == NULL)
		{
			WRITE_LOG_LOCAL(_T("IOCPWorkThread recieve stop status,[SocketClose:%u]"), sock);
			pThis->OnSocketClosed(sock);
			continue;
		}
		if(!ret)
		{
			WRITE_LOG_LOCAL(_T("GetQueuedCompletionStatus error---sock:%u"), sock);
			pThis->OnSocketClosed(sock);
			continue;
		}
		...
		if (NumberOfBytesTransfered == 0 || lpOverlapped == NULL)
		{
			WRITE_LOG_LOCAL(_T("Error occured!! sock:%u"), sock);
			pThis->OnSocketClosed(sock);
			continue;
		}
