	...
	while( m_nRecvdDataSize >= sizeof(PACKET_HEAD) )
	{
		...
		else
		{
			...
			if (m_bHeartbeatRqst && pPacketHeader->dwMajorType == PT_HEARTBEAT_ACK)
			{
				m_bHeartbeatRqst = false;
				m_iHeartbeatTryCnt = 0;
				WRITE_LOG_LOCAL(_T("receive heartbeat ack!! socket:%d"), m_ClientSocket);
				break;
			}
...
BOOL CWANClient::SendHeartBeat()
{
	...
	WRITE_LOG_LOCAL(_T("Send Heartbeat to socket:%d, try cnt:%d"), m_ClientSocket, m_iHeartbeatTryCnt);
...
BOOL CWANClient::SendData(char* pData, int nSize)
{
	...
	while( curLen < totalLen )
	{
		...
		if( len == 0 ) // close
		{
			WRITE_LOG_LOCAL(_T("CWANClient::SendData, send 0 byte!"));
			return FALSE;
		}
		else if( len == SOCKET_ERROR )
		{
			...
			{
				WRITE_LOG_LOCAL(_T("CWANClient::SendData, get a SOCKET_ERROR, errno:%d!"), dwErrorCode);
				return FALSE;	// close
			}
		}
