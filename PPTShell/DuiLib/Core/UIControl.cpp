...
CControlUI::~CControlUI()
{
	...
	if( m_pManager != NULL ) {
		...
		m_pManager->ReleaseImage(m_diBk.pImageInfo);
		m_pManager->ReleaseImage(m_diFore.pImageInfo);
...
void CControlUI::SetTipText( LPCTSTR pStrTipValue )
{
	m_sTipText	= pStrTipValue;
}

CDuiString CControlUI::GetTipText() const
{
	return m_sTipText;
}
...
void CControlUI::SetBkImage(LPCTSTR pStrImage)
{
	...
	//bool bSucess = DrawImage(NULL, m_diBk);
...
bool CControlUI::SetBkImageWithResult( LPCTSTR pStrImage )
{
	if( m_diBk.sDrawString == pStrImage && m_diBk.pImageInfo != NULL ) return true;
	m_diBk.Clear();
	m_diBk.sDrawString = pStrImage;
	bool bSucess = DrawImage(NULL, m_diBk);
	if( m_bFloat && m_cxyFixed.cx == 0 && m_cxyFixed.cy == 0 && m_diBk.pImageInfo ) {
...
void CControlUI::SetToolTip(LPCTSTR pstrText)
{
	...
	if (_tcscmp(m_sToolTip.GetData(), strTemp.GetData()) != 0
		&& this->GetManager())
	{
		HWND hPaintWnd = this->GetManager()->GetPaintWindow();
		if (hPaintWnd
			&& IsWindowVisible(hPaintWnd)
			&& this->GetManager()->GetTooltipControl() == this)
		{
			::PostMessage(hPaintWnd, WM_MOUSEHOVER, 0, (LPARAM)MAKELONG(this->GetPos().left + 1, this->GetPos().top + 1));
		}
	}
...
{
	static int nSignLen = _tcslen(LANG_SIGN); 
	...
	{
		if ( _tcsnicmp(LANG_SIGN, pstrValue, nSignLen) == 0 )
		{
			//LPTSTR pstr = NULL;
			//DWORD dwId = _tcstol(pstrValue + _tcslen(LANG_SIGN), &pstr, 10);  ASSERT(pstr);    
			CDuiString strValue = GETLANGSTRING(pstrValue + nSignLen);
			SetText(strValue.GetData());
		}
		else
			SetText(pstrValue);
	}
    else if( _tcscmp(pstrName, _T("tooltip")) == 0 )
	{
		if ( _tcsnicmp(LANG_SIGN, pstrValue, nSignLen) == 0 )
		{
			...
			//DWORD dwId = _tcstol(pstrValue + _tcslen(LANG_SIGN), &pstr, 10);  ASSERT(pstr);    
			CDuiString strValue = GETLANGSTRING(pstrValue + nSignLen);
			SetToolTip(strValue.GetData());
		}
		else
			SetToolTip(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("tiptext")) == 0 )
	{
		if ( _tcsnicmp(LANG_SIGN, pstrValue, nSignLen) == 0 )
		{
			//LPTSTR pstr = NULL;
	...
	else if( _tcscmp(pstrName, _T("transparent")) == 0 ) m_bTransparent = (_tcscmp(pstrValue, _T("true")) == 0 ? true : false);
}
...
SIZE CControlUI::GetImageSize()
{
	...
	{
		sz.cx = m_diBk.pImageInfo->nX;
		sz.cy = m_diBk.pImageInfo->nY;
	}
	else
	{
		sz = CRenderEngine::GetImageSize(m_diBk.sDrawString);
	}
	return sz;
}

void CControlUI::SetTransparentable( bool bTransparentable )
{
	m_bTransparent = bTransparentable;
}

bool CControlUI::IsTransparentable()
{
