...
m_currentCaretObject(NULL)
{
	...
	::ZeroMemory(&m_ptLastMousePos, sizeof(POINT));
...
void CPaintManagerUI::SetTransparent(DWORD dwColor)
{
	if( m_hWndPaint != NULL ) {
		typedef BOOL (__stdcall *PFUNCSETLAYEREDWINDOWATTR)(HWND, COLORREF, BYTE, DWORD);
		PFUNCSETLAYEREDWINDOWATTR fSetLayeredWindowAttributes;
		...
		DWORD dwStyle = ::GetWindowLong(m_hWndPaint, GWL_EXSTYLE);
		DWORD dwNewStyle = dwStyle;
		if( true) dwNewStyle |= WS_EX_LAYERED;
		else dwNewStyle &= ~WS_EX_LAYERED;
		if(dwStyle != dwNewStyle) ::SetWindowLong(m_hWndPaint, GWL_EXSTYLE, dwNewStyle);
		fSetLayeredWindowAttributes(m_hWndPaint, dwColor, 0, LWA_COLORKEY);
	}
}
    ...
    case WM_CLOSE:
        {
            ...
            // focus loss...
			// 在100PPT播放退出的时候，回触发 CSkinFrameWnd::OnActivate导致	wincore 中ASSERT(::IsWindow(m_hWnd));诊断异常
    ...
    case WM_PAINT:
        {
			...
			if(m_bUpdateNeeded)
			{
				...
				if(!::IsRectEmpty(&rcClient))
				{
					...
					else
					{
						...
						while(pControl = m_pRoot->FindControl(__FindControlsFromUpdate, NULL, UIFIND_VISIBLE | UIFIND_ME_FIRST))
						{
			...
			if(m_bAlphaBackground)
			{
				...
				CControlUI* pControl = FindControl(_T("mainWindowLayout"));
				if(pControl && pControl->IsVisible())
				{
					RECT rect1 = pControl->GetPos();
					CRenderEngine::RestoreAlphaColorDirect(m_pBmpOffscreenBits, nClientWidth, &rect1);
				}
				//modify by caiyn 2016.01.27 解决更新日志界面 更新信息显示异常的问题
				CControlUI* pControlLogList = FindControl(_T("layoutLogList"));
				if(pControlLogList && pControlLogList->IsVisible())
				{
					RECT rect1 = pControlLogList->GetPos();
					CRenderEngine::RestoreAlphaColorDirect(m_pBmpOffscreenBits, nClientWidth, &rect1);
				}
    ...
    case WM_MOUSEHOVER:
        {
			...
			if (m_ToolLastParams.x != pt.x 
				|| m_ToolLastParams.y != pt.y)
			{
				
				::SendMessage(m_hwndTooltip, TTM_SETTOOLINFO, 0, (LPARAM) &m_ToolTip);
				m_ToolLastParams = pt;
			}
			::SendMessage(m_hwndTooltip, TTM_TRACKACTIVATE, (WPARAM)TRUE, (LPARAM) &m_ToolTip);
...
{
	CDuiString strHash = CRenderEngine::GetImageHash(bitmap);

    TImageInfo* data = static_cast<TImageInfo*>(m_ResInfo.m_ImageHash.Find(strHash.GetData()));
	if (data)
	{
		data->dwReferences ++;
		return data;
    ...
    if( !data ) data = static_cast<TImageInfo*>(m_SharedResInfo.m_ImageHash.Find(strHash.GetData()));
...
const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR bitmap, LPCTSTR type, DWORD mask, bool bUseHSL)
{
    ...
    if( !data ) {
       data = AddImage(bitmap, type, mask, bUseHSL, false);
    }
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR type, DWORD mask, bool bUseHSL, bool bShared)
{
    ...
    if( type != NULL ) {
        if( isdigit(*bitmap) ) {
            ...
            data = CRenderEngine::LoadImage(iIndex, type, mask, &strHash);
        }
    }
    else {
        data = CRenderEngine::LoadImage(bitmap, NULL, mask, &strHash);
    }
	...
	if (data)
	{
		if (bShared)
		{
            TImageInfo* pOldImageInfo = static_cast<TImageInfo*>(m_SharedResInfo.m_ImageHash.Find(strHash));
            if (pOldImageInfo)
            {
                CRenderEngine::FreeImage(pOldImageInfo);
                m_SharedResInfo.m_ImageHash.Remove(strHash);
            }

			if( !m_SharedResInfo.m_ImageHash.Insert(strHash, data) ) {
		...
		{
            TImageInfo* pOldImageInfo = static_cast<TImageInfo*>(m_ResInfo.m_ImageHash.Find(strHash));
            if (pOldImageInfo)
            {
                CRenderEngine::FreeImage(pOldImageInfo);
                m_ResInfo.m_ImageHash.Remove(strHash);
            }

			if( !m_ResInfo.m_ImageHash.Insert(strHash, data) ) {
				CRenderEngine::FreeImage(data);
				data = NULL;
			}
		}
...
void CPaintManagerUI::ReleaseImage(const TImageInfo* pImage, bool bShared /*= false*/ )
{
	if (!pImage
		|| pImage->bRes)
	{
	...
	TImageInfo* data = NULL;
	TCHAR szHash[16] = {0};
	_stprintf_s(szHash, _T("%08x"), pImage->dwHash);
	
	if (bShared) 
	{
		data = static_cast<TImageInfo*>(m_SharedResInfo.m_ImageHash.Find(szHash));
		if (data)
		{
			data->dwReferences--;
			if (data->dwReferences <= 0)
			{
				CRenderEngine::FreeImage(data) ;
				m_SharedResInfo.m_ImageHash.Remove(szHash);
			}
		}
	}
	else
	{
		data = static_cast<TImageInfo*>(m_ResInfo.m_ImageHash.Find(szHash));
		if (data)
		{
			data->dwReferences--;
			if (data->dwReferences <= 0)
			{
				CRenderEngine::FreeImage(data) ;
				m_ResInfo.m_ImageHash.Remove(szHash);
			}
		}
	}
...
void CPaintManagerUI::RemoveImage(LPCTSTR bitmap, bool bShared)
{
	...
	if (bShared) 
	{
		data = static_cast<TImageInfo*>(m_SharedResInfo.m_ImageHash.Find(strHash));
		if (data)
		{
			CRenderEngine::FreeImage(data) ;
			m_SharedResInfo.m_ImageHash.Remove(strHash);
	...
	{
		data = static_cast<TImageInfo*>(m_ResInfo.m_ImageHash.Find(strHash));
		if (data)
		{
			CRenderEngine::FreeImage(data) ;
			m_ResInfo.m_ImageHash.Remove(strHash);
		...
		if(LPCTSTR bitmap = m_ResInfo.m_ImageHash.GetAt(i)) {
			...
			if( data != NULL ) {
				if( !data->sResType.IsEmpty() ) {
					...
					pNewData = CRenderEngine::LoadImage(data->sResPath.GetData(), NULL, data->dwMask);
				}
				if( pNewData == NULL ) continue;
...
CStdPtrArray* CPaintManagerUI::FindSubControlsByProc( CControlUI* pParent, GETCONTROLPROC Proc, DWORD uFlgas )
{
	if( pParent == NULL ) pParent = GetRoot();
	ASSERT(pParent);
	m_aFoundControls.Empty();
	pParent->FindControl(__FindControlsFromProc, (LPVOID)Proc, uFlgas);
	return &m_aFoundControls;
}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlsFromProc( CControlUI* pThis, LPVOID pData )
{
	GETCONTROLPROC Proc = static_cast<GETCONTROLPROC>(pData);
	if(Proc(pThis))
	{
		pThis->GetManager()->GetFoundControls()->Add((LPVOID)pThis);
	}
...
CControlUI* CPaintManagerUI::GetTooltipControl() const
{
...
void CPaintManagerUI::DrawCaret( HDC hDC, const RECT& rcPaint )
{
	...
	if(m_bCaretActive && m_bCaretShowing && m_currentCaretObject)
	{
		...
		if(::IntersectRect(&temp, &rcPaint, &m_rtCaret))
		{
			...
			if(dwColor == 0)
				dwColor = m_SharedResInfo.m_dwDefaultFontColor;
			CRenderEngine::DrawColor(hDC, temp, dwColor);
		}
	}
}

bool	CALLBACK CPaintManagerUI::GetDistransparentAndVisibleControl( CControlUI* pCtrl )
{
	if (!pCtrl->IsTransparentable() && pCtrl->IsVisible())
	{
		return true;
	}
	return false;
