...
public:
    ...
    static TImageInfo* LoadImage(STRINGorID bitmap, LPCTSTR type = NULL, DWORD mask = 0, CDuiString* strHash = NULL, bool bAdjusted = false);
	...
	static CDuiString GetImageHash(LPCTSTR lptcsPath, LPCTSTR type = NULL);
	static SIZE GetImageSize(LPCTSTR lptcsPath);
	...
	static void RestoreAlphaColorDirect(LPBYTE pBits, int bitsWidth, PRECT rc);
};
