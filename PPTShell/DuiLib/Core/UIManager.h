...
typedef bool (CALLBACK* GETCONTROLPROC)(CControlUI*);
...
public:
	...
	CControlUI*	GetTooltipControl() const;
	...
	void SetTransparent(DWORD dwColor);
	...
	void ReleaseImage(const TImageInfo* pImage, bool bShared = false);
	...
	CStdPtrArray* FindSubControlsByProc(CControlUI* pParent, GETCONTROLPROC Proc, DWORD uFlgas);
...
private:
	...
	static CControlUI* CALLBACK __FindControlsFromProc(CControlUI* pThis, LPVOID pData);

	static bool			CALLBACK GetDistransparentAndVisibleControl(CControlUI* pCtrl);
