...
namespace DuiLib
{
	...
	void CContainerUI::SetNeedCalHorizenWidth( bool bNeed )
	{
		m_bNeedCalHorizen = bNeed;
	}

	bool CContainerUI::GetNeedCalHorizenWidth()
	{
		return m_bNeedCalHorizen;
	}
	...
	void CContainerUI::ProcessScrollBar(RECT rc, int cxRequired, int cyRequired)
	{
		...
		if (m_bNeedCalHorizen)
		{
			if (NULL == m_pHorizontalScrollBar && !m_pHorizontalScrollBar->IsVisible())
			{
				return;
			}
