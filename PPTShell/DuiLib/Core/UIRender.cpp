...
static wstring AnsiToUnicode(string ansi)
{
	// ansi --> unicode
...
#ifdef _UNICODE	
static wstring Str2Unicode(const wstring& str)
{
	return str;
}
#else
static wstring Str2Unicode(const string& str)
{
...
CDuiString CRenderEngine::GetImageHash( LPCTSTR lptcsPath, LPCTSTR type )
{
	LPBYTE pData = NULL;
	DWORD dwSize = 0;
	...
	do 
	{
		if( type == NULL ) {
			...
			if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
				...
				ZIPENTRY ze; 
				int i; 
				if( FindZipItem(hz, lptcsPath, true, &i, &ze) != 0 ) break;
				dwSize = ze.unc_size;
				if( dwSize == 0 ) break;
				pData = new BYTE[ dwSize ];
			...
			HRSRC hResource = ::FindResource(CPaintManagerUI::GetResourceDll(), lptcsPath, type);
	...
	while (!pData)
	{
		...
		HANDLE hFile = ::CreateFile(lptcsPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, \
			FILE_ATTRIBUTE_NORMAL, NULL);
		if( hFile == INVALID_HANDLE_VALUE ) break;
		dwSize = ::GetFileSize(hFile, NULL);
		if( dwSize == 0 ) break;
	...
	DWORD dwHash = crc_32(pData, dwSize);
	delete[] pData;
	pData = NULL;

	TCHAR szBuffer[16] = {0};
	_stprintf_s(szBuffer, _T("%08x"), dwHash);
	return szBuffer;
}

TImageInfo* CRenderEngine::LoadImage( STRINGorID bitmap, LPCTSTR type /*= NULL*/, DWORD mask /*= 0*/, CDuiString* strHash /*= NULL*/, bool bAdjusted /*= false*/ )
{
    LPBYTE	pData = NULL;
    DWORD	dwSize = 0;
	bool	bResource = true;
	CDuiString sFilePath;
	do 
	{
		if( type == NULL ) {
			CDuiString sFile = CPaintManagerUI::GetResourcePath();
	...
	else
	{
		if (strHash)
		{
			dwHash = _tcstoul(strHash->GetData(), NULL, 16);
		}
	}
	...
	unsigned short SOI = *(unsigned short*)pData; 
    delete[] pData;
	if( !pImage ) {
		//if is jpeg but can't support by stbi_image
		if (n >= 4 && dwSize > 2 && SOI == 0xD8FF && !bAdjusted)
		{
			...
			_stprintf_s(szTempFileName, _T("%sfixed_%x.jpg"), szTempPath, dwHash);

#ifndef _UNICODE
			wstring strOriFile = AnsiToUnicode(sFilePath.GetData());
			string	strDestFile;
			wstring	strDestFileW;

			strDestFileW	= AnsiToUnicode(szTempFileName);
			strDestFile		= szTempFileName;

#else
			wstring	strDestFile;
			wstring	strDestFileW;
			wstring strOriFile = sFilePath.GetData();

			strDestFileW	= szTempFileName;
			strDestFile		= szTempFileName;
#endif
			
			if (GetFileAttributes(szTempFileName) == INVALID_FILE_ATTRIBUTES)
			{
				...
				if (x > cx && y > cy
					|| x == 0 && y == 0)
				{
					CompressImagePixel(strOriFile.c_str(), strDestFileW.c_str(), cy, cx, L"image/jpeg");
				}
				else
				{
					CompressImagePixel(strOriFile.c_str(), strDestFileW.c_str(), y, x, L"image/jpeg");
				}
			}

			return LoadImage(strDestFile.c_str(), type, mask, strHash, true);
		}
		return NULL;
	}
...
SIZE CRenderEngine::GetImageSize( LPCTSTR lptcsPath )
{

	SIZE	size = {0};
	
	HANDLE	hFile = ::CreateFile(lptcsPath,
		GENERIC_READ,
		FILE_SHARE_READ,
		NULL,
		OPEN_EXISTING,
	...
	if( hFile == INVALID_HANDLE_VALUE ){
		...
		strPath += lptcsPath;
		hFile = ::CreateFile(strPath.GetData(),
			GENERIC_READ,
			FILE_SHARE_READ,
			NULL,
			OPEN_EXISTING,
	...
	Image* pImage = new Image(Str2Unicode(lptcsPath).c_str());
	if (!pImage)
	{
		return size;
	}
...
// 	}
// 	
// 	LPBYTE pImage = stbi_load_from_memory(pData, dwSize, (int*)&size.cx, (int*)&size.cy, NULL, 4);
// 	delete[] pData;
// 	if( pImage ) {
// 		stbi_image_free(pImage);
// 	}
...
void CRenderEngine::RestoreAlphaColorDirect( LPBYTE pBits, int bitsWidth, PRECT rc )
{
	for(int i = rc->top; i < rc->bottom; ++i)
	{
		for(int j = rc->left; j < rc->right; ++j)
