...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if( uMsg == WM_KILLFOCUS ) lRes = OnKillFocus(uMsg, wParam, lParam, bHandled);
			...
			m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_TABSWITCH);
		}
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0),m_dwTipTextColor(0xFFBAC0C5)
	{
		SetTextPadding(CDuiRect(4, 3, 4, 3));
		SetBkColor(0xFFFFFFFF);
		m_bHandleSpecialKeydown = false;
	}
	...
	void CEditUI::Event( TEventUI& event )
	{
		...
		{
			if (m_bHandleSpecialKeydown && m_pWindow != NULL)
			{
				m_pWindow->HandleMessage(WM_KEYDOWN, event.chKey, 0);
			}
			
		}
	}

	void CEditUI::SetHandleSpecialKeydown( bool bHandle )
	{
		m_bHandleSpecialKeydown = bHandle;
	}

	void CEditUI::SetTipText(LPCTSTR pStrTipValue)
	{
	...
	CDuiString CEditUI::GetTipText()
	{
		return CControlUI::GetTipText();
	}

	void CEditUI::SetTipTextColor( LPCTSTR pStrColor )
	{
