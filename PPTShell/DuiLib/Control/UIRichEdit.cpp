...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
	...
	if(m_re->GetLineSpacing()>0)
	{
		/*pf.dyLineSpacing=m_re->GetLineSpacing();
		pf.bLineSpacingRule = 3;
		LRESULT lResult;
		HRESULT a = GetTextServices()->TxSendMessage(EM_SETPARAFORMAT, 0, (LPARAM)&pf, &lResult);*/
		cf.yOffset = m_re->GetLineSpacing()*10;
		LRESULT lResult;
		HRESULT result = GetTextServices()->TxSendMessage(EM_SETCHARFORMAT, 0, (LPARAM)&cf, &lResult);
	}
...
{
	m_rtCaret.right = m_rtCaret.left + xWidth;
	m_rtCaret.bottom = m_rtCaret.top + yHeight;
...
{
	bool result = true;
	int width = m_rtCaret.right - m_rtCaret.left;
	int height = m_rtCaret.bottom - m_rtCaret.top;
	m_rtCaret.left = x;
	m_rtCaret.top = y;
	m_rtCaret.right = x + width;
	m_rtCaret.bottom = y + height;
	if (m_re->GetManager()->IsBackgroundTransparent())
	{
...
void CTxtWinHost::SetLineSpacing( int lineSpacing )
{
	/*pf.dyLineSpacing=m_re->GetLineSpacing();
		pf.bLineSpacingRule = 3;
		LRESULT lResult;
		HRESULT a = GetTextServices()->TxSendMessage(EM_SETPARAFORMAT, 0, (LPARAM)&pf, &lResult);*/
		cf.yOffset = lineSpacing * 10;
		LRESULT lResult;
		HRESULT result = GetTextServices()->TxSendMessage(EM_SETCHARFORMAT, 0, (LPARAM)&cf, &lResult);
}

RECT CTxtWinHost::GetCaretPos()
{
	return m_rtCaret;
}
...
CRichEditUI::CRichEditUI() : m_pTextHost(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
    m_bWantCtrlReturn(true), m_bRich(true), m_bReadOnly(false), m_bWordWrap(false), m_dwTextColor(0), m_iFont(-1), 
	m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bInited(false),m_uButtonState(0),
	m_dwTipTextColor(0xFFBAC0C5)
{
	m_nLineSpacing = 0;
...
{
    m_bRich = bRich;
    if( m_pTextHost ) m_pTextHost->SetRichTextFlag(bRich);
}
...
void CRichEditUI::SetReadOnly(bool bReadOnly)
{
    ...
    if( m_pTextHost ) m_pTextHost->SetReadOnly(bReadOnly);
}
...
void CRichEditUI::SetWordWrap(bool bWordWrap)
{
    ...
    if( m_pTextHost ) m_pTextHost->SetWordWrap(bWordWrap);
}
...
void CRichEditUI::SetFont(int index)
{
    m_iFont = index;
    if( m_pTextHost ) {
        m_pTextHost->SetFont(GetManager()->GetFont(m_iFont));
    }
}
...
void CRichEditUI::SetTextColor(DWORD dwTextColor)
{
    ...
    if( m_pTextHost ) {
        m_pTextHost->SetColor(dwTextColor);
    }
}
...
void CRichEditUI::SetLimitText(int iChars)
{
    m_iLimitText = iChars;
    if( m_pTextHost ) {
        m_pTextHost->LimitText(m_iLimitText);
    }
}
...
CDuiString CRichEditUI::GetText() const
{
	...
	if(m_pTextHost)
	{
		m_pTextHost->SetLineSpacing(m_nLineSpacing);
	}
    return sText;
}
...
{
    m_sText = pstrText;
    if( !m_pTextHost ) return;
    SetSel(0, -1);
    ReplaceSel(pstrText, FALSE);
}

void CRichEditUI::SetTipText(LPCTSTR pStrTipValue)
{
	CControlUI::SetTipText(pStrTipValue);

	NeedParentUpdate();
}

CDuiString CRichEditUI::GetTipText()
{
	return CControlUI::GetTipText();
}

void CRichEditUI::SetTipTextColor( LPCTSTR pStrColor )
{
	if( *pStrColor == _T('#')) pStrColor = ::CharNext(pStrColor);
	LPTSTR pstr = NULL;
	DWORD clrColor = _tcstoul(pStrColor, &pstr, 16);

	m_dwTipTextColor = clrColor;
}

DWORD CRichEditUI::GetTipTextColor()
{
...
{
    if( !m_pTextHost ) return CDuiString();
    CHARRANGE cr;
    cr.cpMin = cr.cpMax = 0;
    TxSendMessage(EM_EXGETSEL, 0, (LPARAM)&cr, 0);
    LPWSTR lpText = NULL;
...
{ 
	...
	if(::OpenClipboard(NULL))
	{
		...
		HGLOBAL clipbuffer;
		TCHAR *buffer;
		clipbuffer = ::GlobalAlloc(GMEM_DDESHARE, sText.GetLength()+1);
		buffer = (TCHAR *)::GlobalLock(clipbuffer);
		_tcscpy(buffer, sText.GetData());
...
void CRichEditUI::DoInit()
{
    ...
    CreateHost(this, &cs, &m_pTextHost);
    if( m_pTextHost ) {
        m_pTextHost->SetTransparent(TRUE);
        LRESULT lResult;
        m_pTextHost->GetTextServices()->TxSendMessage(EM_SETLANGOPTIONS, 0, 0, &lResult);
        m_pTextHost->OnTxInPlaceActivate(NULL);
        m_pManager->AddMessageFilter(this);
		if (GetText() == _T("") && !m_sTipText.IsEmpty())
		{
			SetText(m_sTipText);
			m_pTextHost->SetColor(m_dwTipTextColor);
		}
    }
...
void CRichEditUI::SetScrollPos(SIZE szPos)
{
    ...
    if( cy != 0 ) {
        int iPos = 0;
        if( m_pTextHost && !m_bRich && m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) 
...
void CRichEditUI::DoEvent(TEventUI& event)
{
    ...
    {
        if( m_pTextHost && m_pTextHost->DoSetCursor(NULL, &event.ptMouse) ) {
            return;
        }
    }
    if( event.Type == UIEVENT_SETFOCUS ) {
        if( m_pTextHost ) {
            m_pTextHost->OnTxInPlaceActivate(NULL);
			if (GetText() == m_sTipText)
			{
				SetText(_T(""));
			}
			m_pTextHost->SetColor(m_dwTextColor);
    ...
    if( event.Type == UIEVENT_TIMER ) {
        if( m_pTextHost ) {
            m_pTextHost->GetTextServices()->TxSendMessage(WM_TIMER, event.wParam, event.lParam, 0);
        } 
    }
...
void CRichEditUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    if( m_pTextHost ) {
        ...
        m_pTextHost->SetClientRect(&rcRich);
...
void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("tiptextcolor")) == 0 ) SetTipTextColor(pstrValue);
	...
	else if( _tcscmp(pstrName, _T("linespacing")) == 0 )
	{
...
LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
	...
	if( uMsg == WM_CHAR  ) 
	{
		if( wParam == VK_TAB)
		{
			GetManager()->SendNotify((CControlUI*)this, DUI_MSGTYPE_TABSWITCH);
			return 0;
		}
	}
    ...
    if( (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) {
        ...
        DWORD dwHitResult = m_pTextHost->IsCaptured() ? HITRESULT_HIT : HITRESULT_OUTSIDE;
	...
	if((uMsg ==WM_KEYDOWN&&TCHAR(wParam) == VK_DELETE)||uMsg ==WM_IME_CHAR|| uMsg == WM_CHAR)
	{
		GetManager()->SendNotify((CControlUI*)this, DUI_MSGTYPE_TEXTCHANGED);
	}
    if( Hr == S_OK ) 
	{
...
void CRichEditUI::PaintStatusImage(HDC hDC)
{
	...
	TDrawInfo drawInfo;
	memset(&drawInfo, 0, sizeof(TDrawInfo));
	if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
		if( !m_sDisabledImage.IsEmpty() ) {
			drawInfo.sDrawString	= m_sDisabledImage;
			if( !DrawImage(hDC, drawInfo) ) m_sDisabledImage.Empty();
			else return;
		}
	}
	else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
		if( !m_sFocusedImage.IsEmpty() ) {
			drawInfo.sDrawString	= m_sFocusedImage;
			if( !DrawImage(hDC, drawInfo) ) m_sFocusedImage.Empty();
			else return;
		}
	}
	else if( (m_uButtonState & UISTATE_HOT ) != 0 ) {
		if( !m_sHotImage.IsEmpty() ) {
			drawInfo.sDrawString	= m_sHotImage;
			if( !DrawImage(hDC, drawInfo) ) m_sHotImage.Empty();
			else return;
		}
	}

	if( !m_sNormalImage.IsEmpty() ) {
		drawInfo.sDrawString	= m_sNormalImage;
		if( !DrawImage(hDC, drawInfo) ) m_sNormalImage.Empty();
		else return;
	}
}

void CRichEditUI::SetLineSpacing( int lineSpacing )
{
	m_nLineSpacing = lineSpacing;
	if(m_pTextHost)
	{
		m_pTextHost->SetLineSpacing(lineSpacing);
	}
	Invalidate();
}

int CRichEditUI::GetLineSpacing()
{
