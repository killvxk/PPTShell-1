...
bool CFileOperate::GetModuleFilePath(char* modulefilepath, int length, HMODULE hModule)
{
	...
	modulefileallpath_length = ::GetModuleFileName(hModule, modulefileallpath, sizeof(modulefileallpath) - 1);
...
bool CFileOperate::CheckFileIsExist(const char* filePath)
{
	...
	DWORD dwfileFlag = 0;
	dwfileFlag = ::GetFileAttributes(filePath);
...
bool CFileOperate::CreateMultiFolder(const char *pDirPath)
{
	...
	if ( E_FILE_TYPE_NOT_EXIST == fileType )//目录不存在创建目录
	{
		...
		{
			if (TRUE == ::CreateDirectory(strDirPath.c_str(), NULL))
			{
				return true;
			}
			else
...
E_FILE_TYPE CFileOperate::CheckFileAttribute(const char *pFilePath)
{
	...
	DWORD dwfileFlag = 0;
	dwfileFlag = ::GetFileAttributes(pFilePath);
	if (0xFFFFFFFF == dwfileFlag)
	{
		return E_FILE_TYPE_NOT_EXIST;
	}
...
bool CFileOperate::DeleteFolder(const char *pFolder)
{
	...
	if (FALSE == ::RemoveDirectory(pFolder) ){ return false; }
	return true;
}
...
bool CFileOperate::GetAboslutePath(const char* filepath, char *aboslutepath, int lenght)
{
	...
	if ( !flag )
	{
		DWORD dw = ::GetFullPathName(filepath, lenght - 1, aboslutepath, NULL);
	...
	do
	{
		if (0 != strcmp(".", FindFileData.cFileName) && 0 != strcmp("..", FindFileData.cFileName))
		{
			...
			if (FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				...
				{
					if (FALSE == ::RemoveDirectory(szNextDir))
					{ 
						bFlag = false; break;
					}
				}
...
bool CFileOperate::GetFirstFileName(const char *pFolder, HANDLE &handle, WIN32_FIND_DATA& FindFileData)
{
	if ( NULL == pFolder ) return false;
	...
	handle = ::FindFirstFile(DirSpec, &FindFileData);
