...
// 	if (-1 == lSize) { goto _Out;}
	...
	if (NULL == ppBuff)
		...
		while (true)
		{
			...
			if (301 == nAckStatus || 302 == nAckStatus)
			{
				closesocket(conn);
				conn = NULL;
				strlwr(sTmp);
		...
		while (0 < (nRecLen = recv(conn, &szBuf[lDownloadSize], nBufSize - lDownloadSize - 1, 0)))
		{
			lDownloadSize += nRecLen;
			if(lDownloadSize == nBufSize - 1)
			{
				nBufSize = nBufSize * 2;
				char * pszBuff = new char[nBufSize];
				memcpy(pszBuff, szBuf, lDownloadSize);
				delete szBuf;
				szBuf = pszBuff;
			}
		}
...
BOOL CNet::DownFile(char* szUrl, char* szPath, char* szName, char** ppBuff, void* p) 
{
	SOCKET conn = NULL;
	...
	CAPClientInterface * pAPClient = (CAPClientInterface *)p;
	if(pAPClient)
		pAPClient->m_updateParam.dwDownloadFileSize = lSize;
...
// 	memcpy(szBuf, &sTmp[n], nRecLen-n);
	...
	while (lConfDownloadSize < lSize && 0 < (nRecLen = recv(conn, szBuf, sizeof(szBuf), 0)))
	{
		...
		if(pAPClient)
		{
			...
			{
				pAPClient->m_updateParam.dwDownloadingSize = lConfDownloadSize;
				pAPClient->m_updateParam.updateInfoParam.dwCurrentProgress = ((float)pAPClient->m_updateParam.dwDownloadingSize / pAPClient->m_updateParam.dwDownloadFileSize) * 100;
				pAPClient->m_updateParam.updateInfoParam.dwTotalProgress = pAPClient->m_updateParam.dwPreTotalProgress + 
					((float)pAPClient->m_updateParam.dwDownloadingSize / pAPClient->m_updateParam.dwDownloadFileSize) *
					( ((float)1 / pAPClient->m_updateParam.dwDownloadCount ) * 100);

				dwElapseTime = GetTickCount() - dwLastTime;
				pAPClient->m_updateParam.updateInfoParam.fDownloadSpeed = (float)dwPregressSize / (dwElapseTime / 1000.0f);
				dwLastTime = GetTickCount();
				dwPregressSize = 0;
			}
			if(g_pUpdateBuff)
				memcpy(g_pUpdateBuff,&pAPClient->m_updateParam.updateInfoParam,sizeof(UpdateInfo_Param));
		}
	}
...
BOOL CNet::DownFile( char* szUrl, char* szPath, void* p )
{
	SOCKET conn = NULL;
	char	szBuf[SIZE_200K];
